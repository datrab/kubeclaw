...
// Path to redis.js — resolved relative to this file so it works both in the
// Docker image (/app/skills/redis.js) and when run locally from any directory.
const _REDIS_JS_DEFAULT = path.join(path.dirname(fileURLToPath(import.meta.url)), 'redis.js');
...
function validateConfig(config, progress) {
  ...
  config.agents.buster.redis_js_path ??= _REDIS_JS_DEFAULT;
...
function buildBusterPayload(config, progress, moduleId, taskType, taskPrompt, status, opts = {}) {
  ...
  if (taskType === 'module_test') {
    ...
    return {
      ...base,
      // v2 buster field names (buster-orchestrator.js reads these)
      module_id: moduleId,
      prompt: taskPrompt,
      suites: mod?.test_suites || [],
      // v1 compat aliases kept so older images still work
  ...
  if (taskType === 'gate_test') {
    ...
    return {
      ...
      // Gate suites + config — without these the orchestrator falls back to
      // DEFAULT_SUITES (['build','health']) and DEFAULT_CONFIG ({serve:{type:'static'}}),
...
function dispatchRedisTask(config, progress, agentType, moduleId, taskType, payload, status = null, opts = {}) {
  ...
  const redisJsPath = validateSafePath(
    agentConfig.redis_js_path || _REDIS_JS_DEFAULT,
...
async function getRedisModule(config) {
  ...
  const redisPath = agentConf?.redis_js_path || _REDIS_JS_DEFAULT;
...
async function readCompletionFromRedis(config, moduleId) {
  ...
  try {
    ...
    const result = await redisMod.readCompletion(stream, moduleId);
    if (result) return result;
  } catch (e) {
    log('DEBUG', `Redis completion read failed for ${moduleId}: ${e.message}`);
  }

  // Fallback: buster v2 emits buster.task_completed to the telemetry stream
  // instead of the completion stream. Read it directly as a completion signal.
  try {
    const redisMod = await getRedisModule(config);
    if (!redisMod) return null;
    const client = redisMod.client;
    const telemetryKey = `buster:telemetry:${config.project}:${moduleId}`;
    const entries = await client.xrevrange(telemetryKey, '+', '-', 'COUNT', 30);
    for (const [, fields] of entries) {
      const dataStr = fields[fields.indexOf('data') + 1] || '{}';
      let evt;
      try { evt = JSON.parse(dataStr); } catch { continue; }
      if (evt.type === 'buster.task_completed') {
        const outcome = evt.data?.outcome || 'FAIL';
        log('DEBUG', `Buster telemetry completion for ${moduleId}: outcome=${outcome}`);
        return {
          status: outcome === 'PASS' ? 'PASS' : 'FAIL',
          source: 'buster-telemetry',
          summary: evt.data?.reason || evt.data?.suite_summary || '',
          module: moduleId,
        };
      }
    }
  } catch (e) {
    log('DEBUG', `Buster telemetry poll failed for ${moduleId}: ${e.message}`);
  }
...
async function _runBusterGateOnce(config, progress, gateId, gate, model, timeout, instructions, attempt) {
  ...
  await archiveModuleCompletions(config, gateId);
  ...
  const result = await pollGeneric(config, async () => {
    ...
    // Catches pre-test FAILs from the orchestrator (no subagent spawned → no output file).
    // Same pattern as pollDual() for module tests.
    try {
      const redisEntry = await readCompletionFromRedis(config, gateId);
      if (redisEntry && redisEntry.status) {
        const mappedStatus = mapRedisStatus(redisEntry.status);
