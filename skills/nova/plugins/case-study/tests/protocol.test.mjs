...
const valid={status:'generated',markdown};assert.deepEqual(parseCaseStudy(valid,input),{...valid,projectId:'api',runId:'run-1'});
for(const bad of[{...valid,identity:{projectId:'api',runId:'run-1'}},{...valid,extra:true},{...valid,markdown:markdown.replace('## Challenge','## Context')}])assert.throws(()=>parseCaseStudy(bad,input));
