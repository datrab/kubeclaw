...
export function buildRequest(agent:string,input:CaseStudyInput):Readonly<Record<string,unknown>>{
  return {protocol:'kubeclaw.case-study.v2',agent,identity:{projectId:input.projectId,runId:input.runId},task:input.task,
    facts:input.facts,requiredSections,groundingRules:['Use only supplied facts.','Do not invent metrics, quotes, dates, or outcomes.']};
}
export function parseCaseStudy(value:unknown,input:CaseStudyInput):CaseStudy{
  if(!value||typeof value!=='object'||Array.isArray(value)) throw new Error('case study must be an object');
  const source=value as Record<string,unknown>;
  if(Object.keys(source).some((key)=>!['status','projectId','runId','markdown'].includes(key))||source.status!=='generated') throw new Error('case study shape is invalid');
  if(source.projectId!==input.projectId||source.runId!==input.runId) throw new Error('case study identity mismatch');
