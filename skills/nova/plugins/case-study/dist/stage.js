import { buildRequest, parseCaseStudy } from './protocol.js';
export async function execute(input, context) {
    const agent = context.contract.config.agent;
    if (typeof agent !== 'string' || !agent.trim())
        throw new Error('case study agent is not configured');
    let study;
    try {
        ...
        return { schemaVersion: 'stage-result.v2', outcome: 'blocked', reason: { code: 'case_study.invalid_output', message: error instanceof Error ? error.message : String(error) }, artifacts: [] };
    }
