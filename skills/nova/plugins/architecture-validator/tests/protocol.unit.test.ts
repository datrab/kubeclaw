...
assert.equal(parseArchitectureOutput({ result: {
  verdict: 'request_fix', summary: 'needs refinement', findings: [warning], checkedFiles: ['src/api.ts'],
} }).findings[0].scope, 'integration_boundary');

assert.throws(() => parseArchitectureOutput({ result: {
  verdict: 'passed', summary: 'contradiction', findings: [warning], checkedFiles: ['x'],
} }), /contradicts/);
assert.throws(() => parseArchitectureOutput({ result: {
...
assert.throws(() => parseArchitectureOutput({ result: {
  verdict: 'blocked', summary: 'not blocking', findings: [warning], checkedFiles: ['x'],
} }), /requires a blocking or error finding/);
