...
export function buildSummary(input:SummaryInput):ProjectSummary{
  const projectId=safe(input.projectId);const runId=safe(input.runId);
  if(!projectId||!runId) throw new Error('project and run identities are required');
  ...
  if(modulesPassed>modulesTotal) throw new Error('passed modules exceed total modules');
  const testTotal=testsPassed+testsFailed;
  if(!Number.isSafeInteger(testTotal)) throw new Error('test total exceeds safe integer range');
