import type { ArtifactRef, PluginInvocationContext, StageResult, WaitRequest } from "@kubeclaw/plugin-sdk";
...
function waitFrom(value:unknown,expected:{waitId:string;issuerId:string;expiresAt:string;projectId:string}):WaitRequest{const item=record(value);const wait=record(item.wait);const issuer=record(wait.authorizedIssuer);const request=record(wait.request);if(item.created!==true||wait.schemaVersion!=="wait-request.v2"||wait.waitId!==expected.waitId||wait.kind!=="signal"||wait.signalType!=="prism.approval.resolved"||issuer.type!=="operator"||issuer.id!==expected.issuerId||wait.expiresAt!==expected.expiresAt||request.projectId!==expected.projectId)throw new Error("PRISM_DESIGN_WAIT_INVALID");return wait as unknown as WaitRequest;}
export async function execute(input:Input,context:PluginInvocationContext):Promise<StageResult>{
  ...
  try{
    ...
    if(!architectureRef||architectureRef.digest!==input.architectureArtifact.contentDigest)throw new Error("PRISM_DESIGN_ARCHITECTURE_REFERENCE_INVALID");
    ...
    if(architectureResponse.digest!==architectureRef.digest||architectureResponse.sizeBytes!==architectureRef.sizeBytes)throw new Error("PRISM_DESIGN_ARCHITECTURE_PROOF_INVALID");
    ...
    const stored=await context.invoke("artifacts.write",{operation:"put_json",resource:{type:"artifact.object",canonicalId:`prism-baseline:${input.projectId}:${result.bundleDigest}`},payload:{namespace:"kubeclaw.prism",mediaType:"application/json",value:{...result,designRequest:request,approvedArchitectureDigest:input.architectureArtifact.contentDigest}}});
