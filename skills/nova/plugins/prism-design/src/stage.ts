...
export async function execute(input:Input,context:PluginInvocationContext):Promise<StageResult>{
  ...
  try{
    ...
    const response=await context.invoke("runtime.dispatch",{operation:"dispatch",resource:{type:"runtime.agent",canonicalId:settings.agent},payload:withRuntimeDispatchProfile({request,idempotencyKey:`${input.runId}:prism:${input.architectureArtifact.contentDigest}:${phase}`},context.contract.runtimeDispatchProfile)});const result=response.result as Record<string,unknown>;
    if(!approved){
