...
export interface GateInput{readonly runId:string;readonly gateId:string;readonly attempt:number;readonly task:string;readonly suiteEvidence:readonly{readonly suite:string;readonly passed:boolean;readonly summary:string}[];readonly suitePlan:TestSuitePlan}
...
export function parseVerdict(value:unknown,input:GateInput):GateVerdict{
 ...
 if(Object.keys(source).some((key)=>!['outcome','summary','failureClass','findings'].includes(key))||!outcomes.includes(String(source.outcome))||!classes.includes(String(source.failureClass)))throw new Error('gate verdict shape is invalid');
