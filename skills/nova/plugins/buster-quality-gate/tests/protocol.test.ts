...
const valid = { outcome: 'passed' as const, runId: 'run-1', gateId: 'quality', attempt: 1, summary: 'Passed.', failureClass: 'none' as const, findings: [] };
...
for (const failureClass of ['test_failure', 'contract', 'configuration', 'infrastructure', 'rate_limit', 'timeout'] as const) {
  ...
  assert.deepEqual(parseVerdict(failed, input), failed);
}
