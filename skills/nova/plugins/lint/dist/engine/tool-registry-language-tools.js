...
import { affectedTypeScriptConfigs, eslintFindingSeed, isJavaScriptOrTypeScriptProject, registerTool, requireNumber, requireString, TOOL_OUTPUT_PREVIEW_MISSING, uniqueTypeScriptFindings, } from './tool-registry-core.js';
...
registerTool({
    ...
    run: (ctx) => {
        ...
        if (configs.length === 0)
            return notApplicable('No configured TypeScript project is affected by the requested scope.');
...
registerTool({
    ...
    run: (ctx) => {
        ...
        if (!parsed.ok)
            return failParse(ctx, 'ruff', parsed, result, target);
...
registerTool({
    ...
    run: (ctx) => {
        ...
        const parsed = tryParseJson(result.stdout);
        if (!parsed.ok)
            return failParse(ctx, 'shellcheck', parsed, result, ctx.repoRoot);
        ...
        if (result.exitCode !== 0 && findings.length === 0)
            return failParse(ctx, 'shellcheck', { error: 'non-zero exit without diagnostics' }, result, ctx.repoRoot);
        return {
            errors: findings.filter((f) => f.severity === 'error').length,
            warnings: findings.filter((f) => f.severity === 'warning').length,
            findings,
...
registerTool({
    ...
    run: (ctx) => {
        ...
        const parsed = tryParseJson(result.stdout);
        if (!parsed.ok)
            return failParse(ctx, 'eslint', parsed, result, target);
        const findings = [];
