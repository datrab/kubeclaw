...
import { objectRecord as recordValue, selectPresentValue, textValue, } from '../support/value-boundary.js';
import { selectTruthyValue } from '../support/optional-absence.js';
import { isJavaScriptOrTypeScriptProject, LINT_VULNERABILITY_FOUND, npmAuditSeverity, registerTool, requireString, } from './tool-registry-core.js';
...
registerTool({
    ...
    run: (ctx) => {
        ...
        if (!parsed.ok)
            return failParse(ctx, 'npm-audit', parsed, result, path.join(ctx.repoRoot, 'package.json'));
        ...
        if (result.exitCode !== 0 && findings.length === 0)
            return failParse(ctx, 'npm-audit', { error: 'non-zero exit without vulnerability findings' }, result, path.join(packageRoot, 'package.json'));
        return {
...
registerTool({
    ...
    run: (ctx) => {
        ...
        const parsed = tryParseJson(result.stdout);
        if (!parsed.ok)
            return failParse(ctx, 'pip-audit', parsed, result, path.join(ctx.repoRoot, 'requirements.txt'));
