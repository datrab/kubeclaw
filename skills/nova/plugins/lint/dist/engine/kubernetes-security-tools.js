...
function documentForResource(rendered, resource) {
    for (const document of rendered.split(/^---\s*$/m)) {
        if (resourceIdentity(document, 1) === resource)
            return document;
    }
    return '';
}
function normalizedDocumentDigest(document) {
    const normalized = document.trim().replace(/\r\n/g, '\n');
    return crypto.createHash('sha256').update(normalized).digest('hex');
}
function isCanonicalLintConfigFalsePositive(rendered, issue, today = new Date().toISOString().slice(0, 10), filter = CANONICAL_LINT_CONFIG_FALSE_POSITIVE) {
    if (today > filter.expires)
        return false;
    if (issue.ID !== filter.code)
        return false;
    if (issueMessage(issue) !== filter.message)
        return false;
...
function trivyFindings(ctx, chartDir, rendered, issues) {
    ...
    return issues.filter((issue) => !isCanonicalLintConfigFalsePositive(rendered, issue)).map((issue) => {
        const resource = resourceIdentity(rendered, issue.CauseMetadata?.StartLine);
...
function runTrivyKubernetes(ctx) {
    ...
    for (const chartDir of configuredTargetPaths(ctx)) {
        ...
        try {
            ...
            if (!parsed.ok)
                return failParse(ctx, 'trivy-kubernetes', parsed, result, chartDir);
            if (result.exitCode !== 0 && result.exitCode !== 1)
                return failParse(ctx, 'trivy-kubernetes', { error: `unexpected exit code ${result.exitCode}` }, result, chartDir);
            ...
            if (result.exitCode !== 0 && issues.length === 0)
                return failParse(ctx, 'trivy-kubernetes', { error: 'non-zero exit without misconfigurations' }, result, chartDir);
        }
        finally {
