...
function dependencyGraph(modules) {
    const graph = new Map();
    for (const module of modules) {
        if (typeof module.source !== 'string')
            continue;
        if (!graph.has(module.source))
            graph.set(module.source, new Set());
        for (const dependency of Array.isArray(module.dependencies) ? module.dependencies : []) {
            ...
            if (!graph.has(dependency.resolved))
                graph.set(dependency.resolved, new Set());
        }
    }
    return graph;
}
function collectCompletedComponent(state, node) {
    const component = [];
    while (state.stack.length > 0) {
        const member = state.stack.pop();
        if (member === undefined)
            throw new Error('Circular dependency component stack was unexpectedly empty.');
...
function visitDependencyNode(state, node) {
    ...
    if (state.lowLinks.get(node) === state.indexes.get(node))
        collectCompletedComponent(state, node);
}
function stronglyConnectedComponents(modules) {
    ...
    for (const node of [...graph.keys()].sort()) {
        if (!state.indexes.has(node))
            visitDependencyNode(state, node);
    }
...
function dependencyCruiserFindings(ctx, modules) {
    ...
    for (const module of modules) {
        ...
        for (const dependency of Array.isArray(module.dependencies) ? module.dependencies : []) {
            if (!sourceLayer || dependency.coreModule || dependency.couldNotResolve || !dependency.resolved)
                continue;
            const targetLayer = layerForFile(ctx.policy.architecture.layers, dependency.resolved);
            if (!targetLayer || sourceLayer.may_depend_on.includes(targetLayer.id))
                continue;
            findings.push({
                file: source,
                line: null,
...
function dependencyCruiserTool() {
    return {
        ...
        run: (ctx) => {
            ...
            if (!parsed.ok)
                failParse(ctx, 'dependency-cruiser', parsed, result);
            if (!Array.isArray(parsed.data?.modules))
                failParse(ctx, 'dependency-cruiser', { error: 'missing modules array' }, result);
            if (result.exitCode !== 0)
                failParse(ctx, 'dependency-cruiser', { error: 'non-zero execution status' }, result);
...
function knipTool() {
    return {
        ...
        binary: 'knip',
        run: (ctx) => {
            if (!ctx.tool.config_path || !fs.existsSync(ctx.tool.config_path))
                failConfigMissing('knip-config-missing', 'Knip requires its exact configured path.');
            ...
            const parsed = tryParseJson(result.stdout);
            if (!parsed.ok)
                failParse(ctx, 'knip', parsed, result);
            if (!Array.isArray(parsed.data?.issues))
                failParse(ctx, 'knip', { error: 'missing issues array' }, result);
            if (result.exitCode !== 0)
                failParse(ctx, 'knip', { error: 'non-zero execution status' }, result);
            return findingsResult(knipFindings(parsed.data));
        },
    };
}
function executeJscpd(ctx, configPath, targets, label) {
    ...
    try {
        ...
        if (!fs.existsSync(reportPath))
            failParse(ctx, 'jscpd', { error: 'missing JSON report' }, result);
        const parsed = tryParseJson(fs.readFileSync(reportPath, 'utf8'));
        if (!parsed.ok)
            failParse(ctx, 'jscpd', parsed, result);
        if (!Array.isArray(parsed.data?.duplicates))
            failParse(ctx, 'jscpd', { error: 'missing duplicates array' }, result);
        if (result.exitCode !== 0)
            failParse(ctx, 'jscpd', { error: 'non-zero execution status' }, result);
...
function jscpdTool() {
    return {
        ...
        run: (ctx) => {
            if (!ctx.tool.config_path || !fs.existsSync(ctx.tool.config_path))
                failConfigMissing('jscpd-config-missing', 'JSCPD requires its exact configured path.');
            const testConfigPath = path.join(path.dirname(ctx.tool.config_path), 'jscpd-tests.json');
            if (!fs.existsSync(testConfigPath))
                failConfigMissing('jscpd-test-config-missing', 'JSCPD requires its dedicated test calibration.');
