...
function parseJsonLines(ctx, toolId, result, target) {
    ...
    for (const line of result.stdout.split('\n').filter(Boolean)) {
        ...
        if (!parsed.ok)
            failParse(ctx, toolId, parsed, result, target);
...
function advanceJsonStream(state, character) {
    if (state.inString) {
        if (state.escaped)
            state.escaped = false;
        else if (character === '\\')
...
function beginJsonStreamValue(ctx, toolId, result, target, state, character, index) {
    if (/\s/.test(character))
        return false;
    if (character !== '{' && character !== '[')
        failParse(ctx, toolId, { error: 'unexpected text outside JSON value' }, result, target);
    state.start = index;
    state.depth = 1;
    return true;
}
function parseJsonStream(ctx, toolId, result, target) {
    ...
    if (state.start >= 0 || values.length === 0)
        failParse(ctx, toolId, { error: state.start >= 0 ? 'incomplete JSON stream' : 'empty JSON stream' }, result, target);
    return values;
}
function collectForbiddenImports(findings, packageInfo, settings, moduleRoot) {
...
function gofmtTool() {
    return {
        ...
        run: (ctx) => {
            ...
            if (files.length === 0)
                return notApplicable('No Go files match the requested scope.');
            const result = requireToolExecution(safeExec('gofmt', ['-l', ...files], { cwd: ctx.repoRoot, timeout: ctx.tool.timeout_ms }), 'gofmt');
            if (result.exitCode !== 0)
                failParse(ctx, 'gofmt', { error: 'format check failed' }, result, ctx.repoRoot);
...
function goVetTool() {
    return {
        ...
        run: (ctx) => {
            ...
            if (modules.length === 0)
                return notApplicable('No Go module is affected by the requested scope.');
            for (const moduleRoot of modules) {
                ...
                if (result.exitCode !== 0 && findings.length === findingsBefore)
                    failParse(ctx, 'go-vet', { error: 'non-zero exit without diagnostics' }, result, moduleRoot);
            }
            return findingsResult(findings);
        },
    };
        ...
        run: (ctx) => {
            ...
            for (const moduleRoot of modules) {
                ...
                for (const line of result.stdout.split('\n').filter(Boolean)) {
                    ...
                    if (!match)
                        failParse(ctx, 'gocyclo', { error: 'unrecognized complexity finding' }, result, moduleRoot);
                    findings.push({
        ...
        run: (ctx) => {
            ...
            for (const moduleRoot of modules) {
                ...
                for (const packageInfo of packages)
                    collectForbiddenImports(findings, packageInfo, settings, moduleRoot);
            }
            return findingsResult(findings);
        },
    };
    ...
    return {
        ...
        run: (ctx) => {
            ...
            for (const moduleRoot of modules) {
                ...
                findings.push(...messages.filter((item) => item.finding?.osv).map((item) => govulnFinding(item, moduleRoot)));
            }
            return findingsResult(findings);
        },
    };
}
function govulnFinding(item, moduleRoot) {
...
export function registerGoTools(registerTool) {
