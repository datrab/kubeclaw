    ...
    return {
        ...
        run: (ctx) => {
            ...
            for (const dockerfile of dockerfiles) {
                ...
                if (result.exitCode !== 0 && allFindings.length === findingsBefore)
                    return failParse(ctx, 'hadolint', { error: 'non-zero exit without diagnostics' }, result, dockerfile);
            }
            return {
    ...
    return {
        ...
        run: (ctx) => {
            ...
            for (const chartDir of configuredTargetPaths(ctx)) {
                ...
                const parsed = tryParseJson(result.stdout);
                if (!parsed.ok)
                    return failParse(ctx, 'kubeconform', parsed, result, chartDir);
