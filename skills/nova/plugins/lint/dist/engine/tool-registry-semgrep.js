...
function parseSemgrepResult(ctx, result, target) {
    ...
    if (executionErrors.length > 0) {
        const message = selectPresentValue(executionErrors[0]?.message, executionErrors[0]?.type, 'Semgrep reported an execution error');
        throw Object.assign(new Error(message), {
            code: 'semgrep-execution-failed',
        });
    }
    if (result.exitCode !== 0) {
        return failParse(ctx, 'semgrep', { error: 'non-zero execution status' }, result, target);
    }
...
registerTool({
    ...
    run: (ctx) => {
        ...
        const result = requireToolExecution(safeExec('semgrep', semgrepArgs(ctx, config), {
            ...
            env: {
                SEMGREP_LOG_FILE: path.join(os.tmpdir(), `kubeclaw-semgrep-${process.pid}.log`),
            },
        }), 'semgrep');
