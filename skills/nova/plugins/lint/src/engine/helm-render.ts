...
async function renderChart(ctx: any, chartDir: any) {
  const release = `lint-${path.basename(chartDir).toLowerCase().replace(/[^a-z0-9-]/g, '-')}`;
  const rendered = requireToolExecution(await safeExec('helm', ['template', release, chartDir, '--include-crds'], { signal: ctx.signal, cwd: ctx.repoRoot, timeout: ctx.tool.timeout_ms }), 'helm-template');
