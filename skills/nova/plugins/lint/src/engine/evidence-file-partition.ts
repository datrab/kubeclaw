...
async function trackedFiles(repositoryRoot: string, signal?: AbortSignal): Promise<ReadonlySet<string>> {
  const result = requireToolExecution(await safeExec(
    'git', ['ls-files', '--cached', '-z'], { cwd: repositoryRoot, signal },
...
export async function partitionEvidenceFiles(
  repositoryRoot: string,
  files: readonly string[],
  signal?: AbortSignal,
): Promise<EvidenceFilePartition> {
  const tracked = await trackedFiles(repositoryRoot, signal);
