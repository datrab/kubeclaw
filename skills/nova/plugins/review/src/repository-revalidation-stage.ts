  ...
  for (let attempt = 0; attempt <= values.maxRetries; attempt += 1) {
    ...
    try {
      ...
      const response = record(await values.context.invoke('runtime.dispatch', { operation: 'dispatch',
        ...
        payload: withRuntimeDispatchProfile(Object.freeze({ ...requestPayload, runtimeDispatchAttempt: attempt }), values.context.contract.runtimeDispatchProfile) }), 'runtime dispatch response');
      return integrity(() => {
