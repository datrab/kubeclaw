...
function closedFields(value, allowed, label) {
...
function issueArray(value, label) {
    if (!Array.isArray(value))
        throw new Error(`${label} must be an array`);
...
function parse(value) {
    ...
    const parsed = {
        ...
        unverified_requirements: stringArray(output.unverified_requirements, 'unverified_requirements'),
    ...
    else if (parsed.critical_issues.length === 0
