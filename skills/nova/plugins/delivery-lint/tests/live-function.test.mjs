...
async function run(id, input) {
  ...
  const adapters = new core.AdapterRuntime({
    ...
    effects: new core.EffectCoordinator(new core.FileEffectJournal(path.join(temporary, `${id}-effects.jsonl`))),
  ...
  try {
    ...
    return { result, outcome: completed?.entry?.payload?.outcome };
  } finally {
...
console.log(JSON.stringify({ ok: true, plugin: 'kubeclaw.delivery-lint', suite: 'live-function' }));
