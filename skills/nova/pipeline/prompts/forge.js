...
import { buildForgeReadyForTestingLifecycleCommand, makePromptResult } from './shared.js';
...
export async function buildForgePrompt(config, moduleId, mod, dir, status, maxFails, novaPrompt) {
  ...
  const completionBlock = [
    ...
    '',
    `**Step 1:** Update \`${statusJsonPath}\` with the canonical READY_FOR_TESTING lifecycle shape:`,
    'Clear any previous terminal `completion_summary` and `completed_at` values when you do this.',
    '```bash',
    `cd ${config.repo_root}`,
    ...buildForgeReadyForTestingLifecycleCommand(config, dir),
    '```',
    '',
