...
import { buildPluginRegistry } from './registry.js';
...
function discoverSwarmConfigPath() {
  for (const candidate of discoverPlatformSwarmConfigCandidates()) {
    if (fs.existsSync(candidate)) return candidate;
  }
  return discoverPlatformSwarmConfigCandidates()[0];
}
...
export function loadConfig(projectName, opts = {}) {
  ...
  const swarmConfigPath = discoverSwarmConfigPath();
  if (!fs.existsSync(swarmConfigPath)) {
    throw new Error(
      `Swarm config not found: ${swarmConfigPath}\n` +
      `  Set SWARM_CONFIG env or place swarm.config.json in ~/.openclaw/ (auto-detected)`
    );
  }
  ...
  if (config._pluginRegistry) {
    log('INFO', `[plugins] startup registry ready: ${config._pluginRegistry.summary.enabledModules}/${config._pluginRegistry.summary.discoveredModules} modules enabled, ${config._pluginRegistry.summary.stageOwnerCount} stage owner(s)`);
  }

  // Make headHash() usable without passing config explicitly from lifecycle helpers.
...
export function validateConfig(config, progress) {
  ...
  const KNOWN_TOP_LEVEL_FIELDS = new Set([
    ...
    '_runStats', '_testOverrides', '_pluginRegistry',
  ]);
  ...
  config._pluginRegistry = registryBuild.registry;
}
