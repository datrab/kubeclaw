...
export function accumulateToolSummary(summary: any, result: any) {
  if (result.status === 'not_applicable') summary.tools_not_applicable++;
  else if (result.status !== 'ok') summary.tools_failed++;
  else {
    summary.tools_ok++;
