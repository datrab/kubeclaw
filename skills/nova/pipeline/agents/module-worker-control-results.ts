...
export function buildModuleBusterWorkerControlResult(config: AnyRecord, workerInput: AnyRecord = {}, input: AnyRecord = {}, opts: AnyRecord = {}) {
  ...
  const metadata = {
    ...
    model: workerInput?.worker?.backendConfig?.model || null,
    model_source: workerInput?.worker?.backendConfig?.modelSource || null,
    reasoning_level: workerInput?.worker?.backendConfig?.reasoningLevel || null,
    thinking_source: workerInput?.worker?.backendConfig?.thinkingSource || null,
    runtime: workerInput?.worker?.backendConfig?.runtimeKind || null,
    stream_log_path: input?.streamLogPath || null,
    module_attempt_ref: refs.moduleAttemptRef || null,
