...
import { updateGatewayObservability, updateTranscriptObservability } from './telemetry.js';
...
function normalizeIdentity(identity = {}, sessionLabelOrKey = null) {
  return {
    ...
    gateway_label: identity.gateway_label || identity.gatewayLabel || identity.label || null,
    session_key: identity.session_key || identity.sessionKey || sessionLabelOrKey || null,
...
export async function observeAcpMonitorSurfaces(config, sessionLabelOrKey, identity = {}, opts = {}) {
  ...
  const normalizedIdentity = normalizeIdentity(identity, sessionLabelOrKey);
