...
function getRunRedisLogDir(config) {
  return config?._runLogDir ? path.join(config._runLogDir, 'redis') : null;
}

export function getRedisLogTargets(config, fileName = 'redis-exchanges.jsonl') {
  if (!config?._runLogDir && !config?._logDir) return [];
  const targets = [];
  if (config?._logDir) targets.push(path.join(redisLogDir(config), fileName));
  if (config?._runLogDir) targets.push(path.join(getRunRedisLogDir(config), fileName));
  return targets;
}
...
export function logRedisExchange(config, direction, type, scope, scopeId, payload) {
  try {
    ...
    appendRedisArtifactRecord(config, entry, 'redis-exchanges.jsonl');
  } catch {
    // Fully non-blocking — never propagate
  }
}
