...
import { moduleBusterOutputPath } from '../core/paths.ts';
...
import { createRedisCompletionEventAdapter, createLocalEvidenceEventAdapter } from './completion-event-adapters.ts';
import { waitForBusterCompletion } from './buster-completion-controller.ts';
...
function buildModuleRateLimitStatusOptions(config, moduleId, expectedIdentity = {}) {
  return {
    moduleId,
    phase: 'buster',
    identity: {
...
function appendDurableModuleCompletionAlert(config, moduleId, expectedIdentity = {}, reason, extra = {}) {
  appendDurableOperatorAlert(config, 'module.operator_alert', {
    module_id: moduleId,
    attempt: expectedIdentity.attempt ?? null,
    dispatch_id: expectedIdentity.dispatch_id || null,
    gateway_label: expectedIdentity.gateway_label || null,
...
function buildModuleControllerPollResult({
  ...
  const moduleRateLimitStatusOptions = buildModuleRateLimitStatusOptions(config, moduleId, expectedIdentity);
  if (controllerResult?.reason === 'fatal_error') {
    appendDurableModuleCompletionAlert(config, moduleId, expectedIdentity, 'completion_event_adapter_failed', {
      status: STATUS.FAIL,
  ...
  const visibleDrift = (completion.drift || []).filter((entry) => entry.code !== 'completion_identity_weak');
  if (visibleDrift.length > 0) {
    log('WARN', `Completion evidence drift for ${moduleId}: ${visibleDrift.map((entry) => entry.code).join(', ')}`);
  }
  log('OK', `Redis completion: status=${redisEntry.status} mapped=${mappedStatus} outcome=${completion.outcome} source=${redisEntry.source || 'unknown'} run=${redisEntry.run_id || '—'} attempt=${redisEntry.attempt || '—'} dispatch=${redisEntry.dispatch_id || '—'}`);
  ...
  if (completion.rateLimited && !completion.targetReached) {
    return pollResult(false, 'rate_limited', buildModuleSessionRateLimitStatus(redisEntry, moduleRateLimitStatusOptions));
  }
  ...
  if (completion.blocked) return pollResult(false, 'blocked', {
    ...redisEntry,
...
export async function waitForModuleBusterCompletion(config, moduleDir, moduleId, expectedStatuses, timeoutMinutes, expectedIdentity = {}, pollResult, opts = {}) {
  ...
  const RedisCtor = selectDeps(opts.deps, 'completionEventAdapters')?.RedisCtor;
  ...
  const localAdapter = createLocalEvidenceEventAdapter(config, {
    eventBus,
    identity,
    paths: [moduleCompletionWatchPath(config, moduleDir)],
    emitExisting: true,
  });
  ...
  const redisDone = redisAdapter.start();
  redisDone?.catch?.(() => {});
  localAdapter.start();
  ...
  try {
    ...
    if (error?.code === 'PIPELINE_EVENT_WAIT_TIMEOUT' || isBudgetExhaustedError(error)) {
      appendDurableModuleCompletionAlert(config, moduleId, expectedIdentity, 'timeout', {
        status: STATUS.FAIL,
    ...
    controller.abort('module_completion_finished');
    localAdapter.stop('module_completion_finished');
    redisAdapter?.stop('module_completion_finished');
