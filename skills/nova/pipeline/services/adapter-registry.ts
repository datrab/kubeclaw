...
function normalizePathAlias(value: any) {
  if (selectTruthyValue(() => (typeof value !== 'string'), () => (!value.trim()))) return null;
  if (!value.includes('/') && !value.startsWith('.')) return null;
  return path.resolve(value);
}

function normalizeKey(value: any, fallback: any) {
...
export class UnknownAdapterError extends Error {
  constructor(label: any, rawKey: any, fallbackKey: any, registeredKeys: any = []) {
...
function resolveFromRegistry(registry: any, rawKey: any, fallbackKey: any, label: any) {
...
function projectSummaryGeneratorKey(config: any) {
...
function validateMethods(adapter: any, methods: any = [], label: any) {
...
export function resolveRegisteredRedisAdapter(config: any = {}, {
  agentType = null,
  source = 'redis',
  requiredMethods = [],
}: any = {}) {
  const agentConf = agentType
    ...
    : Object.values(selectDefinedValue(() => (config?.agents), () => ({}))).find((a: any) => typeof a === 'object' && a?.dispatch === 'redis');
...
export function resolveRegisteredProjectSummaryGenerator(config: any = {}) {
