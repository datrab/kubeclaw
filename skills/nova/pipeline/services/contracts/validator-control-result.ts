...
function normalizeStageId(stageId: string | null = null, producerType: string | null = null): string {
  return stageId || `validator:${producerType || 'unknown'}`;
}
...
function summarizeValidationFailures(failures: unknown[] = []): string | null {
  ...
  return failures.map((failure = {}) => {
    ...
    return item.code || item.explanation || item.stage || 'validation_failure';
  }).join(', ');
}
...
function buildValidatorSummary(result: UnknownRecord = {}, opts: UnknownRecord = {}): string {
  const producerType = opts.producerType || 'validator';
...
function mapValidationFailureToFinding(failure: UnknownRecord = {}): UnknownRecord {
  return {
    code: failure.code || 'VALIDATION_FAILURE',
    severity: 'error',
    message: failure.explanation || failure.message || 'Validation failure',
    category: failure.stage || null,
    target: failure.target || null,
    retryable: false,
    environmentIssue: false,
    metadata: {
      next_step: failure.next_step || null,
  ...
  for (const [toolId, toolResult] of Object.entries(report.tools)) {
    ...
    if (toolResult.status === 'error') {
      findings.push({
        ...
        severity: 'error',
        message: toolResult.error || `${toolId} failed`,
        category: toolId,
        target: null,
        retryable: true,
    ...
    for (const finding of Array.isArray(toolResult.findings) ? toolResult.findings : []) {
      ...
      findings.push({
        code: item.code || `${toolId}.finding`,
        severity: item.severity === 'warning' ? 'warn' : 'error',
        message: item.message || `${toolId} finding`,
        category: toolId,
        target: item.file || null,
        retryable: false,
        environmentIssue: false,
        metadata: {
          tool_id: toolId,
...
export function buildModuleValidatorControlResult(config: UnknownRecord, result: UnknownRecord = {}, opts: UnknownRecord = {}): UnknownRecord {
  if (typeof opts.producerType !== 'string' || !opts.producerType.trim()) {
  ...
  if (typeof opts.outcomeClass !== 'string' || !opts.outcomeClass.trim()) {
  ...
  const runId = result?.run_id || opts.runId || getRunId(config as never) || config?._runId || config?.run_id || null;
  const moduleId = result?.module_id || opts.moduleId || opts?.input?.ids?.moduleId || null;
  const moduleDir = result?.module_dir || opts.moduleDir || opts?.input?.workspace?.moduleDir || null;
  ...
  const metadata: UnknownRecord = {
    ...
    scope: opts.scope || (moduleId ? 'module' : 'pipeline'),
    outcome_class: opts.outcomeClass,
    failures: cloneSerializable(failures),
    report_summary: cloneSerializable(result?.report?.summary || null),
    report_artifact: result?.report_artifact || opts.reportArtifact || null,
    report: opts.includeReport === true ? cloneSerializable(result?.report || null) : undefined,
  };
  ...
  return buildTypedValidatorControlResult({
    ...
    stageId,
    validatorRef: opts.validatorRef || null,
...
export function coerceTypedValidatorControlResult(result: unknown, { producerType }: { producerType?: string | null | undefined } = {}): UnknownRecord {
  if (isTypedValidatorControlResult(result, producerType ?? null)) return result;
  throw new Error(`validator:${producerType || 'unknown'} plugin output must be a typed validator control result; compatibility-shaped validation results are not accepted at the validator boundary`);
}

export function validateTypedValidatorControlResult(result: unknown, {
  producerType = null,
  stageId = `validator:${producerType || 'unknown'}`,
  ...
  } else if (typeof result.diagnostics.summary !== 'string' || !result.diagnostics.summary.trim()) {
...
export function normalizeTypedValidatorControlResult(rawResult: unknown, {
  ...
  try {
    ...
    throw createContractInvalidError(`${label || stageId} validator returned invalid control result: ${validationErrors.join('; ')}`, {
      label: label || stageId,
      stageId,
  ...
  if (errors.length > 0) {
    throw createContractInvalidError(`${label || stageId} validator returned invalid control result: ${errors.join('; ')}`, {
      label: label || stageId,
      stageId,
      hookFamily: 'validator.run',
      moduleId,
