// services/correlation.js — Canonical correlation fallback helpers
...
const STATUS_GATEWAY_LABEL_RESOLVERS = [
  ...
  { family: 'status.active_agent', path: 'status.active_agent.label', via: 'label', get: (status) => status?.active_agent?.label },
  { family: 'status', path: 'status.dispatch_id', via: 'dispatch_id', get: (status) => status?.dispatch_id },
  { family: 'status.active_agent', path: 'status.active_agent.dispatch_id', via: 'dispatch_id', get: (status) => status?.active_agent?.dispatch_id },
];
...
const RESULT_GATEWAY_LABEL_RESOLVERS = [
  ...
  { family: 'status.active_agent', path: 'result.status.active_agent.label', via: 'label', get: (result) => result?.status?.active_agent?.label },
  { family: 'result', path: 'result.dispatchId', via: 'dispatchId', get: (result) => result?.dispatchId },
  { family: 'result', path: 'result.dispatch_id', via: 'dispatch_id', get: (result) => result?.dispatch_id },
  { family: 'rate_limit_status', path: 'result.rate_limit_status.dispatch_id', via: 'dispatch_id', get: (result) => result?.rate_limit_status?.dispatch_id },
  { family: 'status', path: 'result.status.dispatch_id', via: 'dispatch_id', get: (result) => result?.status?.dispatch_id },
  { family: 'status.active_agent', path: 'result.status.active_agent.dispatch_id', via: 'dispatch_id', get: (result) => result?.status?.active_agent?.dispatch_id },
  ...
  { family: 'module_status.active_agent', path: 'result.module_status.active_agent.label', via: 'label', get: (result) => result?.module_status?.active_agent?.label },
  { family: 'module_status', path: 'result.module_status.dispatch_id', via: 'dispatch_id', get: (result) => result?.module_status?.dispatch_id },
  { family: 'module_status.active_agent', path: 'result.module_status.active_agent.dispatch_id', via: 'dispatch_id', get: (result) => result?.module_status?.active_agent?.dispatch_id },
];
...
export function resolveStatusCorrelation(status, fallback = {}) {
  return finalizeCorrelationBundle({
    ...
    gateway_label: resolveFieldWithProvenance(status, STATUS_GATEWAY_LABEL_RESOLVERS, resolveFallbackField(fallback, ['gateway_label', 'gatewayLabel', 'label']), 'gateway_label'),
...
export function resolveResultCorrelation(result, fallback = {}) {
  return finalizeCorrelationBundle({
    ...
    gateway_label: resolveFieldWithProvenance(result, RESULT_GATEWAY_LABEL_RESOLVERS, resolveFallbackField(fallback, ['gateway_label', 'gatewayLabel', 'label']), 'gateway_label'),
...
export function resolveStatusSessionKey(status, fallback = null) {
  return resolveStatusCorrelation(status, { session_key: fallback }).session_key;
}

export function resolveStatusDispatchId(status, fallback = null) {
  return resolveStatusCorrelation(status, { dispatch_id: fallback }).dispatch_id;
}

export function resolveStatusGatewayLabel(status, fallback = null) {
  return resolveStatusCorrelation(status, { gateway_label: fallback }).gateway_label;
}

export function resolveResultSessionKey(result, fallback = null) {
  return resolveResultCorrelation(result, { session_key: fallback }).session_key;
}

export function resolveResultDispatchId(result, fallback = null) {
  return resolveResultCorrelation(result, { dispatch_id: fallback }).dispatch_id;
}

export function resolveResultAttempt(result, fallback = null) {
  return resolveResultCorrelation(result, { attempt: fallback }).attempt;
}

export function resolveResultGatewayLabel(result, fallback = null) {
  return resolveResultCorrelation(result, { gateway_label: fallback }).gateway_label;
}

export function resolveResultGateType(result, fallback = null) {
  return resolveResultCorrelation(result, { gate_type: fallback }).gate_type;
}
