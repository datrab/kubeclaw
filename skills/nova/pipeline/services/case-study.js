...
import { emitEvent, onSummaryStarted, onSummaryCompleted } from './telemetry.js';
...
function getCaseStudyDeps(config) {
  return { ...DEFAULT_CASE_STUDY_DEPS, ...(config?._testOverrides?.caseStudy || {}) };
}
...
export async function generateCaseStudy(config, progress) {
  const deps = getCaseStudyDeps(config);
  ...
  let sessionKey = null;
  let model = cs.model || config.models?.echo || 'anthropic/claude-sonnet-4-6';
  let label = null;
  let dispatch = null;
  let agentId = null;
  ...
  try {
    dispatch = caseStudyDispatchMode(model);
    ...
    caseStudyAttempt = resolveResultAttempt(sessionData, caseStudyAttempt);
    ...
    await deps.discord(config, 'INFO', '📝 Case Study Agent Spawned', 'Generating publishable case study from pipeline data.', buildCaseStudyDiscordFields({ run_id: runId, attempt: caseStudyAttempt, gateway_label: label, session_key: sessionKey }, [
    ...
    const caseStudyRateLimitRecovery = createTrackedSummarySessionRateLimitRecoveryOptions(config, {
      ...
      runIdFallback: runId,
      attemptFallback: caseStudyAttempt ?? 1,
      dispatchIdFallback: () => caseStudyDispatchId,
      gatewayLabelFallback: () => caseStudyGatewayLabel || caseStudyDispatchId || label,
    ...
    const trackedCaseStudyOutcome = resolveTrackedSessionRateLimitOutcome(pollRes, caseStudyRateLimitRecovery, {
      ...
      gatewayLabel: caseStudyGatewayLabel || caseStudyDispatchId || label,
    ...
    if (pollRes?.rate_limit_exhausted) {
      const caseStudyRateLimitExit = await finalizeSummarySessionRateLimitExit(pollRes, {
        ...
        attemptFallback: caseStudyAttempt,
        runIdFallback: runId,
        dispatchIdFallback: caseStudyDispatchId,
        gatewayLabelFallback: caseStudyGatewayLabel || caseStudyDispatchId || label,
        sessionKeyFallback: sessionKey,
    ...
    if (!pollRes.ok) {
      caseStudyAttempt = resolveResultAttempt(pollRes, caseStudyAttempt);
      ...
      await deps.discord(config, 'WARN', '📝 Case Study: No Output', `Case study agent finished without producing a report. Reason: ${failureReason}`, [
        ...buildCaseStudyDiscordFields({
          run_id: runId,
          attempt: caseStudyAttempt,
          dispatch_id: resolveStatusDispatchId(pollRes?.status, caseStudyDispatchId),
          gateway_label: resolveStatusGatewayLabel(pollRes?.status, caseStudyGatewayLabel || caseStudyDispatchId || label),
    ...
    onSummaryCompleted({ config }, 'case_study', {
      ...
      dispatch_id: resolveStatusDispatchId(lastCaseStudyStatus, caseStudyDispatchId),
      session_key: resolveStatusSessionKey(lastCaseStudyStatus, sessionKey),
      gateway_label: resolveStatusGatewayLabel(lastCaseStudyStatus, caseStudyGatewayLabel || caseStudyDispatchId || label),
      model,
      runtime: dispatch,
    });
