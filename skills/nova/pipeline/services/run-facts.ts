...
import {
  buildEvaluationDimensions,
  buildFactSources,
  buildTerminalFacts,
} from "./run-facts-evaluation.ts";
...
function readJsonIfPresent(filePath: string | null) {
  try {
    ...
    /* INTENTIONAL_NONCRITICAL(optional_probe_failed): this optional probe converts unreadable or absent input to explicit absence. */
    return null;
  }
}
...
function latestSummary(config: any) {
  try {
    ...
    return (
      readJsonIfPresent(artifacts.pipeline_summary_path) ||
...
function moduleFacts(progress: any, readModels: any) {
  ...
  const ids = sortedUnique([
    ...Object.keys(progressModules),
    ...Object.keys(lifecycleModules),
  ]);
  const entries = ids.map((id: any) => {
    ...
    return {
      ...
      fail_count: Number.isFinite(Number(lifecycle.fail_count))
        ? Number(lifecycle.fail_count)
        : 0,
      current_attempt: Number.isFinite(Number(lifecycle.current_attempt))
        ? Number(lifecycle.current_attempt)
        : null,
  ...
  return {
    ...
    completed: entries.filter((entry: any) => entry.completed).length,
    failed_or_blocked: entries.filter((entry: any) =>
      ["FAIL", "BLOCKED", "FAILED"].includes(entry.status),
    ).length,
...
function gateFacts(progress: any, readModels: any) {
  ...
  const ids = sortedUnique([
    ...Object.keys(progressGates),
    ...Object.keys(lifecycleGates),
  ]);
  const entries = ids.map((id: any) => {
    ...
    return {
      ...
      status: textOrNull(lifecycle.status) || "PENDING",
      completed:
        lifecycle.completed === true || lifecycle.scheduler_consumed === true,
  ...
  return {
    ...
    completed: entries.filter((entry: any) => entry.completed).length,
    failed_or_blocked: entries.filter((entry: any) =>
      ["FAIL", "BLOCKED", "REJECTED", "INVALID_OUTPUT"].includes(entry.status),
    ).length,
    pending: entries.filter(
      (entry: any) =>
        !entry.completed &&
        !["FAIL", "BLOCKED", "REJECTED", "INVALID_OUTPUT"].includes(
          entry.status,
        ),
    ).length,
    status_counts: statusCounts(entries),
...
function agentFacts(stats: any = null, summary: any = null) {
  ...
  const agents = {
    forge: Number.isFinite(Number(stats?.total_forge_attempts))
      ? Number(stats.total_forge_attempts)
      : Number(summaryAgents.forge || 0),
    buster: Number.isFinite(Number(stats?.total_buster_attempts))
      ? Number(stats.total_buster_attempts)
      : Number(summaryAgents.buster || 0),
    echo: Number.isFinite(Number(stats?.total_echo_reviews))
      ? Number(stats.total_echo_reviews)
      : Number(summaryAgents.echo || 0),
...
export function buildRunFacts(
  ...
  return {
    ...
    source: buildFactSources(modules, gates, stats),
    terminal: buildTerminalFacts(readModels, stats, summary, terminalOverride),
    modules,
    gates,
    agents,
    evaluation_dimensions: buildEvaluationDimensions(
      config,
      progress,
      stats,
      summary,
