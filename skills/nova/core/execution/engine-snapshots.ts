...
import { CURRENT_RUNTIME_DISPATCH_PROFILE, CURRENT_REVIEW_CACHE_PROFILE, parseRuntimeDispatchProfile, parseReviewCacheProfile, portableJson, type RuntimeDispatchProfile, type ReviewCacheProfile, type AdministrativeReopenDecision, type PipelineDefinition, type ResumeSignal } from '@kubeclaw/plugin-sdk';
...
export type RunSnapshot = RunSnapshotBase & (
  { readonly schemaVersion: 'run-snapshot.v1' | 'run-snapshot.v2'; readonly runtimeDispatchProfile?: never; readonly reviewCacheProfile?: never }
  | { readonly schemaVersion: 'run-snapshot.v3'; readonly runtimeDispatchProfile: RuntimeDispatchProfile; readonly reviewCacheProfile?: never }
  | { readonly schemaVersion: 'run-snapshot.v4'; readonly runtimeDispatchProfile: RuntimeDispatchProfile; readonly reviewCacheProfile: ReviewCacheProfile }
);
function snapshotDigest(value: unknown, version: RunSnapshot['schemaVersion']): string {
  if (!['run-snapshot.v1', 'run-snapshot.v2', 'run-snapshot.v3', 'run-snapshot.v4'].includes(version)) throw new Error('RUN_SNAPSHOT_VERSION_INVALID');
...
export function assertRunSnapshot(snapshot: RunSnapshot): void {
  ...
  if (!['run-snapshot.v1', 'run-snapshot.v2', 'run-snapshot.v3', 'run-snapshot.v4'].includes(snapshot.schemaVersion)
  ...
  if (snapshot.schemaVersion === 'run-snapshot.v4') {
    parseRuntimeDispatchProfile(snapshot.runtimeDispatchProfile); parseReviewCacheProfile(snapshot.reviewCacheProfile);
    validateContractValue('runtimeRunSnapshotV4', snapshot);
  }
  else if (snapshot.schemaVersion === 'run-snapshot.v3') {
  ...
  if (snapshot.schemaVersion !== 'run-snapshot.v4' && Object.hasOwn(snapshot, 'reviewCacheProfile')) throw new Error('RUN_SNAPSHOT_LEGACY_CACHE_PROFILE_INVALID');
}
...
export function writeRunSnapshots(runRoot: string, graph: ExecutionGraphSnapshot, registry: Readonly<Record<string, unknown>>): void {
  ...
  const unsigned = { schemaVersion: 'run-snapshot.v4' as const, graph, registry, runtimeDispatchProfile: CURRENT_RUNTIME_DISPATCH_PROFILE, reviewCacheProfile: CURRENT_REVIEW_CACHE_PROFILE };
