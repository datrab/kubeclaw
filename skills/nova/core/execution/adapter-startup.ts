...
function dependencyInvocation(adapterId: string, capability: string, request: CapabilityInvocation, parent: AdapterInvocationOwner | undefined, deliveryId: string | undefined) {
  ...
  const idempotencyKey = portableDependencyKey(adapterId, capability, request, dependencyParent, deliveryId, owner);
  return { idempotencyKey, dependencyIdentity: {prefix: `adapter:${adapterId}:${capability}:`, suffix: owner, currentKey: idempotencyKey,
