...
```
buster/
  buster-pipeline.js   Main task processor and session monitor
  ...
  pipeline/
    ...
    services/
      discord.js           Discord artifact write and webhook delivery helpers
      ...
      runtime.js           Redis/discovery runtime helpers
...
| `gate_id` | string\|null | Gate identifier when `task_type` is `"gate_test"` |
...
| `telemetry_stream` | string\|null | Legacy compatibility field. The value does not rename the stream; Buster normalizes to the canonical run stream. |
...
After this call, the Buster Pipeline kills the subagent session.
