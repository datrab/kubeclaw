...
async function runWsTest(test, baseUrl, vars, timeoutMs) {
  ...
  return new Promise(async (resolve) => {
    ...
    // Try to get WebSocket: prefer 'ws' package, fall back to globalThis.WebSocket
    let WebSocketImpl;
    try {
    ...
    if (!WebSocketImpl) {
      resolve({
        passed: false,
        failures: ['WebSocket not available: neither "ws" package nor globalThis.WebSocket found'],
        status: null,
    ...
    try {
      ...
      resolve({
        passed: false,
        failures: [`WebSocket connection failed: ${err.message}`],
        status: null,
        elapsed: Date.now() - start,
      });
      return;
