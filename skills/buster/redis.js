...
import { loadRedisCtor, resolveDiscordWebhookUrl } from './pipeline/services/runtime.js';
...
function buildCompletionIdentity(opts = {}) {
  const runId = normalizeIdentityValue(opts.runId ?? opts.run_id ?? process.env.PIPELINE_RUN_ID ?? process.env.RUN_ID);
  const attempt = normalizeIdentityValue(opts.attempt ?? process.env.PIPELINE_ATTEMPT ?? process.env.ATTEMPT);
  const dispatchId = normalizeIdentityValue(opts.dispatchId ?? opts.dispatch_id ?? process.env.PIPELINE_DISPATCH_ID ?? process.env.DISPATCH_ID);
  const sessionKey = normalizeIdentityValue(opts.sessionKey ?? opts.session_key ?? process.env.PIPELINE_SESSION_KEY ?? process.env.SESSION_KEY);
  const completionKey = (runId && attempt)
    ? `${runId}:${attempt}:${dispatchId || sessionKey || 'unscoped'}`
    : null;
...
const lib = {
   ...
   * @param {string} [opts.runId] - Pipeline run identifier for completion correlation
   * @param {number|string} [opts.attempt] - Attempt number for completion correlation
   * @param {string} [opts.dispatchId] - Unique dispatch/session label for this Buster attempt
   * @param {string} [opts.sessionKey] - Optional spawned child session key when known
   */
  async complete(opts) {
    ...
    const completionIdentity = buildCompletionIdentity(opts);
    ...
    try {
      ...
      completionLog.completion_entry = {
        status: status.toUpperCase(),
        source: 'agent',
        reason: 'verify-task.js failed',
        ...completionIdentity,
      };
    ...
    completionLog.completion_entry = {
      ...
      source: 'agent',
      summary: completionData.summary?.slice(0, 200),
    ...
    try {
      ...
      if (action === 'send') {
        ...
        const res = await lib.complete({
          ...
          runId:     getArg('run-id') || undefined,
          attempt:   getArg('attempt') || undefined,
          dispatchId: getArg('dispatch-id') || undefined,
          sessionKey: getArg('session-key') || undefined,
        ...
        throw new Error(
          ...
          '    [--run-id <id>]         # pipeline run correlation\n' +
          '    [--attempt <n>]         # module/gate attempt correlation\n' +
          '    [--dispatch-id <id>]    # unique Buster dispatch correlation\n' +
          '    [--session-key <key>]   # optional child session key when known\n' +
