...
export function parseVerdict(value, input) {
    ...
    const source = value;
    if (Object.keys(source).some((key) => !['verdict', 'summary', 'findings', 'session'].includes(key)))
        throw new Error('test verdict shape is invalid');
    const verdict = source.verdict;
    if (!['PASS', 'FAIL'].includes(String(verdict)))
        throw new Error('test verdict shape is invalid');
    const summary = source.summary;
    if (typeof summary !== 'string' || !summary.trim() || summary.length > 8192)
    ...
    if (verdict === 'PASS' && (source.findings.length > 0 || input.suiteEvidence.some((suite) => !suite.passed)))
        throw new Error('PASS contradicts test evidence');
    if (verdict === 'FAIL' && source.findings.length === 0)
    ...
    if (typeof session.sessionId !== 'string' || !session.sessionId || typeof session.startedAt !== 'string' || typeof session.completedAt !== 'string' ||
        ...
        throw new Error('test session evidence is invalid');
    if (verdict === 'PASS' && session.termination !== 'completed')
        throw new Error('PASS requires completed test session');
    return { verdict: verdict, runId: input.runId, taskId: input.taskId, attempt: input.attempt, summary, findings: source.findings,
