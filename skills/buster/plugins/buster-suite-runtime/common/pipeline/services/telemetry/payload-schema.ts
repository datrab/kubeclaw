...
function locateContractRoot(start: string): string {
  let cursor = start;
  for (;;) {
    const candidates = [
      path.join(cursor, 'contracts', 'telemetry', 'v1'),
      path.join(cursor, 'skills', 'pipeline', 'contracts', 'telemetry', 'v1'),
    ];
    const found = candidates.find((candidate) => fs.existsSync(path.join(candidate, 'catalog.json')));
    if (found) return found;
    const parent = path.dirname(cursor);
    if (parent === cursor) throw new Error('TELEMETRY_CONTRACT_ROOT_NOT_FOUND');
    cursor = parent;
  }
}
const contractRoot = locateContractRoot(moduleDirectory);
