...
export async function runNamespaceControllerPreflight(lease: AnyRecord, env: K8sCommandEnv): Promise<string> {
  ...
  const resourceNames: string[] = String(resources.stdout).trim().toLowerCase().split(/\s+/).filter(Boolean);
  if (!resourceNames.some((name: string) => name === 'busternamespaceleases' || name === `busternamespaceleases.${BUSTER_LEASE_API_GROUP}`)) {
...
function decodeSecretDataValue(value: unknown): string {
  if (typeof value !== 'string' || !value.trim()) throw new Error('secret data value is missing');
  return Buffer.from(value, 'base64').toString('utf8');
}

export async function readTestCredentials(specs: TestCredentialSpec[], namespace: string, env: K8sCommandEnv): Promise<AnyRecord[]> {
...
export async function waitForNamespaceLeaseReady(leaseName: string, timeoutSeconds: number, log: SuiteLog, env: K8sCommandEnv): Promise<NamespaceLeaseStatus> {
...
export async function verifyCopiedSecrets(names: string[], targetNamespace: string, env: K8sCommandEnv): Promise<void> {
...
export async function waitForPreviewUrl(leaseName: string, timeoutSeconds: number, log: SuiteLog, env: K8sCommandEnv): Promise<NamespaceLeaseStatus> {
  const deadline = Date.now() + timeoutSeconds * 1000;
