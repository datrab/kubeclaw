...
function isDeliveryResult(value: unknown): value is VisualDiscordDeliveryResult {
  ...
  return ['sent', 'skipped_no_webhook', 'skipped_capability', 'failed_noncritical'].includes(status ?? '')
    && typeof (value as { sent?: unknown }).sent === 'boolean';
}

export function summarizeDiscordDelivery(results: VisualDiscordDeliveryResult[] = []): Record<string, unknown> {
  results.forEach((result) => { if (!isDeliveryResult(result)) throw new Error('visual-reg Discord delivery result must be a typed delivery result'); });
  ...
  if (results.length > 0) status = sent === results.length ? 'sent'
...
export function resolveVisualRegOverallStatus(results: Array<{ status: SuiteStatus }>, enforced = false): SuiteStatus {
  if (results.some((result) => result.status === STATUS.ERROR)) return STATUS.ERROR;
  if (results.some((result) => result.status === STATUS.FAIL)) return enforced ? STATUS.FAIL : STATUS.ERROR;
  return STATUS.PASS;
}

export async function deliverVisualResults(input: {
  moduleId: string; pageResults: PageResult[]; status: SuiteStatus; enforced: boolean; mode: string;
  webhookUrl: string; capabilitySkip: VisualDiscordDeliveryResult | null; deliveryContext: Record<string, unknown>; log: Log;
}): Promise<{ deliveries: VisualDiscordDeliveryResult[]; discordMode: string }> {
  const deliveries: VisualDiscordDeliveryResult[] = [];
  if (input.capabilitySkip) deliveries.push(input.capabilitySkip);
  else if (input.mode === 'summary') deliveries.push(await discordSummary(input.moduleId, input.pageResults, input.status, input.enforced, {
    webhookUrl: input.webhookUrl, discordDiffThreshold: 2, log: input.log, deliveryContext: input.deliveryContext,
  }));
  else for (const page of input.pageResults) if (page.actualPath) deliveries.push(await discordSingle(input.moduleId, page.name, page.actualPath,
    page.diffPath ?? null, page.diffPercent ?? 0, page.status, { webhookUrl: input.webhookUrl, log: input.log, deliveryContext: input.deliveryContext }));
  return { deliveries, discordMode: input.mode };
}

export async function finalizeVisualReg(input: {
  startTime: number; result: MultiPathResult; status: SuiteStatus; enforced: boolean; mode: string; thresholds: Record<string, unknown> | null; pathsTotal: number;
  baselineDir: string; discordMode: string; deliveries: VisualDiscordDeliveryResult[]; moduleId: string; telemetryContext: unknown; log: Log;
