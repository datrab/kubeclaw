...
async function main() {
    ...
    if (!jobFile || !repositoryRoot)
        throw new Error('BUSTER_WORKER_RUNNER_ARGS_INVALID');
    ...
    const testConfig = materializeRepositoryValues(job.testConfig, repositoryRoot);
    const task = materializeRepositoryValues(job.task, repositoryRoot);
