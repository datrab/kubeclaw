...
import { JOB_SCHEMA, STATUS_SCHEMA, isRecord, parseJob, parseResult, sha256, } from './protocol.js';
...
const resultRoot = path.resolve(process.env.BUSTER_V2_RESULT_ROOT ?? '/workspace/git-repo/.swarm/runtime-results');
...
const kubernetesCredentials = path.resolve(process.env.BUSTER_V2_KUBERNETES_CREDENTIALS ?? '/var/run/buster-worker/kubernetes');
if (!Number.isSafeInteger(port) || port < 1 || port > 65_535)
    throw new Error('BUSTER_WORKER_PORT_INVALID');
if (token.length < 32)
    throw new Error('BUSTER_WORKER_TOKEN_INVALID');
...
if (!Number.isSafeInteger(maxExtractedBytes) || maxExtractedBytes < maxArchiveBytes
    || maxExtractedBytes > 2_147_483_648)
    throw new Error('BUSTER_WORKER_EXTRACTED_LIMIT_INVALID');
if (!Number.isSafeInteger(runnerUid) || runnerUid < (testMode ? 0 : 1)
...
function runDirectory(jobId) {
    return path.join(runRoot, jobId.replace(/[^a-z0-9._-]/gu, '_'));
}
function removeRunDirectory(jobId) {
...
function runnerEnvironment(job, directory, jobGid) {
    const environment = {};
    for (const key of RUNNER_ENV_KEYS) {
        if ((key === 'BUILDKIT_HOST' && !job.capabilities.includes('image_build'))
...
function sharedDirectory(directory, gid) {
    fs.mkdirSync(directory, { recursive: true, mode: 0o770 });
    if (!testMode)
        fs.chownSync(directory, process.getuid?.() ?? 1000, gid);
    fs.chmodSync(directory, 0o770);
}
function sharedFile(file, content, gid) {
    fs.writeFileSync(file, content, { mode: 0o440 });
    if (!testMode)
        fs.chownSync(file, process.getuid?.() ?? 1000, gid);
}
function materializeKubeconfig(directory, gid) {
    ...
    if (!host)
        throw new Error('BUSTER_WORKER_KUBERNETES_ENDPOINT_UNAVAILABLE');
...
async function body(request) {
    ...
    for await (const chunk of request) {
        ...
        if (size > maxRequestBytes)
            throw new Error('BUSTER_WORKER_REQUEST_TOO_LARGE');
        chunks.push(bytes);
    }
    return JSON.parse(Buffer.concat(chunks).toString('utf8'));
}
function start(job, requestDigest) {
    if (readStatus(job.jobId)?.state !== 'accepted')
        return;
    ...
    if (!testMode)
        fs.chownSync(directory, process.getuid?.() ?? 1000, jobGid);
    fs.chmodSync(directory, 0o770);
...
function terminate(jobId, child) {
    signalProcessGroup(child, 'SIGTERM');
    if (terminationTimers.has(jobId))
        return;
...
function shutdown() {
    server.close();
    for (const [jobId, child] of running)
        terminate(jobId, child);
