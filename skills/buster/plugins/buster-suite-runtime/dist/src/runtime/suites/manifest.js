...
import { createSuiteLog, suiteErrorMessage as errorMessage, suiteObject as objectRecord, suiteObjectOrEmpty as objectRecordOrEmpty, } from './support.js';
...
const manifestSuiteState = { logSink: null };
...
function collectSecretRefs(data) {
    return data.env
...
function checkImagePullSecrets(data, privateRegistries, findings) {
    if (!Array.isArray(privateRegistries) || privateRegistries.length === 0)
        return;
...
function checkProbes(data, healthPath, findings) {
    ...
    if (!data.hasReadinessProbe)
        findings.push(createFinding(SEVERITY.MODERATE, 'App has health_path configured but deployment lacks readinessProbe', { rule: 'readiness-probe' }));
    if (!data.hasLivenessProbe)
        findings.push(createFinding(SEVERITY.MODERATE, 'App has health_path configured but deployment lacks livenessProbe', { rule: 'liveness-probe' }));
}
function checkResourceLimits(data, findings) {
    if (!data.hasLimits)
        findings.push(createFinding(SEVERITY.MINOR, 'No resource limits defined on containers (resources.limits)', { rule: 'resource-limits' }));
}
function hasBlockingFindingSince(findings, startIndex) {
...
function configFailure(startTime, message, rule) {
...
function loadDeployment(pathValue, startTime) {
    try {
...
function loadSecret(pathValue, findings) {
    if (!pathValue)
        return null;
...
function runManifestCheck(findings, label, check) {
...
function evaluateManifest(data, secret, config, healthPath, findings) {
    if (!checkParseable(data, findings))
        return { checks: 1, failed: 1 };
    log(`kind: ${data.kind} ✓`);
    let failed = 0;
...
function completedManifestVerdict(startTime, config, deployment, data, findings, evaluation) {
...
export default async function manifestSuite(context) {
    ...
    if (!deploymentYamlRel)
        return configFailure(startTime, 'manifest.deployment_yaml is required when the manifest suite is requested', 'manifest-deployment-required');
...
function deploymentYamlMissingPath(deploymentYamlPath, deploymentYamlRel) {
    if (deploymentYamlPath)
        return deploymentYamlPath;
    return deploymentYamlRel;
}
function manifestShouldFail(hasCritical, exceedsThreshold) {
    if (hasCritical)
        return true;
