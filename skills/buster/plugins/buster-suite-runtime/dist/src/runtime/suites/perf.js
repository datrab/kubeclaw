...
const perfSuiteState = { logSink: null };
const log = createSuiteLog('perf', 'PERF', (entry) => perfSuiteState.logSink?.(entry));
function safeArtifactSegment(value, fallback = 'missing_artifact_segment') {
...
function lighthouseCategoryName(category) {
    const configuredName = CATEGORY_NAMES[category];
    if (configuredName !== undefined)
        return configuredName;
    return category;
}
function scratchRunSegment(context) {
...
export function resolvePerfReportPaths(context = {}, perfConf = {}) {
...
function toolErrorVerdict(startTime, message, rule = 'lighthouse-error') {
    log(`ERROR: ${message}`);
...
function resolvePerfSettings(context) {
...
async function runLighthouse(context, settings) {
    ...
    if (!fs.existsSync(settings.scratchPath))
        throw new Error(`Lighthouse report not found at ${settings.scratchPath}`);
...
function evaluateEnforcedScores(scores, thresholds) {
    const findings = [];
    let checksFailed = 0;
...
function evaluateInformationalScores(scores) {
    const findings = [];
    for (const [category, score] of Object.entries(scores)) {
        const name = lighthouseCategoryName(category);
        if (score < 50)
            findings.push(createFinding(SEVERITY.MODERATE, `${name}: score ${score} (low)`, { rule: `lighthouse-${category}` }));
        else if (score < 70)
            findings.push(createFinding(SEVERITY.MINOR, `${name}: score ${score} (could improve)`, { rule: `lighthouse-${category}` }));
    }
    return { findings, checksTotal: Object.keys(scores).length, checksFailed: 0 };
}
function evaluateScores(scores, thresholds) {
    if (thresholds)
        return evaluateEnforcedScores(scores, thresholds);
    return evaluateInformationalScores(scores);
}
function completedPerfVerdict(startTime, settings, scores, evaluation) {
    const enforced = settings.thresholds !== null;
    const checksPassed = evaluation.checksTotal - evaluation.checksFailed;
    const status = enforced && evaluation.checksFailed > 0 ? STATUS.FAIL : STATUS.PASS;
...
export default async function perfSuite(context) {
