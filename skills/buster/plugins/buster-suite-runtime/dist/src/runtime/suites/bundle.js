...
function scanDir(dir, log) {
    const files = [];
    const probeErrors = [];
    function recordProbeError(message) {
...
function missingOutputFailure(startTime, wwwDir, log) {
...
async function probeBundleSize(wwwDir, context) {
    try {
        ...
        if (Number.isFinite(parsed))
            return { sizeKb: parsed, error: null };
...
function enforcedBundleFindings(totalSizeKb, fileCount, thresholds) {
    const findings = [];
...
function informationalBundleFindings(totalSizeKb) {
    if (totalSizeKb !== null && totalSizeKb > 5120)
        return [createFinding(SEVERITY.MODERATE, `Bundle size ${totalSizeKb} KB is large (>5 MB)`, { rule: 'size-info' })];
    if (totalSizeKb !== null && totalSizeKb > 3072)
        return [createFinding(SEVERITY.MINOR, `Bundle size ${totalSizeKb} KB (>3 MB)`, { rule: 'size-info' })];
    return [];
}
function bundlePolicyFindings(enforced, totalSizeKb, fileCount, thresholds) {
    if (enforced)
        return enforcedBundleFindings(totalSizeKb, fileCount, thresholds);
...
export default async function bundleSuite(context) {
    ...
    const config = context.config?.bundle === undefined ? {} : context.config.bundle;
    ...
    if (!fs.existsSync(wwwDir))
        return missingOutputFailure(startTime, wwwDir, log);
...
function bundleWwwDirAuthority(config) {
    if (config.www_dir !== undefined && config.www_dir !== null)
        return config.www_dir;
    return DEFAULTS.www_dir;
}
function bundleScanDegraded(sizeProbeError, scan) {
    if (sizeProbeError)
        return true;
