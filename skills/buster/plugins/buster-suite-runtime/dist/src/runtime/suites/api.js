...
const apiSuiteState = { logSink: null };
...
function resolveSpecPath(specFile, projectDir) {
    ...
    if (!resolved)
        throw new Error('api.spec_file is required');
    return resolved;
}
...
function apiContractFailure(startTime, message, rule) {
    log(message);
...
function apiProjectDirAuthority(serve) {
    ...
    if (requestedProjectDir) {
        ...
        if (!resolved)
            throw new Error('serve.project_dir is outside allowed repository scope');
        return resolved;
    }
    return DEFAULTS.project_dir;
}
function apiTimeoutMsAuthority(defaults, field) {
    const configured = defaults[field];
    if (configured !== undefined && configured !== null)
        return Number(configured);
    return field === 'ws_timeout_ms' ? DEFAULTS.ws_timeout_ms : DEFAULTS.timeout_ms;
}
export default async function apiSuite(context) {
    ...
    if (!specFile)
        return apiContractFailure(startTime, 'api.spec_file is required when the API suite is requested', 'api-spec-required');
    const specPath = resolveSpecPath(specFile, projectDir);
    if (!fs.existsSync(specPath))
        return apiContractFailure(startTime, `API spec file not found: ${specPath}`, 'api-spec-missing');
    let spec;
    try {
    ...
    if (tests.length === 0)
        return apiContractFailure(startTime, 'API spec must contain at least one test', 'api-spec-empty');
    ...
    if (!auth.ok)
        return apiContractFailure(startTime, auth.error === undefined ? 'API auth setup failed' : auth.error, 'api-auth-setup');
