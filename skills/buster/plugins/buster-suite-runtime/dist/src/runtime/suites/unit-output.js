...
function requiredCount(value, field) {
    if (value === undefined)
        throw new Error(`${field} count missing from test output`);
...
function parsePytest(output) {
    ...
    if (!match?.[1] || !/\b(passed|failed|error|skipped|warning)\b/.test(match[1]))
        return null;
...
function parseFixtureJson(output) {
    ...
    for (let index = lines.length - 1; index >= 0; index -= 1) {
        ...
        try {
            ...
            if (data?.ok !== true || !Array.isArray(data.checked))
                continue;
...
export function parseOutput(output, exitCode) {
...
function blockFailures(output, separator) {
    const failures = [];
...
function pytestFailures(output) {
    const failures = [];
    for (const line of output.match(/^FAILED\s+(.+)/gm) ?? []) {
        ...
        if (match?.[1])
            failures.push({ test: match[1], message: (match[2] ?? 'unit_test_failed_without_message').slice(0, 300) });
    }
    return failures;
}
