...
async function responseBody(response, expect) {
    ...
    try {
        ...
        try {
            ...
            return null; /* INTENTIONAL_NONCRITICAL(optional_probe_failed): unreadable optional bodies are validated as absent. */
        }
    }
}
function bodyFailures(body, expect) {
    const failures = [];
    if (expect.body_type) {
    ...
    for (const [key, expected] of Object.entries(expect.body_contains)) {
        ...
        else if (expected !== true && String(actual) !== String(expected))
            failures.push(`Expected body.${key} = ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`);
    }
    return failures;
}
function requestOptions(test, defaults, vars, signal) {
    ...
    if (test.body && ['POST', 'PUT', 'PATCH'].includes(method))
        options.body = JSON.stringify(interpolateObject(test.body, vars));
    return options;
}
export async function runHttpTest(test, baseUrl, defaults, vars, timeoutMs) {
    ...
    try {
        ...
        if (expect.status != null && response.status !== expect.status)
            failures.unshift(`Expected status ${expect.status}, got ${response.status}`);
