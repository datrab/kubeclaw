...
async function runStep(state, name, started, action) {
    const { plan } = state;
    if (plan.criticalFailed)
        return;
    plan.log(`--- ${name} ---`);
...
async function buildImage(state) {
    const { plan } = state;
    ...
    if (plan.sourceImage)
        await runStep(state, 'source-image-promote', `Promoting ${plan.sourceImage}`, async () => {
            ...
            state.sourceImageId = result.digest;
            state.registryImageDigest = result.digest;
            state.deploymentImage = result.immutableImage;
            return `Promoted: ${plan.sourceImage} -> ${result.immutableImage}`;
        });
    else
        await runStep(state, 'buildkit-build-push', `Building ${plan.imageName}`, async () => {
...
async function provisionNamespace(state) {
    const { plan } = state;
    await runStep(state, 'namespace-lease', plan.namespace, async () => {
        ...
        if (state.leaseStatus.namespaceName !== plan.namespace)
            throw new Error(`Lease returned unexpected namespace ${state.leaseStatus.namespaceName}`);
...
async function deployApplication(state) {
    const { plan } = state;
    await runStep(state, 'manifest-apply', `${plan.manifestPaths.length} manifest(s)`, async () => {
        if (!state.deploymentImage)
            throw new Error('BuildKit image digest missing before manifest apply');
    ...
    await runStep(state, 'pods-ready', `Waiting up to ${plan.readyTimeout}s`, async () => {
        try {
            await waitForPods(plan.namespace, plan.readyTimeout, plan.log, plan.commandEnv);
            return `All pods ready in ${plan.namespace}`;
        }
        catch (error) {
            throw new Error(`Pods not ready within ${plan.readyTimeout}s. Pod status:\n${await getPodStatus(plan.namespace, plan.commandEnv)}`, { cause: error });
        }
    });
}
async function checkHealth(state) {
    ...
    await runStep(state, 'health-check', serviceUrl, async () => {
        if (shouldUsePortForwardHealthCheck({ purpose: plan.purpose, previewExposureProvider: plan.previewProvider })) {
            state.httpCode = await withServicePortForward(plan.namespace, plan.serviceName, plan.port, plan.healthPath, plan.log, plan.commandEnv, async (url) => retryHttpHealthCheck(url, plan.log, plan.healthRetries, plan.healthBaseDelayMs));
        ...
        state.httpCode = result.statusCode;
        state.internalBodyBytes = result.body.length;
...
async function resolvePreview(state) {
    const { plan } = state;
    if (plan.criticalFailed || plan.purpose !== 'final-preview' || plan.previewProvider !== 'tailscale-ingress')
        return;
...
function buildVerdict(state, serviceUrl) {
    const { plan } = state;
    const failed = plan.checks.filter((check) => !check.passed);
    const status = failed.length === 0 ? STATUS.PASS : STATUS.FAIL;
    ...
    const findings = failed.map((check) => createFinding(SEVERITY.CRITICAL, check.detail, { rule: check.name }));
...
export async function executeK8sPlan(plan) {
    const state = { plan, leaseStatus: null, previewLeaseStatus: null, testCredentials: [], internalBodyBytes: null,
