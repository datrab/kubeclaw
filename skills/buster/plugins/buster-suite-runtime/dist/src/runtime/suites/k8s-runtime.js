...
import { errorMessage, errorOutput, execFileAsync, isSuccessfulHttpStatus, parseKubectlJson, renderManifestForK8sSuiteWithStats, trimOut, } from './k8s-base.js';
export async function applyManifests(manifestPaths, imageName, registryTag, targetNs, timeoutMs, log, env) {
    ...
    try {
        ...
        for (const manifestPath of manifestPaths) {
            if (!fs.existsSync(manifestPath))
                throw new Error(`Requested manifest not found: ${manifestPath}`);
        ...
        try {
            ...
            log(`non-blocking manifest temp cleanup failed: ${errorMessage(error)}`);
        }
    }
}
async function listPodNames(ns, env) {
...
export async function waitForPods(ns, timeoutSeconds, log, env) {
    ...
    while (Date.now() < deadline) {
        ...
        try {
            ...
            if (readinessAttempt > 1)
                log(`Pod readiness settled after ${readinessAttempt} attempt(s)`);
            return;
        }
        catch (error) {
            lastError = error;
...
export async function getPodStatus(ns, env) {
    try {
...
export function shouldUsePortForwardHealthCheck({ purpose, previewExposureProvider }) {
    return !(purpose === 'final-preview' && previewExposureProvider === 'tailscale-ingress');
}
function sleepMs(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}
export async function retryHttpHealthCheck(url, log, retries, baseDelayMs) {
    let lastError = null;
...
async function httpTextCheck(url, expectedText, log) {
...
export async function retryHttpTextCheck(url, expectedText, log, retries, baseDelayMs) {
    let lastError = null;
    const attempts = Math.max(1, Math.floor(retries));
    const delayMs = Math.max(0, Math.floor(baseDelayMs));
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
