...
export function validateDockerfileFromImages(dockerfilePath) {
    const findings = [];
    ...
    for (const [index, line] of fs.readFileSync(dockerfilePath, 'utf8').split(/\r?\n/).entries()) {
        ...
        if (image.startsWith('$')) {
            ...
            if (!result.ok)
                findings.push(`Dockerfile FROM "${image}" at line ${index + 1} must be fully qualified with registry/namespace (${result.reason})`);
        }
        if (match[2])
            stages.add(match[2].toLowerCase());
    }
    return findings;
}
function referencedSecretNames(deploymentPath) {
    const content = fs.readFileSync(deploymentPath, 'utf8');
    return [...content.matchAll(/secretKeyRef:\s*\n\s+name:\s*['"]?([^\s'"#]+)/g)]
        .map((match) => match[1])
        .filter((name) => name !== undefined);
}
function declaredSecretName(secretPath) {
    const content = fs.readFileSync(secretPath, 'utf8');
    const match = content.match(/kind:\s*Secret[\s\S]*?metadata:\s*\n\s+name:\s*['"]?([^\s'"#]+)/);
    return match?.[1] ?? null;
}
function validateSecretAuthority(serve) {
    if (!serve.deployment_yaml || !serve.secret_yaml)
        return null;
    ...
    if (!deploymentPath || !secretPath)
        return 'serve deployment or secret path is invalid';
...
function generatedDockerfile(serve, image, type) {
    if (type === 'server') {
        return [
            `FROM ${image}`,
            'WORKDIR /app',
...
function generatedManifest(imageName, serviceName, port) {
...
function failure(startedAt, message, rule) {
...
function requireScopedPath(value, options, rule, message) {
    const resolved = resolveRepoScopedPath(value, options);
    if (!resolved)
        throw new BuildPreparationError(message, rule);
    return resolved;
}
function prepareDockerfile(input, tempDir, image, type, port) {
    if (input.dockerfile) {
...
function prepareManifests(input, tempDir, imageName, serviceName, port) {
    let manifest;
...
function prepareBuild(context) {
    const input = context.config?.serve ?? {};
    ...
    if (secretFailure)
        throw new BuildPreparationError(secretFailure, 'serve-secret-ref');
    ...
    if (dockerfileFindings.length)
        throw new BuildPreparationError(dockerfileFindings.join('\n'), 'dockerfile-from-image');
...
async function deployBuild(context, build) {
...
function wrapBuildVerdict(startedAt, build, verdict, extra = {}) {
...
export default async function buildSuite(context) {
    ...
    try {
        ...
        if (verdict.status !== STATUS.PASS)
            return wrapBuildVerdict(startedAt, build, verdict);
        ...
        const portForward = await startServicePortForward(namespace, build.serviceName, build.port, build.input.health_path ?? '/', log, buildK8sCommandEnv(null));
