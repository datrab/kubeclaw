...
export function parseSecretNameFromRef(ref) {
...
function normalizeCredentialKeys(value) {
    return Array.isArray(value)
        ? uniqueStrings(value.filter((key) => typeof key === 'string' && Boolean(key.trim())))
        : [];
}
export function normalizeTestCredentialSpecs(k8sCfg = {}, previewCfg = {}) {
    const specs = [];
    const configured = Array.isArray(k8sCfg.test_credentials) ? k8sCfg.test_credentials : [];
    for (const entry of configured) {
        if (!isObjectDoc(entry))
            continue;
        const secretName = normalizeSecretName(entry.secret_name);
        const keys = normalizeCredentialKeys(entry.keys);
        if (secretName === null || keys.length === 0)
            continue;
        specs.push({
            secretName,
            keys,
...
export function buildPreviewCredentialCommand(secretName, keys, namespace) {
    if (!secretName)
        return null;
