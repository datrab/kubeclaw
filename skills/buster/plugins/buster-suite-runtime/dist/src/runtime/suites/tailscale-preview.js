...
function trimOut(value, max = 800) {
...
function normalizeSmokePaths(value) {
    if (!Array.isArray(value))
        return [];
    return [...new Set(value.filter((entry) => typeof entry === 'string' && entry.startsWith('/')))];
}
function normalizeSmokeExpectedText(value) {
    if (!isRecord(value))
        return {};
    return Object.fromEntries(Object.entries(value).filter(([pathValue, marker]) => (pathValue.startsWith('/') && typeof marker === 'string' && marker.length > 0)));
}
...
function resolveK8sTarget(context, cfg) {
    const k8s = context.suiteResults?.k8s;
    if (!k8s)
        throw new Error('tailscale-preview source_suite "k8s" requires the k8s suite to run first');
    if (k8s.status !== STATUS.PASS)
        throw new Error(`k8s suite did not pass before tailscale-preview: ${k8s.status}`);
...
class PreviewCheckRunner {
    ...
    async run(name, action) {
        if (this.checks.some((check) => !check.passed))
            return;
        try {
...
async function checkStaticSurfaces(target, timeoutMs) {
    const checks = [];
...
function previewUrlForPath(baseUrl, smokePath) {
...
async function fetchPreviewResponse(url, timeoutMs) {
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(`HTTP ${res.status} from preview URL`);
...
function previewVerdict(startTime, runner, target, metadata) {
...
export default async function tailscalePreviewSuite(context) {
    ...
    await runner.run('preview-target', async () => {
        ...
        if (!/^https?:$/.test(content.protocol))
            throw new Error(`Unsupported content URL protocol: ${content.protocol}`);
        if (target.provider && target.provider !== 'tailscale-ingress')
            throw new Error(`Expected tailscale-ingress provider, got ${target.provider}`);
        return `${target.sourceSuite}: preview=${target.previewUrl} content=${target.contentUrl}`;
    });
    const resolvedTarget = target;
    if (resolvedTarget !== null && resolvedTarget.sourceSuite === 'explicit')
        await runner.run('dns-resolve', async () => {
            if (!target)
    ...
    await runner.run('static-surface-checks', async () => {
        if (!target)
            throw new Error('preview target unavailable');
        if (target.smokePaths.length === 0)
            return 'No static surface smoke paths configured';
