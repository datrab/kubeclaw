...
async function smokeHttp(port, paths, expectedText, timeoutMs, log) {
    const findings = [];
...
function normalizeSmokePaths(value) {
    ...
    for (const entry of value)
        buildLocalhostSuiteUrl(0, entry, 'serve.smoke_paths[]');
    return value;
}
function normalizeSmokeExpectedText(value, smokePaths) {
    if (value == null)
        return {};
    const record = objectRecord(value);
    if (!record)
        throw new Error('serve.smoke_expected_text must be an object keyed by smoke path when provided');
    ...
    for (const [smokePath, expected] of Object.entries(record)) {
        ...
        if (!allowed.has(smokePath))
            throw new Error(`serve.smoke_expected_text key must also be listed in serve.smoke_paths: ${smokePath}`);
        const expectedText = nonEmptyString(expected);
        if (!expectedText)
            throw new Error(`serve.smoke_expected_text[${smokePath}] must be a non-empty string`);
        normalized[smokePath] = expectedText;
    }
    return normalized;
}
function invalidSettingsVerdict(startTime, reason, message, rule) {
...
function resolveHealthSettings(serve) {
...
async function attemptUntilHealthy(settings, log) {
    let lastResult = { ok: false, status: null, responseTime: 0, error: 'not attempted', body: null };
...
function failedHealthVerdict(startTime, settings, summary) {
...
function completedHealthVerdict(startTime, settings, summary, findings) {
...
export default async function healthSuite(context) {
    ...
    if (!summary.passed)
        return failedHealthVerdict(startTime, settings, summary);
