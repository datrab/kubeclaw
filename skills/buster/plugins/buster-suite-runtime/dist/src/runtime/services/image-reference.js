...
export function validateImageReference(imageRef) {
    ...
    if (!IMAGE_REF_RE.test(value))
        return { ok: false, reason: 'invalid_image_reference' };
    if (!value.includes('/') || !hasExplicitRegistryComponent(value))
        return { ok: false, reason: 'not_fully_qualified' };
