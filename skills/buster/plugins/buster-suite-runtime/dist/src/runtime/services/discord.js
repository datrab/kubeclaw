...
import { createTelemetryContext, emitEvent, closeTelemetry, } from './telemetry.js';
...
import { appendDiscordCorrelation, persistDiscordArtifact, persistDiscordAuditReceipt, persistDiscordDeliveryReceipt, resolveDiscordAuditTargets, resolveDiscordDeliveryReceiptTargets, } from './discord-artifacts.js';
...
function reportBusterDiscordIncident(context = {}, classification, error, message, options = {}) {
    reportClassifiedNonBlockingError({
        ...
        classification,
        incidentKey: buildNonBlockingIncidentKey('buster-discord', selectTruthyValue(() => (context?.project), () => ('missing_project')), selectTruthyValue(() => (context?.run_id), () => ('missing_run_id')), selectTruthyValue(() => (context?.module_id), () => ('scope_global')), classification, selectTruthyValue(() => (options.scope), () => ('scope_global'))),
        message,
        error,
...
function hasExplicitDiscordPayload(message) {
...
function normalizeMessage(message) {
    ...
    return sanitizeDiscordMessage(hasExplicitDiscordPayload(message)
        ? {
...
function contextActionabilityField(context, actionability, key) {
...
function discordHealthKey(correlation = {}, surface = 'webhook') {
...
function buildTelemetryContext(correlation) {
    if (correlation.telemetry_context)
        return { ctx: correlation.telemetry_context, owned: false };
    if (!correlation.project || !correlation.run_id)
...
function emitDiscordObservability(correlation, type, payload = {}) {
    const { ctx, owned } = buildTelemetryContext(correlation);
    if (!ctx)
        return;
    void (async () => {
        try {
            await emitEvent(ctx, type, payload);
        }
        finally {
            if (owned)
...
function formatWebhookDeliveryDetail(error) {
...
function emitDiscordHealthChange(correlation, type, payload) {
...
function markDiscordDegraded(correlation, surface, reason, detail) {
...
function emitDiscordRestoredIfNeeded(correlation, surface, reason, detail) {
    const key = discordHealthKey(correlation, surface);
...
function discordWebhookDeliveryMuted(context = {}) {
    ...
    if (context.disableDiscordWebhooks === true)
        return true;
...
export function sendDiscord(message, context = {}) {
    ...
    const payload = {
        ...
        embeds: (Array.isArray(normalized.embeds) ? normalized.embeds : []).map((embed) => appendDiscordCorrelation(embed, correlation)),
    };
...
export async function deliverDiscordWebhookRequest(request = {}, context = {}) {
    ...
    if (discordWebhookDeliveryMuted(context))
        return { ok: true, skipped: true, muted: true };
    if (!correlation.webhook_url)
        return { ok: true, skipped: true, reason: 'missing_webhook' };
    const { webhook_url: _webhook_url, ...transportOptions } = normalizedRequest;
    if (!Number.isFinite(transportOptions.timeoutMs))
        transportOptions.timeoutMs = loadBusterDiscordWebhookTimeoutMs();
    try {
