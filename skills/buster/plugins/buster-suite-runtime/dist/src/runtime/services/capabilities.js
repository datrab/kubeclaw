...
function contextCapabilitySource(context = {}) {
    if (context.capabilities !== undefined && context.capabilities !== null)
        return context.capabilities;
    if (context.payload?.capabilities !== undefined && context.payload.capabilities !== null)
        return context.payload.capabilities;
    return [];
}
...
function missingCapabilityReason(missing = [], required = []) {
    ...
    if (missingList)
        return `missing capability: ${missingList}`;
...
function parseCapabilitiesEnv(value) {
    return normalizeBusterCapabilities(value);
}
export function parseCapabilitiesFromEnv(env, key) {
...
function hasBusterCapability(capabilities, capability) {
    return normalizeBusterCapabilities(capabilities).includes(capability);
}
function missingBusterCapabilities(capabilities, required = []) {
...
export function requiredCapabilitiesForSuite(suiteName, _context = {}) {
    switch (suiteName) {
        case 'build':
...
function resolveAlertTarget(candidate, field, validationPath = candidate) {
    ...
    if (selectTruthyValue(() => (containsParentTraversal(validationRaw)), () => (containsParentTraversal(raw))))
...
function alertTargets(context = {}) {
    ...
    return [...new Set(targets.filter((target) => Boolean(target)))];
}
export function appendDurableOperatorAlert(context = {}, alert = {}) {
...
function buildCapabilityDeniedAlert({ suite = null, action, required = [], capabilities = [], missing = [], context = {} } = {}) {
    return {
...
export function assertBusterCapabilities(context = {}, { suite = null, action = 'tool_execution', required = [] } = {}) {
    ...
    throw new BusterCapabilityDeniedError(`Buster capability denied for ${selectPresentValue(suite, DEFAULT_ASSERTION_SUITE)}: ${action} requires ${missing.join(', ')}`, alert);
}
