...
export function publishSuiteArtifacts(telemetry, { moduleId, gateId, suiteName, result, attempt }) {
    const pipelineRunLogPath = telemetry?.pipelineRunLogPath;
    if (typeof pipelineRunLogPath !== 'string' || !pipelineRunLogPath)
        return [];
    ...
    for (const [name, file] of files)
        artifacts.push(publishArtifact(config, { logical_id: `quality/${workId}/${attempt}/${suiteName}/${name}`, kind: suiteName === 'visual-reg' ? 'visual-evidence' : 'suite-artifact', media_type: 'application/octet-stream', bytes: fs.readFileSync(file), producer: telemetry.emitter, content_class: 'artifact', correlation }));
    return artifacts;
}
