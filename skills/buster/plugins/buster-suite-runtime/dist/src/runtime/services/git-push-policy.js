...
export function normalizeScopedAddPaths(addPaths) {
    if (!Array.isArray(addPaths) || addPaths.length === 0)
        throw new Error('gitPushWithRetry commit mode requires non-empty opts.addPaths');
    const normalized = addPaths.map((entry) => String(entry === undefined || entry === null ? '' : entry).trim()).filter(Boolean);
...
export function normalizePushBranch(branch) {
    ...
    if (invalid)
        throw new Error(`gitPushWithRetry branch must be a valid branch name: ${branch}`);
    return branch;
}
