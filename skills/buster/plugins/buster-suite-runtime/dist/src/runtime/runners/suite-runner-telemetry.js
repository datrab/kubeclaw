...
function optionalNumber(value) { return typeof value === 'number' && Number.isFinite(value) ? value : null; }
function optionalBoolean(value) { return typeof value === 'boolean' ? value : null; }
async function publishArtifacts(tctx, moduleId, gateId, suiteName, result, attempt) {
    const artifacts = publishSuiteArtifacts(tctx, { moduleId, gateId, suiteName, result, attempt });
    for (const artifact of artifacts)
        await emitEvent(tctx, 'artifact.published', { artifact_id: artifact.artifact_id,
    ...
    return artifacts.map((artifact) => artifact.reference).filter((reference) => typeof reference === 'string');
}
function eventIdentity(telemetry, moduleId) {
...
function completionPayload(identity, suiteName, result, attempt, startMs) {
...
async function emitInfrastructureEvidence(tctx, identity, suiteName, result, attempt) {
    if (!['k8s', 'health', 'tailscale-preview'].includes(suiteName))
        return;
...
export async function emitSuiteCompleted(tctx, moduleId, suiteName, result, attempt, startMs) {
    const telemetry = tctx && typeof tctx === 'object' ? tctx : null;
