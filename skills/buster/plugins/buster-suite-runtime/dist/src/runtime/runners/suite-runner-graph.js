...
function identity(moduleId, project) {
    ...
    if (missing.length)
        throw createSuiteRunnerValidationError('Buster suite runner requires explicit module and project identity', { reason: 'missing_suite_identity', missing_fields: missing });
    return { moduleId: normalizedModule, project: normalizedProject };
}
function sorted(names, order) {
...
function contextForRun(opts, payload, resolved, config, suiteMap, cleanups, resultsDir) {
...
function prepare(authority, suites, opts) {
    const payload = opts.payload ?? {};
    const resolved = identity(opts.moduleId, payload.project);
    const repoRoot = opts.repoRoot?.trim();
    if (![Boolean(repoRoot), path.isAbsolute(repoRoot ?? '')].every(Boolean))
...
function dependencyFailure(state, suiteName) {
    const dependencies = state.authority.dependencies[suiteName];
    if (!dependencies)
        throw createSuiteRunnerValidationError(`Missing dependency declaration for Buster suite: ${suiteName}`, { reason: 'missing_suite_dependencies', suite: suiteName });
    for (const dependency of dependencies) {
        const result = state.suiteMap[dependency];
        if (result?.status === STATUS.FAIL && result.critical)
            return `${dependency} failed`;
        if (result?.status === STATUS.ERROR)
            return `${dependency} errored`;
    }
    return null;
}
async function skipBlocked(state, pending) {
    for (const suiteName of state.ordered.filter((name) => pending.has(name))) {
        ...
        state.suiteMap[suiteName] = verdict;
...
function readySuites(state, candidates) {
...
async function executeBatch(state, ready) {
    ...
    for (const entry of batch.results) {
        const result = entry.status === 'fulfilled' ? entry.result : createSuiteVerdict(entry.item_id, STATUS.ERROR, {
        ...
        if (entry.status !== 'fulfilled')
            await emitSuiteCompleted(state.telemetryContext, state.moduleId, entry.item_id, result, state.attempt, Date.now());
        if (new Set([STATUS.ERROR, STATUS.FAIL]).has(result.status))
            state.criticalFailed = true;
        if (result.suite === 'build' && result.status === STATUS.PASS && Number.isInteger(result.metadata?.port))
            state.context.config.serve = { ...(state.context.config.serve ?? {}), port: result.metadata?.port };
        state.suiteMap[entry.item_id] = result;
        state.results.push(result);
        state.context.suiteResults[entry.item_id] = result;
    }
}
async function executeGraph(state) {
    ...
    while (pending.size) {
        ...
        if (!ready.length)
            throw createSuiteRunnerValidationError('Buster suite dependency graph has no ready suites', { reason: 'suite_dependency_deadlock', invalid_suites: candidates });
...
export function buildDetailedSuiteSummary(results) { return results.map((result) => `${result.suite}: ${result.status}${detail(result) ? ` - ${detail(result)}` : ''}`).join(' | '); }
export function applyBuildRuntimePort(config, result) {
    if (result.suite === 'build' && result.status === STATUS.PASS && Number.isInteger(result.metadata?.port))
        config.serve = { ...(config.serve ?? {}), port: result.metadata?.port };
}
export function collectReadySuites(names, completed, dependencies, order) {
    const state = { authority: { dependencies, executionOrder: order }, ordered: sorted([...names], order), suiteMap: completed };
    return readySuites(state, state.ordered.filter((name) => !completed[name]));
}
export async function executeSuites(authority, suites, opts) {
    ...
    try {
        ...
        await cleanupSuiteResources(state.cleanups, { module: state.resolvedModuleId, project: state.project });
    }
}
