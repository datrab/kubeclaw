...
export const DEPENDENCIES = { manifest: [], build: ['manifest'], health: ['build'], k8s: [],
    'tailscale-preview': ['k8s'], a11y: ['health'], perf: ['health'], bundle: ['build'], security: ['build'],
    'visual-reg': ['health'], api: ['health'], e2e: ['health'], unit: [] };
export function validateSuiteNames(suites) {
    if (!Array.isArray(suites))
        throw createSuiteRunnerValidationError('Buster suites must be an array of suite names', { reason: 'invalid_suites_shape', invalid_suites: [suites] });
    const invalid = suites.filter((suite) => typeof suite !== 'string' || !suite.trim());
    const names = suites.filter((suite) => typeof suite === 'string' && Boolean(suite.trim())).map((suite) => suite.trim());
    if (!names.length)
        throw createSuiteRunnerValidationError('Buster suites must include at least one suite name', { reason: 'empty_suite_list', invalid_suites: [...suites] });
    ...
    if (invalid.length || unsupported.length)
        throw createSuiteRunnerValidationError(`Invalid Buster suite request: ${unsupported.concat(invalid.map(String)).join(', ')}`, {
            reason: unsupported.length ? 'unsupported_suite' : 'invalid_suite_name', unsupported_suites: unsupported, invalid_suites: invalid,
        });
    return names;
}
export function resolveSuiteTimeoutMs(config) {
...
export function collectReadySuites(suiteNames, completedResults = {}) {
