...
import { JOB_SCHEMA, STATUS_SCHEMA, exact, isRecord, parseResult, requiredCapabilitiesForSuites, relocateRepositoryValues, sha256, stringList, SUPPORTED_SUITES, } from './protocol.js';
const exec = promisify(execFile);
function canonicalDirectory(value) {
    const resolved = fs.realpathSync(path.resolve(value));
    if (!fs.statSync(resolved).isDirectory())
        throw new Error('BUSTER_SUITE_REPOSITORY_INVALID');
    return resolved;
}
...
function parseConfig(raw) {
    ...
    if ([...allowedSuites].some((suite) => !SUPPORTED_SUITES.includes(suite))) {
    ...
    if (!gitExecutable || !Number.isSafeInteger(maxArchiveBytes) || maxArchiveBytes < 1
...
function responseBody(response) {
    if (!isRecord(response.body))
        throw new Error('BUSTER_WORKER_RESPONSE_INVALID');
    return response.body;
}
async function archive(config, repositoryRoot) {
...
async function secret(context, name) {
...
export function activate(context) {
    ...
    return {
        ...
        async invoke({ request, signal, confidential, fence }) {
            ...
            if (request.capability !== 'test.suite.execute' || request.operation !== 'run'
            ...
            if (suites.some((suite) => !config.allowedSuites.has(suite)))
                throw new Error('BUSTER_SUITE_DENIED');
