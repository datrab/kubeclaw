...
function timeoutSignal(timeoutMs) {
    if (selectTruthyValue(() => (!timeoutMs), () => (timeoutMs <= 0)))
...
function requestAbortSignal(callerSignal, timeoutMs) {
    ...
    const signals = [callerSignal, timeout.signal].filter(Boolean);
    if (signals.length === 0)
...
function validatedWebhookOptions(options) {
...
async function sendWebhook(url, options) {
    const request = { method: 'POST', headers: options.headers, body: options.body };
    if (options.signal)
    ...
    if (!response?.ok) {
        ...
        throw new DiscordWebhookDeliveryError(`discord webhook HTTP ${status}${statusText ? ` ${statusText}` : ''}`, { status, statusText, bodyPreview });
    }
    return {
        ok: true,
...
export async function postDiscordWebhook(url, options = {}) {
...
function fetchAuthority(options) {
