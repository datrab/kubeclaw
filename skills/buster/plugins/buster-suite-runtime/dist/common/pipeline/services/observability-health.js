...
export function createObservabilityHealthState() {
    ...
    const observedAt = (value, field) => {
        if (typeof value === 'string' && value.trim())
            return value;
        return new Date().toISOString();
    };
    const restoredAfterAuthority = (value, degradedAt, restoredAt) => {
    ...
    return {
        markDegraded(key, data = {}) {
            const prev = health.get(key);
            if (prev?.degraded)
                return { shouldEmit: false, duplicate: true, restored: false, key, degradedAt: selectTruthyValue(() => (prev.degradedAt), () => (null)), previous: prev };
        ...
        markRestored(key, data = {}) {
            const prev = health.get(key);
            if (!prev?.degraded)
                return { shouldEmit: false, duplicate: false, restored: false, key, degradedAt: null, previous: selectTruthyValue(() => (prev), () => (null)) };
