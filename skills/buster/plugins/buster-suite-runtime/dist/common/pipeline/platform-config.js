...
function resolveProfileUrl(file) {
    ...
    for (;;) {
        ...
        if (parent === cursor)
            throw new Error(`config profile asset not found: ${file}`);
        cursor = parent;
    }
}
const PROFILE_URLS = {
...
function loadProfile(profileName, stack = []) {
    if (cachedProfiles.has(profileName))
        return cloneJson(cachedProfiles.get(profileName));
    ...
    try {
        ...
        if (typeof rawProfile?.extends === 'string') {
            ...
            if (typeof rawProfile._doc === 'string')
                profile._doc = rawProfile._doc;
        }
        else {
            profile = rawProfile;
        }
...
function assertExactObject(input, expected, label) {
...
function configOverrides(value, label) {
    if (value === undefined)
        return {};
...
function assertOverrideTargets(base, overrides, pathParts = [], labelPrefix = 'overrides') {
...
function mergeKnownOverrides(target, overrides) {
    for (const [key, value] of Object.entries(overrides)) {
        if (isPlainObject(value)) {
...
function substituteProfileTemplates(value, context, pathParts = []) {
...
export function isCompactSwarmConfig(config) {
    return isPlainObject(config) && hasCompactConfigAuthority(config);
}
function hasCompactConfigAuthority(config) {
...
export function expandSwarmConfig(rawConfig) {
    if (!isCompactSwarmConfig(rawConfig))
        return rawConfig;
    if (!isPlainObject(rawConfig))
        throw new Error('swarm.config.json: required object');
    ...
    for (const key of RUNTIME_DERIVED_FIELDS) {
        if (hasOwn(rawConfig, key))
            expanded[key] = rawConfig[key];
    }
    if (hasOwn(rawConfig, 'run_id'))
        expanded.run_id = rawConfig.run_id;
...
export function normalizeSwarmConfigInPlace(config) {
...
export function loadPlatformSwarmConfig(configPath = discoverSwarmConfigPath()) {
    ...
    if (!fs.existsSync(resolvedPath)) {
        throw new Error(`Swarm config missing: ${resolvedPath}\n` +
    ...
    try {
        ...
        throw new Error(`Swarm config invalid: ${resolvedPath}\n` +
            `  ${error.message}`);
    }
}
