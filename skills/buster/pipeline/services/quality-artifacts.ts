...
export function publishSuiteArtifacts(telemetry:any,{moduleId,gateId,suiteName,result,attempt}:any){
  ...
  const references=[];
  references.push(publishArtifact(config,{logical_id:`quality/${workId}/${attempt}/${suiteName}/verdict`,kind:'suite-verdict',media_type:'application/json',bytes:JSON.stringify(result),producer:telemetry.emitter,content_class:'artifact',correlation}).reference);
  const files=Object.entries(result.metadata||{}).filter(([,value])=>typeof value==='string'&&fs.existsSync(value)&&fs.statSync(value).isFile());
  for(const[name,file]of files)references.push(publishArtifact(config,{logical_id:`quality/${workId}/${attempt}/${suiteName}/${name}`,kind:suiteName==='visual-reg'?'visual-evidence':'suite-artifact',media_type:'application/octet-stream',bytes:fs.readFileSync(file as string),producer:telemetry.emitter,content_class:'artifact',correlation}).reference);
  return references;
}
