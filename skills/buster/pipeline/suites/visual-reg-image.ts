...
interface ImageComparison {
  diffCount: number; diffPercent: number; width: number; height: number;
  baselineWidth: number; baselineHeight: number;
}

function resizeBuffer(png: any, targetWidth: number, targetHeight: number): Uint8Array {
  ...
  for (let y = 0; y < png.height; y += 1) data.set(png.data.subarray(y * rowBytes, (y * rowBytes) + rowBytes), y * targetWidth * 4);
  return data;
}

export async function compareImages(baselinePath: string, actualPath: string, diffPath: string, threshold: number): Promise<ImageComparison> {
