...
test("original bridge forwards persisted preference identities to the OpenClaw process boundary", async()=>{
 ...
 try {
  ...
  const preferences={generationId:"generation-one",snapshotDigest:"sha256:exact-evidence",snapshot:{subjectId:"user-one",projectId:"project-two",personalEnabled:true,effective:{dense:{score:1}},events:[{eventId:"personal-from-project-one",projectId:"project-one"}]}};
  const response=await fetch(`${url}/v1/design-set`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({projectId:"project-two",request:{architecture:"current"},preferences})});
