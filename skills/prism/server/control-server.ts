...
class ControlService {
  private readonly config: ReturnType<typeof loadControlServerConfig>;
...
private authenticated(request: IncomingMessage): {
  ...
  const {sessionSecret} = this.config;
...
private async runWorker(
  ...
  async handle(request: IncomingMessage, response: ServerResponse) {
    const {pool, repository, artifacts} = this;
    const controlConfig = this.config;
    const {sessionSecret, ingressSecret, spiffeEnabled, dispatchSecret, workerSecret,
      trustedNovaSpiffeId, trustedTestRunnerSpiffeId, trustedWorkerSpiffeId,
      trustedControlSpiffeId, trustedPrismAgentSpiffeId, ingestionSecret,
      ingestionUrl, controlInternalUrl, studioPublicUrl} = this.config;
  ...
  try {
    ...
    if (url.pathname === "/v1/dispatch" && request.method === "POST") {
      ...
      const preferenceSubjectId = authorizePipelinePreferenceSubject(payload.preferenceSubjectId, controlConfig.pipelinePreferenceSubject);
      ...
      await inTransaction(pool, async requestClient => {
    ...
    if (url.pathname === "/v1/agent/design-sets" && request.method === "POST") {
      ...
      const generation = await requirePreferenceGeneration(pool,input.generationId,input.projectId,{operation:"design-set"});
      ...
      return json(response, result.status === 'created' ? 201 : 200, { ...result,
        studioUrl: `${studioPublicUrl}?project=${result.projectId}&document=${result.documentId}` });
    }
    ...
    if (url.pathname === "/v1/projects" && request.method === "POST") {
      ...
      const projectId = await inTransaction(pool, async client => {
    ...
    if (execute && request.method === "POST") {
      ...
      const values = await this.runWorker(operation, engineInput, idempotencyKey);
      ...
      for (const [view, value] of Object.entries(
        ...
        for (const state of Object.keys(value.states))
          for (const viewport of ["compact", "regular", "wide"] as const) {
            ...
            const rendered = await this.runWorker(
              "render",
              {
    ...
    if (url.pathname === "/v1/corpus" && request.method === "POST") {
      ...
      try{
        ...
        const embedded = await this.runWorker("ingest",{ text },`corpus:${sha256(JSON.stringify(input))}`);
    ...
    if (url.pathname === "/v1/corpus/search" && request.method === "GET") {
      ...
      const embedded = await this.runWorker(
        "ingest",
        { text: query },
        `query:${sha256(query)}`,
      );
    ...
    if (url.pathname === "/v1/preferences/policy" && request.method === "PUT") {
      const input = await body(request) as {projectId?: string; personalEnabled?: boolean};
      if (!input.projectId || typeof input.personalEnabled !== "boolean") throw new Error("projectId and personalEnabled are required");
      await setPersonalPreferences(pool, userKey(actor.user), input.projectId, input.personalEnabled);
      return json(response,200,{projectId:input.projectId,personalEnabled:input.personalEnabled});
    }
    ...
    if (url.pathname === "/v1/preferences" && request.method === "GET") {
      ...
      const events = await preferenceEvents(pool, userKey(actor.user), projectId);
      return json(response, 200, {
        events,
