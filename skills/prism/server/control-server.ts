...
/** Compose the original Control HTTP service; caller owns database and listener lifetime. */
export function createControlServer(pool: Database, environment: NodeJS.ProcessEnv = process.env) {
const repository = new RevisionRepository(pool);
const artifacts = new ContentAddressedArtifactStore(
  environment.ARTIFACT_ROOT ?? "/var/lib/prism/artifacts",
);
const workerUrl = new URL(
  environment.PRISM_WORKER_URL ?? "http://prism-worker:8080",
);
...
const trustedTestRunnerSpiffeId = environment.PRISM_TRUSTED_TEST_RUNNER_SPIFFE_ID ?? "";
...
const ingestionUrl=new URL(environment.PRISM_INGESTION_URL??"http://prism-ingestion:8080");
const controlInternalUrl=new URL(environment.PRISM_CONTROL_INTERNAL_URL??"http://prism-control:8080");
...
const controlConfig = loadControlConfig(environment);
  ...
  try {
    ...
    if (url.pathname === "/v1/agent/design-sets" && request.method === "POST") {
      ...
      return json(response, result.status === 'created' ? 201 : 200, { ...result,
        studioUrl: `${environment.PRISM_STUDIO_PUBLIC_URL ?? "https://prism-studio"}?project=${result.projectId}&document=${result.documentId}` });
    }
