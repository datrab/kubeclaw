...
import type { NativeWorkerLaunchAuthority } from './native-worker-ownership.ts';

export interface NativeWorkerProcessControl {
  readonly limits: NativeWorkerControlLimits;
  readonly run: (channel: NativeWorkerControlChannel, signal: AbortSignal, launch: NativeWorkerLaunchAuthority | undefined) => Promise<void>;
}
...
export function startNativeProcessControl(child: ChildProcessWithoutNullStreams, options: NativeWorkerProcessControl | undefined,
  stop: (code: string) => void, closeTimeoutMs: number, launch?: NativeWorkerLaunchAuthority): { finish: () => Promise<void> } {
  if (!options) return { finish: async () => {} };
  const channel = new NativeWorkerControlChannel(child.stdio[3], options.limits);
  const controller = new AbortController();
  const work = Promise.resolve().then(() => options.run(channel, controller.signal, launch)).catch(() => {
