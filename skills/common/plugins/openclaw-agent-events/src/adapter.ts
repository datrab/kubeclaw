...
const SUPPORTED = new Set<string>(SUPPORTED_HOOKS);
...
interface Subscription { unsubscribe?: () => void; dispose?: () => void; off?: () => void }
interface OpenClawSdk { on?: (hook: string, handler: (event: unknown) => void) => Subscription | (() => void) }
function record(value: unknown): Record<string, unknown> {
  return value && typeof value === 'object' && !Array.isArray(value) ? value as Record<string, unknown> : {};
}
function release(subscription: Subscription | (() => void)): void {
...
function projectEvent(event: Record<string, unknown>): Readonly<Record<string, unknown>> {
  const entries = Object.entries(event)
    ...
    .slice(0, 128);
  const metrics = Object.fromEntries(entries.filter(
    ([key, value]) => SAFE_METRIC_FIELD.test(key)
...
export function normalizeAgentEvent(raw: unknown): {
...
export function activateWithSdk(context: AdapterActivationContext, sdk: OpenClawSdk): AdapterInstance {
  const configured = context.config.hooks;
  if (!Array.isArray(configured) || configured.length === 0) throw new Error('AGENT_EVENT_HOOKS_INVALID');
  const hooks = configured.map((hook) => {
    if (typeof hook !== 'string' || !SUPPORTED.has(hook)) throw new Error(`AGENT_EVENT_HOOK_UNSUPPORTED:${String(hook)}`);
    return hook;
  });
  ...
  return {
    async ready() {
      if (typeof sdk.on !== 'function') throw new Error('OPENCLAW_HOOK_API_UNAVAILABLE');
      try {
...
export function activate(context: AdapterActivationContext): AdapterInstance {
  ...
  return activateWithSdk(context, require('openclaw/plugin-sdk') as OpenClawSdk);
}
