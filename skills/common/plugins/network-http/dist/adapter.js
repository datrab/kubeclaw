function positiveInteger(value, label, fallback) {
    ...
    if (!Number.isSafeInteger(resolved) || Number(resolved) < 1)
        throw new Error(`NETWORK_CONFIG_INVALID:${label}`);
...
export function activate(context) {
    ...
    const methods = new Set((context.config.allowedMethods ?? ['GET'])
    ...
    return {
        ...
        async invoke({ request, signal, confidential, fence }) {
            ...
            if (!allowed.has(url.origin))
                throw new Error(`NETWORK_ORIGIN_DENIED:${url.origin}`);
            if (url.username || url.password)
                throw new Error('NETWORK_CREDENTIALS_DENIED');
            const method = typeof request.payload.method === 'string' ? request.payload.method.toUpperCase() : 'GET';
            if (!methods.has(method))
                throw new Error(`NETWORK_METHOD_DENIED:${method}`);
            ...
            if (declared > maxResponseBytes)
                throw new Error('NETWORK_RESPONSE_SIZE_EXCEEDED');
            const bytes = new Uint8Array(await response.arrayBuffer());
            if (bytes.byteLength > maxResponseBytes)
                throw new Error('NETWORK_RESPONSE_SIZE_EXCEEDED');
            const text = new TextDecoder().decode(bytes);
            if (!response.ok)
                throw new Error(`HTTP_${response.status}:${text.slice(0, 1000)}`);
