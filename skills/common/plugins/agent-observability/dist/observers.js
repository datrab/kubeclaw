...
function sanitize(value, depth = 0) {
    ...
    if (Array.isArray(value))
        return value.slice(0, 1_000).map((entry) => sanitize(entry, depth + 1));
    ...
    if (typeof value === 'string' && value.length > 65_536)
        return `${value.slice(0, 65_536)}[truncated]`;
    return value;
}
export function projectAgentEvent(delivery) {
...
export async function ingest(delivery, context) {
...
export async function recordEvidence(delivery, context) {
