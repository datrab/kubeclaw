...
function configuredStringArray(value, label) {
...
function canonicalDirectory(value) {
    ...
    if (!fs.statSync(canonical).isDirectory())
        throw new Error(`COMMAND_WORKING_DIRECTORY_INVALID:${canonical}`);
    return canonical;
}
function canonicalExecutable(value) {
    const canonical = fs.realpathSync(path.resolve(value));
    if (!fs.statSync(canonical).isFile())
        throw new Error(`COMMAND_EXECUTABLE_INVALID:${canonical}`);
...
export function activate(context) {
    const executables = new Set(configuredStringArray(context.config.allowedExecutables, 'allowedExecutables').map(canonicalExecutable));
    ...
    return {
        ...
        async invoke({ request, signal }) {
            ...
            if (request.resource.type !== 'command.executable')
            ...
            if (!executables.has(executable))
                throw new Error(`COMMAND_EXECUTABLE_DENIED:${executable}`);
            ...
            return new Promise((resolve, reject) => {
                ...
                child.once('close', (code, terminationSignal) => {
                    ...
                    if (settled || timedOut || cancelled || outputExceeded)
                        return;
                    settled = true;
                    resolve({
                        exitCode: code,
        ...
        async shutdown() {
            stopping = true;
            const closing = [...active].map((child) => new Promise((resolve) => {
