...
function sanitize(value, depth = 0) {
    ...
    if (Array.isArray(value))
        return value.slice(0, 256).map((entry) => sanitize(entry, depth + 1));
    ...
    if (typeof value === 'string')
        return value.length <= 8_192 ? value : `${value.slice(0, 8_192)}…`;
    return value;
}
...
function configuredText(config, key, maximum) {
...
export function lifecycleNotification(delivery, maximum = 8_192, options = {}) {
...
export async function observe(delivery, context) {
    ...
    if (Array.isArray(suppressed) && suppressed.includes(delivery.event.type))
        return;
    const maximum = context.contract.config.maxMessageChars;
    const stageLabels = context.contract.config.stageLabels;
    await publish(lifecycleNotification(delivery, typeof maximum === 'number' ? maximum : 8_192, {
...
export function auditRecord(delivery) {
    return Object.freeze({
...
export async function recordAudit(delivery, context) {
