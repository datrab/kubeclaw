...
await assert.rejects(
  observe(delivery, {
    contract: {},
    async invoke() { throw new Error('TELEMETRY_SINK_CRASH'); },
    async emit() { throw new Error('unexpected domain event'); },
  }),
  /TELEMETRY_SINK_CRASH/,
);
console.log(JSON.stringify({
  ok: true,
  plugin: 'kubeclaw.telemetry-observer',
  suite: 'live-function',
  registrations: { 'observer:telemetry': ['pass', 'fail', 'crash'] },
}));
