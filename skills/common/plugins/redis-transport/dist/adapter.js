...
function config(value) {
    ...
    if (Object.keys(value).some((key) => !allowed.has(key)))
        throw new Error('REDIS_CONFIG_UNKNOWN_FIELD');
    ...
    const integer = (input, label, maximum) => {
        if (!Number.isSafeInteger(input) || Number(input) < 1 || Number(input) > maximum)
            throw new Error(`REDIS_CONFIG_INVALID:${label}`);
        return Number(input);
    };
    return {
        url,
...
function parseReply(buffer) {
    ...
    if (prefix === '+')
        return { value: header, bytes: lineEnd + 2 };
    if (prefix === '-')
        throw new Error(`REDIS_ERROR:${header.replace(/[\r\n]/gu, ' ')}`);
    if (prefix === ':')
    ...
    if (length === -1)
        return { value: null, bytes: lineEnd + 2 };
...
async function exchange(options, password, parts, signal) {
    if (signal.aborted)
...
function streamId(value) {
    if (typeof value !== 'string' || !/^[0-9]+-[0-9]+$/u.test(value))
        throw new Error('REDIS_STREAM_ID_INVALID');
    return value;
}
function activate(kind, context) {
    ...
    return {
        ...
        async invoke({ request, signal, confidential, fence }) {
            ...
            if (typeof secret.value !== 'string' || !secret.value || /[\r\n]/u.test(secret.value))
                throw new Error('REDIS_SECRET_INVALID');
            const payload = JSON.stringify(request.payload);
            if (Buffer.byteLength(payload) > 1_048_576)
                throw new Error('REDIS_PAYLOAD_SIZE_EXCEEDED');
...
export const activatePublisher = (context) => activate('publisher', context);
export const activateTelemetry = (context) => activate('telemetry', context);
