export const PROMPT_CONTRACT_VERSION = 'prompt-envelope.v1';
...
function canonicalize(value, limits, seen, depth, count) {
    if (depth > limits.maxDepth)
        throw new Error('PROMPT_VALUE_DEPTH_EXCEEDED');
    count.value += 1;
    if (count.value > limits.maxEntries)
        throw new Error('PROMPT_VALUE_ENTRIES_EXCEEDED');
    if (value === null || typeof value === 'boolean' || typeof value === 'string')
        return value;
    if (typeof value === 'number') {
        if (!Number.isFinite(value))
            throw new Error('PROMPT_VALUE_NUMBER_INVALID');
        return value;
    }
    if (typeof value !== 'object')
        throw new Error('PROMPT_VALUE_UNSUPPORTED');
    if (seen.has(value))
        throw new Error('PROMPT_VALUE_CYCLE');
    seen.add(value);
    try {
        if (Array.isArray(value)) {
            if (Object.keys(value).length !== value.length)
                throw new Error('PROMPT_VALUE_ARRAY_SPARSE');
            return value.map((entry) => canonicalize(entry, limits, seen, depth + 1, count));
        }
        if (!plainRecord(value))
            throw new Error('PROMPT_VALUE_PROTOTYPE_INVALID');
        const result = Object.create(null);
        for (const key of Object.keys(value).sort()) {
            if (!key || key.length > 128 || FORBIDDEN_KEYS.has(key))
                throw new Error('PROMPT_VALUE_KEY_INVALID');
...
export function stablePromptJson(value, limits = {}) {
    ...
    if (Buffer.byteLength(output, 'utf8') > resolved.maxBytes)
        throw new Error('PROMPT_VALUE_SIZE_EXCEEDED');
    return output;
}
export function createPromptEnvelope(input, limits = {}) {
    if (!plainRecord(input))
        throw new Error('PROMPT_ENVELOPE_INVALID');
    const allowed = new Set(['task', 'evidence', 'responseContract', 'guidance']);
    if (Object.keys(input).some((key) => !allowed.has(key)))
        throw new Error('PROMPT_ENVELOPE_UNKNOWN_FIELD');
    const guidance = input.guidance ?? [];
    if (!Array.isArray(guidance) || guidance.length > 100)
        throw new Error('PROMPT_GUIDANCE_INVALID');
    const envelope = {
    ...
    return JSON.parse(stablePromptJson(envelope, limits));
}
export function serializePromptEnvelope(input, limits = {}) {
