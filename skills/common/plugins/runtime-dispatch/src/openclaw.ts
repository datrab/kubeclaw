...
function requiredText(value: unknown, label: string): string {
  if (typeof value !== 'string' || !value.trim()) throw new Error(`OPENCLAW_${label}_INVALID`);
  return value.trim();
}
...
function sessionKey(value: unknown): string {
  ...
  return requiredText(
    source.childSessionKey
      ?? source.sessionKey
      ?? source.session_key
      ?? (record(source.session) ? source.session.sessionKey ?? source.session.session_key : null),
    'SESSION_KEY',
  );
}

function terminalState(value: unknown): { readonly terminal: boolean; readonly state: string } {
  const source = details(value);
  if (!record(source)) return { terminal: false, state: 'unknown' };
  let state = String(
    source.state
      ?? source.status
      ?? (record(source.session) ? source.session.state ?? source.session.status : 'unknown'),
  ).toLowerCase();
...
function subagentState(
  value: unknown,
  key: string,
): { readonly terminal: boolean; readonly state: string } {
  const source = details(value);
  if (!record(source)) return { terminal: false, state: 'unknown' };
  for (const collection of [source.active, source.recent]) {
    if (!Array.isArray(collection)) continue;
    const match = collection.find((entry) =>
      record(entry) && (entry.sessionKey === key || entry.session_key === key));
...
export function attachRuntimeEvidence(
  payload: JsonRecord,
  result: unknown,
