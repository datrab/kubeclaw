...
import type { AttemptIdentity, InvocationLease, LifecycleEvent, PluginContext, PluginDomainEvent, StageResult } from '../../sdk/src/index.ts';
...
export class StageExecutor {
  ...
  #runtime(runId: string, state: StageRuntimeState, guidance: Readonly<Record<string, unknown>> | undefined, owner: NonNullable<ReturnType<GrantedRegistry['snapshot']['stages']['get']>>): AttemptRuntime {
    ...
    const context = this.#context({ schemaVersion: 'plugin-context.v2', lease: leaseContract, config: definition.config, input: definition.input, ...(guidance === undefined ? {} : { guidance }), artifacts: [] }, lease, owner, attempt, controller);
