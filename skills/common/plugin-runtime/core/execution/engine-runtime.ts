...
import type { EffectRequest, LifecycleEvent, ObserverCheckpoint, PipelineDefinition, PluginDomainEvent } from '../../sdk/src/index.ts';
...
export async function prepareRuntime(platform: PlatformConfig, definition: PipelineDefinition): Promise<PreparedRuntime> {
  ...
  return { snapshot, granted, activated: await activateRegistry(snapshot, new Set(granted.grants.keys())) };
}
