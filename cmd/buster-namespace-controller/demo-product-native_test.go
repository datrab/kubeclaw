...
func TestNativeProductControllerTransitions(t *testing.T) {
	...
	for _, which := range []string{"foreign-sa", "wrong-signature", "wrong-source", "wrong-generation", "wrong-revision", "wrong-actor", "expired"} {
		t.Run(which, func(t *testing.T) {
			...
			case "expired":
				candidate.IssuedAt = time.Now().UTC().Add(-2 * time.Minute).Truncate(time.Second).Format(time.RFC3339)
			...
			expectedCode := 409
			expectedError := "DEMO_PRODUCT_SUBJECT_CHANGED"
			switch which {
			case "foreign-sa":
				expectedCode, expectedError = 401, "DEMO_PRODUCT_UNAUTHORIZED"
			case "wrong-signature":
				expectedError = "DEMO_PRODUCT_SIGNATURE_INVALID"
			case "wrong-actor":
				expectedError = "DEMO_PRODUCT_REQUEST_INVALID"
			case "expired":
				expectedError = "DEMO_PRODUCT_DECISION_EXPIRED"
			}
			if code != expectedCode || strings.TrimSpace(string(raw)) != expectedError {
				t.Fatalf("native negative %s: expected %d %s, got %d %s", which, expectedCode, expectedError, code, raw)
			}
			var after lease
			...
			t.Logf("native %s rejected HTTP %d %s", which, code, expectedError)
		})
	}
