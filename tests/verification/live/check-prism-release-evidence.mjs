...
for(const run of evidence.cleanRuns){assert.equal(run.status,"passed");assert.equal(run.repositoryCommit,expectedCommit);assert.deepEqual([...run.imageReferences].sort(),expectedImages);assert.match(run.namespace,/^test-prism-/);assert.match(run.baselineDigest,/^sha256:[a-f0-9]{64}$/);assert.match(run.evidenceArtifactDigest,/^sha256:[a-f0-9]{64}$/);assert.equal(run.skippedChecks?.length??0,0);}
