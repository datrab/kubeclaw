...
await record('helm render for nova values preserves service exposure, skills merge behavior, and writable config provenance', async () => {
  ...
  assert(parsed.checks.includes('Rendered Buster Service preserves gateway NodePort'));
  assert(parsed.checks.includes('Rendered Buster Deployment preserves sandbox image and privileged Podman-in-Pod surface'));
  assert(parsed.checks.includes('Rendered Buster Deployment starts buster-pipeline.ts and OpenClaw gateway in the same container'));
  ...
  assert(parsed.checks.includes('Rendered Buster Deployment keeps the legacy stream-processor sidecar disabled'));
  ...
  assert(parsed.checks.includes('Setup scripts classify optional failures and fence legacy broad git setup behind explicit opt-in'));
