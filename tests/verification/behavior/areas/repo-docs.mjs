...
await record('verification docs and hardening trackers point at tests-owned verifier entrypoints, not stale scripts wrappers', async () => {
  ...
  assert.equal(verificationReadme.includes('`scripts/deploy.sh build-local-images [tag]`, `scripts/deploy.sh verify-live [tag]`, `scripts/deploy.sh smoke`, and `scripts/deploy.sh smoke-agent <nova|buster>` are the live deployment command surface'), true);
