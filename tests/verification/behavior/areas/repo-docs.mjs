...
await record('behavior verification doc reflects the live repo-based workflow', async () => {
  ...
  assert.equal(behaviorDoc.includes('default behavior harness coverage currently spans `34` areas'), true);
  ...
  assert.equal(behaviorDoc.includes('fast/full wrappers are silent on clean passes unless a step emits warning output; use `--verbose` or `VERIFICATION_VERBOSE=1` to stream step banners and passing output'), true);
  assert.equal(behaviorDoc.includes('ACP launch reachability can also be run directly with `tests/verification/run-local-acp-verification.sh`'), true);
  assert.equal(behaviorDoc.includes('ACP failures mean local ACP agent/provider/gateway status setup needs attention and make full verification red'), true);
...
await record('verification docs and hardening trackers point at tests-owned verifier entrypoints, not stale scripts wrappers', async () => {
  ...
  assert.equal(verificationReadme.includes('`tests/verification/run-full-verification.sh` is intentionally exhaustive and fail-fast; it exits on the first red surface'), true);
  assert.equal(verificationReadme.includes('fast/full wrappers are silent on clean passes unless a step emits warning output; `--verbose` or `VERIFICATION_VERBOSE=1` restores step banners and pass output'), true);
  ...
  assert.equal(verificationReadme.includes('subagent and ACP launch are part of the full wrapper'), true);
  assert.equal(verificationReadme.includes('live Redis backend smoke is part of the full wrapper and fails when `REDIS_HOST` is not configured'), true);
  assert.equal(verificationReadme.includes('`tests/verification/run-local-acp-verification.sh` is the direct ACP/provider smoke wrapper'), true);
