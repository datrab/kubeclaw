...
- `check-subagent-launch.mjs` stays in the default `tests/verification/run-full-verification.sh` clean-checkout gate.
...
- ACP launch is local/provider-specific and is run explicitly through `tests/verification/run-local-acp-verification.sh`; ACP failure remains a real local smoke failure but does not fail the default clean-checkout wrapper.
