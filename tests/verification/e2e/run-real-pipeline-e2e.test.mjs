...
test('structured result files persist canonical metadata assertions cleanup and bounded diagnostics', () => {
  ...
  assert.equal(written.schema_version, 'real_pipeline_e2e_result.v2');
  ...
  assert.equal(persisted.artifact_paths.v2_runtime.endsWith('.swarm/v2-runtime'), true);
...
test('snapshotResultArtifacts copies only canonical v2 evidence into a stable result bundle', () => {
  ...
  write('progress.json', '{"run_id":"run-bundle"}\n');
  write('real-production-result.json', '{"schemaVersion":"real-production-pipeline-result.v2"}\n');
  write('v2-runtime/runs/run-bundle/events.jsonl', '{"entry":{"type":"run.succeeded"}}\n');
  write('artifacts/v2/catalog.jsonl', '{"namespace":"kubeclaw.project-summary"}\n');
  const durableResult = path.join(workspace.worktreePath, '.swarm', 'runtime-results', 'result.json');
  fs.mkdirSync(path.dirname(durableResult), { recursive: true });
  fs.writeFileSync(durableResult, '{"status":"PASS"}\n');
  ...
  assert.equal(fs.existsSync(snapshot.stable_artifacts.v2_runtime), true);
  assert.equal(fs.existsSync(snapshot.stable_artifacts.v2_artifacts), true);
  assert.equal(fs.readFileSync(path.join(snapshot.stable_artifacts.runtime_results, 'result.json'), 'utf8'), '{"status":"PASS"}\n');
  ...
  assert.equal(manifest.copied.v2_result, snapshot.stable_artifacts.v2_result);
  assert.equal(manifest.missing.length, 0);
});

test('stable bundle reports absent canonical v2 evidence without legacy path inference', () => {
  ...
  assert.equal(manifest.missing.find((entry) => entry.label === 'v2_result')?.status, 'missing');
  assert.equal(manifest.missing.some((entry) => entry.label === 'pipeline_summary'), false);
});
