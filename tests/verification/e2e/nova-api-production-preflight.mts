...
const immutableImage = process.env.KUBECLAW_API_PREFLIGHT_IMAGE;
const repositoryRoot = path.resolve(import.meta.dirname, '../../..');
const runtimeRevision = execFileSync('git', ['rev-parse', '--verify', 'HEAD'], {
  cwd: repositoryRoot, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'],
}).trim();
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'nova-api-preflight-'));
const fixture = path.join(temporary, 'fixture');
const state = path.join(temporary, 'nova-state');
const runId = `run:api-preflight:${crypto.randomUUID()}`;
...
try {
  if (!token) throw new Error('API_PREFLIGHT_TOKEN_MISSING');
  if (!privateKey) throw new Error('API_PREFLIGHT_SOURCE_KEY_MISSING');
  const image = immutableImage ? IMMUTABLE_IMAGE.exec(immutableImage) : null;
  if (!image) throw new Error('API_PREFLIGHT_IMMUTABLE_IMAGE_INVALID');
  const route = resolveProviderCapability(parseCapabilityProviders(), 'buster', 'test.plan.execute');
  if (route.adapter !== 'buster-plan-v1') throw new Error(`API_PREFLIGHT_PROVIDER_UNSUPPORTED:${route.adapter}`);

  fs.mkdirSync(path.join(fixture, 'k8s'), { recursive: true });
  fs.mkdirSync(path.join(fixture, '.swarm'), { recursive: true });
  fs.writeFileSync(path.join(fixture, '.swarm', 'api-flow.json'), `${JSON.stringify({
    schemaVersion: 'kubeclaw.api-flow.v1',
    steps: [{ id: 'root', path: '/', expect: { status: 200 } }],
  }, null, 2)}\n`);
  fs.writeFileSync(path.join(fixture, '.swarm', 'openapi.json'), `${JSON.stringify({
    openapi: '3.1.0', info: { title: 'API production preflight', version: '1.0.0' },
    paths: { '/': { get: { operationId: 'root', responses: { 200: { description: 'Ready' } } } } },
  }, null, 2)}\n`);
  fs.writeFileSync(path.join(fixture, 'k8s', 'deployment.yaml'), [
    'apiVersion: apps/v1', 'kind: Deployment', 'metadata:', '  name: api-preflight',
    '  labels: { app.kubernetes.io/name: api-preflight }', 'spec:', '  replicas: 1',
    '  selector:', '    matchLabels: { app.kubernetes.io/name: api-preflight }', '  template:',
    '    metadata:', '      labels: { app.kubernetes.io/name: api-preflight }', '    spec:',
    ...
    'apiVersion: v1', 'kind: Service', 'metadata:', '  name: api-preflight', 'spec:', '  type: ClusterIP',
    '  selector: { app.kubernetes.io/name: api-preflight }', '  ports:',
  ...
  git('add', '.');
  git('commit', '-qm', 'Create API production preflight fixture');
  ...
  const registry = buildRegistry(discoverPackages({ installationRoots: [pluginRoot], trustPolicy: {
    ...
    verifierId: 'api-production-preflight',
  } }));
  ...
  const declaration: any = { tests: {
    'checked-manifest': { uses: 'kubeclaw.direct-command@1', mode: 'blocking', retries: 0,
      concurrencyGroup: 'manifest', config: { executable: 'cp',
        args: ['k8s/deployment.yaml', '.swarm/checked-api-preflight.yaml'], workingDirectory: '.',
        resultMode: 'exit-code', artifacts: [{ id: 'checked-manifest', path: '.swarm/checked-api-preflight.yaml',
          mediaType: 'application/vnd.kubeclaw.checked-kubernetes-yaml' }] } },
    http: { uses: 'kubeclaw.http@1', mode: 'blocking', retries: 0, needs: ['kubernetes-deployment'],
      concurrencyGroup: 'api-http', config: { path: '/', expectedStatuses: [200] }, inputs: deploymentInput },
    flow: { uses: 'kubeclaw.api-flow@1', mode: 'blocking', retries: 0, needs: ['kubernetes-deployment'],
      concurrencyGroup: 'api-http', config: { flowFile: '.swarm/api-flow.json' }, inputs: deploymentInput },
    openapi: { uses: 'kubeclaw.openapi@1', mode: 'blocking', retries: 0, needs: ['kubernetes-deployment'],
      concurrencyGroup: 'api-http', config: { specFile: '.swarm/openapi.json',
        operations: [{ operationId: 'root', expectedStatuses: [200] }] }, inputs: deploymentInput },
  }, fixtures: {
    'kubernetes-deployment': { uses: 'kubeclaw.kubernetes-fixture@1', retries: 0,
      ...
      config: { image: { reference: immutableImage, digest: image[1] }, serviceName: 'api-preflight',
  ...
  }, concurrencyLimits: { manifest: 1, 'kubernetes-fixture': 1, 'api-http': 3 } };
  const limits = { cpuMillis: 900_000, memoryBytes: 1024 * 1024 * 1024,
    logBytes: 4 * 1024 * 1024, artifactBytes: 32 * 1024 * 1024, artifactFiles: 64, processes: 32 };
  const plan = resolveTestPlan({ planId: 'plan:api:production-preflight', runId,
    project: 'api-production-preflight', scope: { moduleId: null, gateId: 'production-preflight' },
    createdAt: new Date().toISOString(), declaration, suiteTemplates: [], registry,
    facts: { changedPaths: ['k8s/deployment.yaml', '.swarm/api-flow.json', '.swarm/openapi.json'],
      moduleType: 'service', pipelineStage: 'preflight' },
    policy: { defaultTimeoutMs: 900_000, maximumTimeoutMs: 900_000, defaultLimits: limits,
      maximumLimits: limits, maximumRetryCount: 1, maximumMatrixSize: 1, maximumNodes: 5,
      defaultConcurrencyLimit: 1, maximumConcurrencyLimits: { manifest: 1, 'kubernetes-fixture': 1, 'api-http': 3 } } });
  assert.deepEqual(plan.nodes.map((node) => node.id).sort(),
    ['checked-manifest', 'flow', 'http', 'kubernetes-deployment', 'openapi']);
  const grants = new Map<string, readonly string[]>([
    ['checked-manifest', ['command.execute']], ['kubernetes-deployment', ['kubernetes.fixture']],
    ['http', ['network.http']], ['flow', ['network.http']], ['openapi', ['network.http']],
  ]);
  ...
  assert.ok(imported.evidenceDigests.length >= 3);
  ...
  for (const nodeId of ['http', 'flow', 'openapi']) {
    assert.equal(imported.remoteResult.attempts.find((attempt: any) => attempt.nodeId === nodeId)?.outcome, 'passed');
  }
  ...
  process.stdout.write(`${JSON.stringify({ ok: true, schemaVersion: 'nova-api-production-preflight.v1',
    suite: 'api', runId, jobId: result.remote.decision.jobId, provider: route, runtimeRevision,
    ...
    clusterCleanupObserved: false, providersVerified: ['kubeclaw.http@1', 'kubeclaw.api-flow@1', 'kubeclaw.openapi@1'],
    immutableImage, imageDigest: image[1], resources: { leaseName: deployment.leaseName,
      namespace: deployment.namespace, serviceName: 'api-preflight', servicePort: 80 },
    boundary: 'Nova signed source to remote Buster isolated API provider plan', mocks: 0, emulators: 0 }, null, 2)}\n`);
} finally {
