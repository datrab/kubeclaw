...
const immutableImage = process.env.KUBECLAW_HTTP_PREFLIGHT_IMAGE;
const expectedText = process.env.KUBECLAW_HTTP_PREFLIGHT_EXPECTED_TEXT;
const repositoryRoot = path.resolve(import.meta.dirname, '../../..');
const runtimeRevision = execFileSync('git', ['rev-parse', '--verify', 'HEAD'], {
  cwd: repositoryRoot, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'],
}).trim();
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'nova-http-preflight-'));
const fixture = path.join(temporary, 'fixture');
const state = path.join(temporary, 'nova-state');
const runId = `run:http-preflight:${crypto.randomUUID()}`;
...
try {
  if (!token) throw new Error('HTTP_PREFLIGHT_TOKEN_MISSING');
  if (!privateKey) throw new Error('HTTP_PREFLIGHT_SOURCE_KEY_MISSING');
  const image = immutableImage ? IMMUTABLE_IMAGE.exec(immutableImage) : null;
  if (!image) throw new Error('HTTP_PREFLIGHT_IMMUTABLE_IMAGE_INVALID');
  const route = resolveProviderCapability(parseCapabilityProviders(), 'buster', 'test.plan.execute');
  if (route.adapter !== 'buster-plan-v1') throw new Error(`HTTP_PREFLIGHT_PROVIDER_UNSUPPORTED:${route.adapter}`);
  ...
  fs.writeFileSync(path.join(fixture, 'k8s', 'deployment.yaml'), [
    'apiVersion: apps/v1', 'kind: Deployment', 'metadata:', '  name: http-preflight',
    '  labels: { app.kubernetes.io/name: http-preflight }', 'spec:', '  replicas: 1',
    '  selector:', '    matchLabels: { app.kubernetes.io/name: http-preflight }', '  template:',
    '    metadata:', '      labels: { app.kubernetes.io/name: http-preflight }', '    spec:',
    ...
    'apiVersion: v1', 'kind: Service', 'metadata:', '  name: http-preflight', 'spec:', '  type: ClusterIP',
    '  selector: { app.kubernetes.io/name: http-preflight }', '  ports:',
  ...
  git('add', '.');
  git('commit', '-qm', 'Create HTTP production preflight fixture');
  ...
  const registry = buildRegistry(discoverPackages({ installationRoots: [pluginRoot], trustPolicy: {
    ...
    verifierId: 'http-production-preflight',
  } }));
  const declaration: any = { tests: {
    'checked-manifest': { uses: 'kubeclaw.direct-command@1', mode: 'blocking', retries: 0,
      concurrencyGroup: 'manifest', config: { executable: 'cp',
        args: ['k8s/deployment.yaml', '.swarm/checked-http-preflight.yaml'], workingDirectory: '.',
        resultMode: 'exit-code', artifacts: [{ id: 'checked-manifest', path: '.swarm/checked-http-preflight.yaml',
          mediaType: 'application/vnd.kubeclaw.checked-kubernetes-yaml' }] } },
    'in-cluster-http': { uses: 'kubeclaw.http@1', mode: 'blocking', retries: 0,
      needs: ['kubernetes-deployment'], concurrencyGroup: 'http', config: { path: '/', expectedStatuses: [200],
    ...
    'kubernetes-deployment': { uses: 'kubeclaw.kubernetes-fixture@1', retries: 0,
      ...
      config: { image: { reference: immutableImage, digest: image[1] }, serviceName: 'http-preflight',
  ...
  }, concurrencyLimits: { manifest: 1, 'kubernetes-fixture': 1, http: 1 } };
  ...
  const plan = resolveTestPlan({ planId: 'plan:http:production-preflight', runId,
    project: 'http-production-preflight', scope: { moduleId: null, gateId: 'production-preflight' },
    ...
    policy: { defaultTimeoutMs: 600_000, maximumTimeoutMs: 600_000, defaultLimits: limits,
      ...
      defaultConcurrencyLimit: 1, maximumConcurrencyLimits: { manifest: 1, 'kubernetes-fixture': 1, http: 1 } } });
  assert.deepEqual(plan.nodes.map((node) => node.id).sort(),
    ['checked-manifest', 'in-cluster-http', 'kubernetes-deployment']);
  const grants = new Map<string, readonly string[]>([
    ['checked-manifest', ['command.execute']], ['kubernetes-deployment', ['kubernetes.fixture']],
    ['in-cluster-http', ['network.http']],
  ]);
  ...
  const result = await nova.execute({ idempotencyKey: `http:${crypto.randomUUID()}`,
    pipelineStageId: 'stage:http-preflight', plan, repositoryRoot: fixture,
    repositoryId: 'repository:http-preflight', grants, maximumConcurrency: 1,
  ...
  assert.deepEqual(result.remote.decision.nodes.map((node) => [node.nodeId, node.effect]).sort(), [
    ['checked-manifest', 'passed'], ['in-cluster-http', 'passed'], ['kubernetes-deployment', 'passed'],
  ]);
  ...
  const httpAttempt = imported.remoteResult.attempts.find((attempt: any) => attempt.nodeId === 'in-cluster-http');
  const target = httpAttempt?.providerDetails?.values?.url;
  const httpStatus = httpAttempt?.providerDetails?.values?.status;
  assert.ok(deployment?.leaseName && deployment?.namespace);
  assert.equal(target,
    new URL(`http://http-preflight.${deployment.namespace}.svc.cluster.local:80/`).href);
  assert.equal(Number.isSafeInteger(httpStatus) && httpStatus >= 200 && httpStatus <= 299, true);
  ...
  process.stdout.write(`${JSON.stringify({ ok: true, schemaVersion: 'nova-http-production-preflight.v1',
    suite: 'health', runId, jobId: result.remote.decision.jobId, provider: route, runtimeRevision,
    ...
    decision: result.remote.decision.state, nodes: result.remote.decision.nodes.map((node) => ({
      id: node.nodeId, effect: node.effect })), evidenceDigests: imported.evidenceDigests,
    evidenceImported: true, runnerCleanupVerified: true, cleanupVerified: false,
    clusterCleanupObserved: false, resources: { leaseName: deployment.leaseName, namespace: deployment.namespace,
      serviceName: 'http-preflight', servicePort: 80 },
    target, httpStatus, networkRequestVerified: true,
    boundary: 'Nova signed source to remote Buster isolated HTTP provider plan', mocks: 0, emulators: 0 }, null, 2)}\n`);
} finally {
