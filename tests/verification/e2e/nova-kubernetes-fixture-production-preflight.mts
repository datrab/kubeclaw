...
const SECRET_NAME = /^[a-z0-9](?:[a-z0-9.-]{0,251}[a-z0-9])?$/u;
const token = process.env.BUSTER_V2_TOKEN;
const privateKey = process.env.BUSTER_SOURCE_ATTESTATION_PRIVATE_KEY;
const immutableImage = process.env.KUBECLAW_KUBERNETES_PREFLIGHT_IMAGE;
const secretName = process.env.KUBECLAW_KUBERNETES_PREFLIGHT_SECRET;
const repositoryRoot = path.resolve(import.meta.dirname, '../../..');
const runtimeRevision = execFileSync('git', ['rev-parse', '--verify', 'HEAD'], {
  cwd: repositoryRoot, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'],
}).trim();
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'nova-kubernetes-fixture-preflight-'));
const fixture = path.join(temporary, 'fixture');
const state = path.join(temporary, 'nova-state');
const runId = `run:kubernetes-fixture-preflight:${crypto.randomUUID()}`;
...
try {
  if (!token) throw new Error('KUBERNETES_FIXTURE_PREFLIGHT_TOKEN_MISSING');
  if (!privateKey) throw new Error('KUBERNETES_FIXTURE_PREFLIGHT_SOURCE_KEY_MISSING');
  const image = immutableImage ? IMMUTABLE_IMAGE.exec(immutableImage) : null;
  if (!image) throw new Error('KUBERNETES_FIXTURE_PREFLIGHT_IMMUTABLE_IMAGE_INVALID');
  if (!secretName || !SECRET_NAME.test(secretName)) {
    throw new Error('KUBERNETES_FIXTURE_PREFLIGHT_SECRET_INVALID');
  }
  const route = resolveProviderCapability(parseCapabilityProviders(), 'buster', 'test.plan.execute');
  if (route.adapter !== 'buster-plan-v1') {
    throw new Error(`KUBERNETES_FIXTURE_PREFLIGHT_PROVIDER_UNSUPPORTED:${route.adapter}`);
  }
  ...
  fs.writeFileSync(path.join(fixture, 'k8s', 'deployment.yaml'), [
    'apiVersion: apps/v1', 'kind: Deployment', 'metadata:', '  name: kubernetes-preflight',
    '  labels: { app.kubernetes.io/name: kubernetes-preflight }', 'spec:', '  replicas: 1',
    '  selector:', '    matchLabels: { app.kubernetes.io/name: kubernetes-preflight }', '  template:',
    '    metadata:', '      labels: { app.kubernetes.io/name: kubernetes-preflight }', '    spec:',
    ...
    'apiVersion: v1', 'kind: Service', 'metadata:', '  name: kubernetes-preflight', 'spec:',
    '  type: ClusterIP', '  selector: { app.kubernetes.io/name: kubernetes-preflight }', '  ports:',
  ...
  git('add', '.');
  git('commit', '-qm', 'Create Kubernetes fixture production preflight');
  ...
  const registry = buildRegistry(discoverPackages({ installationRoots: [pluginRoot], trustPolicy: {
    ...
    verifierId: 'kubernetes-fixture-production-preflight',
  } }));
  const declaration: any = { tests: {
    'checked-manifest': { uses: 'kubeclaw.direct-command@1', mode: 'blocking', retries: 0,
      concurrencyGroup: 'manifest', config: { executable: 'cp',
        args: ['k8s/deployment.yaml', '.swarm/checked-kubernetes-preflight.yaml'], workingDirectory: '.',
        resultMode: 'exit-code', artifacts: [{ id: 'checked-manifest',
          path: '.swarm/checked-kubernetes-preflight.yaml',
    ...
    'kubernetes-deployment': { uses: 'kubeclaw.kubernetes-fixture@1', retries: 0,
      ...
      config: { image: { reference: immutableImage, digest: image[1] }, serviceName: 'kubernetes-preflight',
        servicePort: 80, namespacePrefix: 'test', retention: { mode: 'retain', seconds: 600 },
        readinessTimeoutSeconds: 300, secretReferences: [secretName] }, inputs: {
        'checked-manifest': { from: 'checked-manifest', output: 'artifact-1',
          mediaType: 'application/vnd.kubeclaw.checked-kubernetes-yaml' },
      } },
  }, concurrencyLimits: { manifest: 1, 'kubernetes-fixture': 1 } };
  ...
  const plan = resolveTestPlan({ planId: 'plan:kubernetes-fixture:production-preflight', runId,
    project: 'kubernetes-fixture-production-preflight',
    scope: { moduleId: null, gateId: 'production-preflight' }, createdAt: new Date().toISOString(),
    declaration, suiteTemplates: [], registry,
    facts: { changedPaths: ['k8s/deployment.yaml'], moduleType: 'service', pipelineStage: 'preflight' },
    policy: { defaultTimeoutMs: 600_000, maximumTimeoutMs: 600_000, defaultLimits: limits,
      maximumLimits: limits, maximumRetryCount: 1, maximumMatrixSize: 1, maximumNodes: 2,
      defaultConcurrencyLimit: 1, maximumConcurrencyLimits: { manifest: 1, 'kubernetes-fixture': 1 } } });
  assert.deepEqual(plan.nodes.map((node) => node.id).sort(), ['checked-manifest', 'kubernetes-deployment']);
  ...
  const nova = createProductionNovaTestGate({ stateRoot: state, endpoint: route.endpoint, token,
    ...
    maximumResponseBytes: 32 * 1024 * 1024, maximumResultBytes: 32 * 1024 * 1024,
  ...
  assert.deepEqual(result.remote.decision.nodes.map((node) => [node.nodeId, node.effect]).sort(), [
    ['checked-manifest', 'passed'], ['kubernetes-deployment', 'passed'],
  ]);
  ...
  assert.equal(imported.jobId, result.remote.decision.jobId);
  ...
  const producer = imported.remoteResult.attempts.find((attempt: any) => attempt.nodeId === 'checked-manifest');
  const manifest = producer?.outputs.find((output: any) => output.kind === 'artifact')?.artifact;
  ...
  assert.match(manifest?.contentDigest, /^sha256:[a-f0-9]{64}$/u);
  assert.equal(deployment?.manifestDigest, manifest.contentDigest);
  assert.equal(deployment?.immutableImage, immutableImage);
  assert.deepEqual(deployment?.secretReferences, [secretName]);
  ...
  process.stdout.write(`${JSON.stringify({ ok: true,
    schemaVersion: 'kubernetes-fixture-production-preflight.v1', suite: 'k8s', runId,
    ...
    clusterCleanupObserved: false, deploymentReadyVerified: true, podReadyVerified: true,
    manifestAppliedVerified: true, approvedSecretCopyVerified: false, namespaceDeleted: false,
    imageDigest: image[1], manifestDigest: manifest.contentDigest,
    resources: { leaseName: deployment.leaseName, namespace: deployment.namespace,
      secretName, serviceName: 'kubernetes-preflight', servicePort: 80 },
    boundary: 'Nova signed source to remote Buster isolated Kubernetes fixture plan',
