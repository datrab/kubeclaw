...
import { runNamespaceFenceMatrix } from './product-native-fence.mjs';
import { runStatusMatrix } from './product-native-status.mjs';
...
try {
  ...
  const convert = yaml => ({ items: yaml.split(/^---\s*$/m).filter(part => /\bkind:/.test(part)).map(part => JSON.parse(run(kubectl, ['--kubeconfig', config, 'create', '--dry-run=client', '--validate=false', '-f', '-', '-o', 'json'], { input: part }))) });
  await runNamespaceFenceMatrix({ request, run, chart, convert, record, sleep });
  await runStatusMatrix({ request, record, crd });
  if (process.env.NATIVE_CONTROLLER === 'true') {
    const testConfig = path.join(temp, 'controller-test.json');
    fs.writeFileSync(testConfig, JSON.stringify({ URL: `https://127.0.0.1:${apiPort}`, Token: token, CA: path.join(temp, 'cert.pem') }), { mode: 0o600 });
    try {
      record('native-controller.txt', run(process.env.GO ?? 'go', ['test', '-tags=native_product', './cmd/buster-namespace-controller', '-run=^TestNativeProductControllerTransitions$', '-count=1', '-v'], { cwd: root, timeout: 90000, env: { ...process.env, KUBECLAW_NATIVE_TEST_CONFIG: testConfig } }));
    } catch (error) {
      record('native-controller.txt', `${error.stdout ?? ''}\n${error.stderr ?? ''}`);
      throw error;
    }
  }
  console.log('Native RBAC/ServiceAccount/VAP fence and original CRD status/CEL matrix passed. No workload or full product-delivery claim.');
} finally {
