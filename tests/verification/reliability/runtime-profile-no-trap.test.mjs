...
import { runtimeDispatchProfileFields, CURRENT_RUNTIME_DISPATCH_PROFILE, portableJson, canonicalJson, withRuntimeDispatchProfile } from '@kubeclaw/plugin-sdk';
...
test('runtime profile selector preserves legitimate JSON metadata and original codec bytes', () => {
  const base = { operation: 'dispatch', resource: { type: 'runtime.agent', canonicalId: 'probe' }, payload: { runtimeDispatchProfile: { modelOwned: true }, z: 1, 'ä': 2 } };
  const attempt = { runId: 'run:json', stageId: 'stage', attemptId: 'attempt:json', attemptNumber: 1 };
  const inputs = [base, { ...base, attempt, idempotencyKey: 'effect:json', capability: 'runtime.dispatch' },
    { ...base, deliveryId: 'delivery:json', dependencyIdentity: { prefix: 'adapter:', suffix: '', currentKey: 'key', scope: 'scope' } },
    { ...base, runtimeDispatchProfile: CURRENT_RUNTIME_DISPATCH_PROFILE }, Object.assign(Object.create(null), base)];
  for (const value of inputs) {
    const before = [canonicalJson(value), portableJson(value)], selected = runtimeDispatchProfileFields(value, 'runtime.dispatch');
    assert.deepEqual(selected, Object.hasOwn(value, 'runtimeDispatchProfile') ? { runtimeDispatchProfile: CURRENT_RUNTIME_DISPATCH_PROFILE } : {});
    assert.deepEqual([canonicalJson(value), portableJson(value)], before);
  }
  assert.equal(withRuntimeDispatchProfile(base), base);
  assert.throws(() => runtimeDispatchProfileFields({ ...base, runtimeDispatchProfile: undefined }, 'runtime.dispatch'), /CANONICAL_JSON_/);
  assert.throws(() => runtimeDispatchProfileFields({ ...base, runtimeDispatchProfile: CURRENT_RUNTIME_DISPATCH_PROFILE }, 'operator.request'), /RUNTIME_DISPATCH_PROFILE_CAPABILITY_INVALID/);
});
