...
test('canonical resource owner admits real resource namespaces without relaxing other request fields', () => {
  ...
  const mutations = [v => v.effectId = 'x'.repeat(193), v => v.deliveryId = 'x'.repeat(513), v => v.deliveryId = '  ', v => v.resource.canonicalId = '', v => v.resource.extra = true, v => v.attempt.attemptNumber = 0, v => v.requestedAt = '2026-99-99T99:99:99Z', v => v.payload = [], v => v.extra = true];
...
test('original activation IDs and public delivery tokens propagate only to owning effect and lock fields', () => {
  const activation = `adapter:${namespaced(160)}:${'r'.repeat(96)}`;
  const value = request(legacy());
  value.attempt = {runId: activation, stageId: 'adapter-activation', attemptId: activation, attemptNumber: 1};
  value.deliveryId = 'delivery/ä '.repeat(40);
  validateContractValue('effectRequest', value);
  validateContractValue('resourceLock', {schemaVersion: 'resource-lock.v2', lockId: 'lock:one', resource: value.resource, ownerLeaseId: activation, fencingToken: 1, status: 'active', acquiredAt: value.requestedAt, expiresAt: value.requestedAt});
  assert.throws(() => validateContractValue('opaqueId', activation));
  assert.throws(() => validateContractValue('attemptIdentity', value.attempt));
  for (const mutate of [v => v.attempt.stageId = 'foreign', v => v.attempt.attemptNumber = 2, v => v.attempt.extra = true, v => v.attempt.runId += 'x']) {
    const altered = structuredClone(value); mutate(altered); assert.throws(() => validateContractValue('effectRequest', altered));
  }
});
