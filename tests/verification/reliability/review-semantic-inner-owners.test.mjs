...
import { buildReviewTask } from '../../../skills/nova/plugins/review/src/protocol.ts';
import { buildVerificationDispatchRequest } from '../../../skills/nova/plugins/review/src/verification-protocol.ts';
import { mapReviewVerificationVerdicts } from '../../../skills/nova/plugins/review/src/review-verdict-policy.ts';
...
test('protocol snapshot owner guards reject Proxy/getter before any new bundle reflection', () => {
  const policy = resolve(mode);
  const consumers = [snapshot => buildReviewTask(snapshot, '', policy),
    snapshot => buildVerificationDispatchRequest('echo', {}, snapshot, {}, policy),
    snapshot => mapReviewVerificationVerdicts(snapshot, {}, {}, policy)];
  for (const consume of consumers) {
    let calls = 0;
    const proxy = new Proxy({}, { get() { calls++; }, ownKeys() { calls++; return []; }, getOwnPropertyDescriptor() { calls++; } });
    const getter = { get bundle() { calls++; return {}; } };
    assert.throws(() => consume(proxy)); assert.equal(calls, 0);
    assert.throws(() => consume(getter)); assert.equal(calls, 0);
  }
});
