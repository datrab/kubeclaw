...
async function consume() {
  ...
  try {
    ...
    const result = await runner.run(runId);
    assert.equal(result.status, expectMissingImport ? 'blocked' : 'succeeded');
    const recovery = await effects.recoveryEntries();
    ...
    assert.equal(receipt?.status, expectMissingImport ? 'failed' : 'completed');
    ...
    const proof = {
      ...
      boundary: { manifestIntegrityRejected: false, importStoreReached: true, providerExecution: false, importedResultSuccess: !expectMissingImport,
...
if (phase === 'produce') await produce(); else if (['consume-en', 'consume-cs', 'consume-da', 'consume-tr', 'consume-sv', 'consume-missing'].includes(phase)) await consume(); else throw new Error(`unknown phase: ${phase}`);
