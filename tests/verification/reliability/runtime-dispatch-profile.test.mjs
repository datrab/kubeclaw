...
import {CURRENT_RUNTIME_DISPATCH_PROFILE as profile, parseRuntimeDispatchProfile, withRuntimeDispatchProfile,
  runtimeDispatchProfileFields, portableJson, sha256Text} from '@kubeclaw/plugin-sdk';
...
test('new original snapshot writes bind required profile; rehashed malformed/future/downgraded contracts reject',()=>{
  ...
  try{
    ...
    for(const changed of [without,{...snapshot,runtimeDispatchProfile:{...profile,encoding:'future'}},
      ...
      {...snapshot,registry:null},{...snapshot,graph:{schemaVersion:'execution-graph-snapshot.v3'}},{...snapshot,undeclared:true}]){
...
test('profile is bound after budget reservation and excluded from actual model task without changing accounting',()=>{
  ...
  const invocation={operation:'dispatch',resource:{type:'runtime.agent',canonicalId:'agent'},payload:reserved};
  const current=withRuntimeDispatchProfile(invocation,profile),legacy=withRuntimeDispatchProfile(invocation,undefined);
  assert.equal(legacy,invocation);assert.equal(current.payload,reserved);
  assert.deepEqual(runtimeDispatchProfileFields(current),{runtimeDispatchProfile:profile});
  ...
  const oldTask=prepareOpenClawTask(legacy.payload,'/tmp/profile-result.json',target);
  const newTask=prepareOpenClawTask(current.payload,'/tmp/profile-result.json',target);
  assert.equal(newTask,oldTask);assert.doesNotMatch(newTask,/runtimeDispatchProfile|runtimePromptBudget/);
  assert.throws(()=>runtimeDispatchProfileFields({...invocation,runtimeDispatchProfile:undefined}));
