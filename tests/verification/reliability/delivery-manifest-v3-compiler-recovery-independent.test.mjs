...
test('registered project-summary input schema admits the valid final-Review v3 compiler graph', async () => {
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'delivery-v3-summary-schema-'));
  let fixture;
  try {
    fixture = await fixtureModule.projectSourceFixture(temporary, { review: true, clean: false });
    fixture.project.final.review = { agent: 'echo' };
    const compiled = compileProject(fixture.project, PORTABLE, PORTABLE, REVIEW, DELIVERY);
    const summary = compiled.definition.stages.find(stage => stage.id === 'project-summary');
    assert.equal(summary.input.final.reviewArtifactEncoding, PORTABLE);
    const schema = JSON.parse(fs.readFileSync(path.join(
      sourceRoot,
      'skills/nova/plugins/project-summary/schemas/input.schema.json',
    ), 'utf8'));
    const validate = new Ajv2020({ allErrors: true, strict: true }).compile(schema);
    assert.equal(validate(summary.input), true, JSON.stringify(validate.errors));
  } finally {
    await fixture?.close();
    fs.rmSync(temporary, { recursive: true, force: true });
  }
