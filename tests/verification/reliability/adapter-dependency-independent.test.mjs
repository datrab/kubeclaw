...
test('independent genuine legacy child history cannot hide corruption behind an absent or changed requested subject',async t=>{
  ...
  try{
    ...
    const variants=[
      ...
      ['child request before parent accepted',rows=>{const at=rows.findIndex(row=>row.type==='accepted'&&row.request.capability==='runtime.dispatch');rows.push(...rows.splice(at,1));return rows;}],
      ...
      ['valid foreign attempt hidden behind known legacy owner key',rows=>{
        let effectId;
        for(const row of rows)if(row.request?.capability==='network.http'){
          row.request.attempt.attemptId='attempt:foreign'; row.request.effectId=stableEffectId(row.request); effectId=row.request.effectId;
        }
        rows.find(row=>row.type==='completed').receipt.effectId=effectId;
        return rows;
      }],
    ];
    for(const [name,change]of variants){
      ...
      assert.match(resumed.err,/ADAPTER_DEPENDENCY_(?:HISTORY_INVALID|REQUEST_VERSION_INVALID|HISTORY_OWNER_MISMATCH)/,name);
