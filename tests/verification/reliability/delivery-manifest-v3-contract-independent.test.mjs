...
function withReview(unsigned, mode) {
  const value = structuredClone(unsigned);
  value.final.reviewStageId = 'review';
  if (mode !== 'legacy') value.final.reviewArtifactEncoding = 'kubeclaw-json.utf16.v1';
  if (mode === 'semantic') value.final.reviewSemanticEncoding = 'review-semantics.utf16-v1';
  const report = artifact('review-report:contract', 'kubeclaw.review', 'review', 'review:1');
  const bundle = artifact('review-bundle:contract', 'kubeclaw.review', 'review', 'review:1');
  if (mode !== 'legacy') {
    report.encoding = 'kubeclaw-json.utf16.v1';
    bundle.encoding = 'kubeclaw-json.utf16.v1';
  }
  value.evidence.push(report, bundle);
  return value;
}

function manyModuleManifest(count) {
  const modules = [];
  const evidence = [];
  const implementations = [];
  for (let index = 0; index < count; index += 1) {
    const moduleId = `module-${String(index).padStart(3, '0')}`;
    const sourceStageId = `forge-${index}`;
    const testStageId = `test-${index}`;
    const template = policy('module');
    const { policyDigest: _ignoredPolicyDigest, ...modulePolicyBody } = template;
    modulePolicyBody.modules = [{ ...modulePolicyBody.modules[0], moduleId }];
    modulePolicyBody.requiredChecks = [{
      ...modulePolicyBody.requiredChecks[0],
      requirementRefs: [{ moduleId, requirementId: 'works' }],
    }];
    const expectedCoverage = { ...modulePolicyBody, policyDigest: gateCoverageDigest(modulePolicyBody) };
    modules.push({ moduleId, sourceStageId, testStageId, expectedCoverage, sourceRevision: revision,
      decisionDigest: sha256Text(`decision-${index}`), resultDigest: sha256Text(`result-${index}`),
      coverage: coverageResult(testStageId, expectedCoverage) });
    const implementation = artifact(`implementation:${moduleId}`, 'kubeclaw.implementation-agent', sourceStageId, `${sourceStageId}:1`);
    implementations.push(implementation);
    evidence.push(implementation,
      artifact(`quality:${testStageId}:decision:1`, 'kubeclaw.buster-quality-gate', testStageId, `${testStageId}:1`),
      artifact(`quality:${testStageId}:1`, 'kubeclaw.buster-quality-gate', testStageId, `${testStageId}:1`));
  }
  const finalTemplate = policy('cumulative');
  const { policyDigest: _ignoredFinalDigest, ...finalPolicyBody } = finalTemplate;
  finalPolicyBody.modules = modules.map(module => module.expectedCoverage.modules[0])
    .sort((left, right) => left.moduleId < right.moduleId ? -1 : left.moduleId > right.moduleId ? 1 : 0);
  finalPolicyBody.requiredChecks = [{
    ...finalPolicyBody.requiredChecks[0],
    requirementRefs: modules.map(module => ({ moduleId: module.moduleId, requirementId: 'works' })),
  }];
  const expectedCoverage = { ...finalPolicyBody, policyDigest: gateCoverageDigest(finalPolicyBody) };
  const finalSource = modules.at(-1).sourceStageId;
  evidence.push(implementations.at(-1),
    artifact('quality:final-test:decision:1', 'kubeclaw.buster-quality-gate', 'final-test', 'final-test:1'),
    artifact('quality:final-test:1', 'kubeclaw.buster-quality-gate', 'final-test', 'final-test:1'),
    artifact('lint:app', 'kubeclaw.lint', 'lint', 'lint:1'));
  return {
    schemaVersion: 'delivery-manifest.v3', projectId: 'app', runId, sourceRevision: revision, modules,
    final: { sourceStageId: finalSource, lintStageId: 'lint', testStageId: 'final-test', expectedCoverage,
      sourceRevision: revision, decisionDigest: sha256Text('final-decision'), resultDigest: sha256Text('final-result'),
      coverage: coverageResult('final-test', expectedCoverage) },
    evidence,
  };
}
...
test('published schema rejects impossible Review optional-field combinations admitted by neither runtime nor type contract', () => {
  const validate = schemaValidator();
  const invalid = unsignedManifest();
  invalid.final.reviewSemanticEncoding = 'review-semantics.utf16-v1';
  const value = withDigest(invalid);
  const schemaResult = validate(value);
  let runtimeResult = true;
  try { contract.parseDeliveryManifestV3(value); } catch { runtimeResult = false; }
  assert.equal(runtimeResult, false);
  assert.equal(schemaResult, false, `Review conditional schema errors: ${JSON.stringify(validate.errors)}`);
});

test('published schema matches runtime safe-integer producer attempts', () => {
  const validate = schemaValidator();
  const invalid = unsignedManifest();
  invalid.evidence[0] = structuredClone(invalid.evidence[0]);
  invalid.evidence[0].producer.attemptNumber = Number.MAX_SAFE_INTEGER + 1;
  invalid.evidence[3] = invalid.evidence[0];
  const value = withDigest(invalid);
  const schemaResult = validate(value);
  let runtimeResult = true;
  try { contract.parseDeliveryManifestV3(value); } catch { runtimeResult = false; }
  assert.equal(runtimeResult, false);
  assert.equal(schemaResult, false, `attemptNumber schema errors: ${JSON.stringify(validate.errors)}`);
});

test('all four finite Review arms pass and ordering/attempt/duplicate disagreements reject', () => {
  for (const value of [unsignedManifest(), withReview(unsignedManifest(), 'legacy'),
    withReview(unsignedManifest(), 'artifact'), withReview(unsignedManifest(), 'semantic')]) {
    assert.equal(contract.parseDeliveryManifestV3(contract.createDeliveryManifestV3(value)).schemaVersion,
      'delivery-manifest.v3');
  }
  for (const mutate of [
    value => { [value.evidence[1], value.evidence[2]] = [value.evidence[2], value.evidence[1]]; },
    value => { value.evidence[2].producer.attemptId = 'other:attempt'; },
    value => { value.evidence[4] = structuredClone(value.evidence[1]); },
    value => { value.evidence[3] = { ...value.evidence[3], digest: sha256Text('different-implementation') }; },
  ]) {
    const invalid = unsignedManifest();
    mutate(invalid);
    assert.throws(() => contract.createDeliveryManifestV3(invalid), /DELIVERY_MANIFEST_INVALID/);
  }
});

test('module/evidence cardinality boundaries accept 128 and reject 129', () => {
  const maximum = manyModuleManifest(128);
  assert.equal(maximum.evidence.length, 388);
  assert.equal(contract.createDeliveryManifestV3(maximum).modules.length, 128);
  const tooMany = manyModuleManifest(129);
  assert.throws(() => contract.createDeliveryManifestV3(tooMany), /DELIVERY_MANIFEST_INVALID/);
});
...
test('shared reader rejects every independently owned v3 ref/bytes/stage disagreement', () => {
  const manifest = contract.createDeliveryManifestV3(unsignedManifest());
  const bytes = portableJson(manifest);
  const expectedRef = {
    artifactId: `project-summary:${runId}`,
    namespace: 'kubeclaw.project-summary', mediaType: 'application/json',
    encoding: 'kubeclaw-json.utf16.v1', digest: sha256Text(bytes), sizeBytes: Buffer.byteLength(bytes),
    producer: { runId, stageId: 'project-summary', attemptId: 'project-summary:1', attemptNumber: 1 },
  };
  const owner = { runId, manifestStageId: 'project-summary', gateStageId: 'final-test', expectedRef, bytes };
  const cases = [
    { ...owner, runId: 'run:other' },
    { ...owner, manifestStageId: 'other-summary' },
    { ...owner, gateStageId: 'other-test' },
    { ...owner, expectedRef: { ...expectedRef, artifactId: 'project-summary:other' } },
    { ...owner, expectedRef: { ...expectedRef, namespace: 'kubeclaw.other' } },
    { ...owner, expectedRef: { ...expectedRef, digest: sha256Text('other') } },
    { ...owner, expectedRef: { ...expectedRef, sizeBytes: expectedRef.sizeBytes + 1 } },
    { ...owner, bytes: `${bytes}\n` },
  ];
  for (const invalid of cases) {
    assert.throws(() => contract.assertDeliveryManifestForRead(manifest, invalid), /DELIVERY_MANIFEST_INVALID/);
  }
});
