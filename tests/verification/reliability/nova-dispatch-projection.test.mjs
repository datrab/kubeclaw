...
import { graphSnapshot } from '../../../skills/nova/core/execution/engine-snapshots.ts';
...
async function fixture(t, options = {}) {
  ...
  const plan = resolveTestPlan({ planId: 'plan:dispatch-projection', runId, project: 'projection', scope: { moduleId: 'api', gateId: null },
    createdAt: now, registry, suiteTemplates: [], declaration: { tests: { health: { uses: 'kubeclaw.http@1', mode: 'blocking', retries: 0,
      config: { url: origin, path: '/health', expectedStatuses: [options.keepBlocked ? 418 : 200], expectedText: 'original receiver' } } } },
  ...
  if (coreResult.status === 'blocked' && !options.keepBlocked) {
  ...
  if (options.keepBlocked) assert.equal(coreResult.status, 'blocked');
  else assert.ok(['cancelled', 'succeeded', 'failed'].includes(coreResult.status), coreResult.status);
...
test('honestly rehashed sourceStageId or missing-source snapshot cannot bypass explicit-revision-only eligibility', { timeout: 60000 }, async t => {
  const f = await fixture(t), file = path.join(f.run, 'run-snapshot.json'), original = fs.readFileSync(file);
  const before = fs.readFileSync(recordsFile(f.scope.intent.dispatchRoot));
  // Negative snapshot-corruption vectors, NOT a claim that a real derived-source
  // producer completed. Original source-stage artifact linkage remains unsupported.
  for (const derived of [false, true]) {
    const snapshot = JSON.parse(original), definition = structuredClone(f.core.definition);
    delete definition.stages[0].input.providerPlan.revision;
    if (derived) definition.stages[0].input.providerPlan.sourceStageId = 'original-source-producer-not-proved';
    snapshot.graph = graphSnapshot(definition, snapshot.graph.schemaVersion);
    const { digest: _digest, ...unsigned } = snapshot;
    snapshot.digest = `sha256:${crypto.createHash('sha256').update(portableJson(unsigned)).digest('hex')}`;
    fs.writeFileSync(file, JSON.stringify(snapshot));
    await assert.rejects(retireNovaDispatch({ ...f.scope, intent: { ...f.scope.intent, snapshotDigest: snapshot.digest } }),
      /NOVA_DISPATCH_RETENTION_CORE_AUTHORITY_REQUIRED/);
    assert.deepEqual(fs.readFileSync(recordsFile(f.scope.intent.dispatchRoot)), before);
  }
  fs.writeFileSync(file, original);
  assert.equal((await retireNovaDispatch(f.scope)).newlyProjected, true);
