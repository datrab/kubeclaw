import { canonicalJson, sha256Text, type ArtifactRef, type AttemptIdentity,
  type PluginInvocationContext } from '@kubeclaw/plugin-sdk';
...
export class RepositoryAuditArtifactCache implements ReviewCacheStore {
  ...
  async read(cacheKey: `sha256:${string}`): Promise<unknown | undefined> {
    ...
    const raw = await this.#context.invoke('artifacts.read', {
      ...
      payload: { namespace: CACHE_NAMESPACE, digest: artifact.digest },
    });
    const response = responseRecord(raw), serialized = canonicalJson(response.value);
