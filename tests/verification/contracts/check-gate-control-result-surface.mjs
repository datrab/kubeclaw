...
const gateRuntimeErrorPath = path.join(sourceRoot, 'skills/nova/pipeline/runners/gate-runtime-error.ts');
const gateControlAdapterPath = path.join(sourceRoot, 'skills/nova/pipeline/runners/gate-control-adapter.ts');
const gateRunInputPath = path.join(sourceRoot, 'skills/nova/pipeline/runners/gate-run-input.ts');
...
const gateRuntimeErrorSource = fs.readFileSync(gateRuntimeErrorPath, 'utf8');
const gateControlAdapterSource = fs.readFileSync(gateControlAdapterPath, 'utf8');
const gateRunInputSource = fs.readFileSync(gateRunInputPath, 'utf8');
...
assert.equal(gateRunnerSource.includes("from './gate-runtime-error.ts'"), true, 'gate-runner should delegate runtime errors to one focused helper');
assert.equal(gateRuntimeErrorSource.includes('export async function buildGateRuntimeErrorControl('), true, 'gate runtime helper should own one runtime-error control builder');
assert.equal(gateRuntimeErrorSource.includes('function isTimeoutError('), true, 'gate runtime helper should classify timeout-shaped runtime errors before terminal projection');
assert.equal(gateRuntimeErrorSource.includes("terminalReasonCode: timedOut ? 'timeout'"), true, 'gate runtime timeout failures must use the canonical timeout terminal reason');
...
assert.equal(gateControlAdapterSource.includes('normalizeRemediableTypedGateControlResult('), true, 'gate control adapter should normalize remediable gate controls through the shared helper');
assert.equal(gateControlAdapterSource.includes('normalizeTypedGateControlResult('), true, 'gate control adapter should normalize waitable gate controls through the shared helper');
...
assert.equal(gateRunInputSource.includes('gate run input requires explicit stageId'), true, 'gate input builder should reject missing scheduled gate stage ids');
...
assert.equal(gateRunInputSource.includes("priorResults: artifacts.filter((artifact: any) => artifact.type === 'gate_output')"), true, 'gate priorResults should only expose canonical gate output evidence');
...
assert.equal(gateControlAdapterSource.includes('must declare gateControl.allowedNextActions'), true, 'gate control adapter must reject non-remediable adapters without allowed actions');
