...
const ledger = JSON.parse(fs.readFileSync('docs/architecture/pipeline-test-gate-api-parity-ledger.json', 'utf8'));
...
assert.equal(ledger.authority.old, 'deleted');
assert.equal(ledger.authority.replacement, 'authoritative');
console.log(JSON.stringify({ ok: true, phase: 'api-parity', items: ledger.expectedItemCount, deferred: 0 }));
