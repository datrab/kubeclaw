...
try {
  for (const role of ['nova', 'buster', 'prism']) {
    ...
    if (role === 'prism') {
      assert.equal(fs.existsSync(path.join(output, 'skills/packages/prism-contract/schemas/prism-v1.schema.json')), true,
        'Prism bundle must contain its canonical schema');
      assert.equal(fs.existsSync(path.join(output, 'skills/packages/prism-contract/fixtures/minimal-web.json')), true,
        'Prism bundle must contain its minimal fixture');
    }
  }
} finally {
...
console.log(JSON.stringify({ ok: true, phase: '5.6-D', roles: 3 }));
