...
const reviewFixCyclePath = path.join(sourceRoot, 'skills/nova/pipeline/runners/review-gate-fix-cycle.ts');
...
const reviewFixCycleSource = fs.readFileSync(reviewFixCyclePath, 'utf8');
...
for (const source of [reviewFixCycleSource, busterFixCycleSource]) {
...
for (const source of [sharedForgeFixCycleSource, reviewFixCycleSource, busterFixCycleSource]) {
...
assert.equal(reviewSource.includes("from './review-gate-fix-cycle.ts'"), true, 'review gate runner should delegate fix-cycle effects to the extracted helper');
...
assert.equal(reviewFixCycleSource.includes("from './gate-forge-fix-cycle.ts'"), true, 'review fix-cycle adapter should delegate shared mechanics to the generic Forge fix-cycle engine');
