...
assert.match(novaValues, /test\.plan\.execute:[\s\S]*adapter: buster-plan-v1[\s\S]*scheme: https[\s\S]*port: 18891/u);
...
assert.match(secretSetup, /subjectAltName=DNS:agent-buster/u);
assert.match(busterValues, /buster-plan-tls[\s\S]*pipeline-test-gate-source-attestation/u);
...
assert.match(novaValues, /NODE_EXTRA_CA_CERTS[\s\S]*buster-plan-trust/u);
