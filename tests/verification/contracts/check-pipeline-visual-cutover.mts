...
for(const check of inventory.requiredAbsence) if(fs.existsSync(check.file)) assert.equal(fs.readFileSync(check.file,'utf8').includes(check.token),false,`legacy token remains: ${check.file}`);
