...
const moduleCompletionAppender = read(sourceRoot, 'skills/nova/pipeline/services/status-store-lifecycle/appenders-module-completion.ts');
const gateAppender = read(sourceRoot, 'skills/nova/pipeline/services/status-store-lifecycle/appenders-gates.ts');
const projections = read(sourceRoot, 'skills/nova/pipeline/services/status-store-lifecycle/projections.ts');
const gateProjection = read(sourceRoot, 'skills/nova/pipeline/services/status-store-lifecycle/projection-gate.ts');
...
assert.equal(appenders.includes('export * from "./appenders-module-completion.ts"'), true, 'lifecycle appenders facade must export module completion ownership');
assert.equal(appenders.includes('export * from "./appenders-gates.ts"'), true, 'lifecycle appenders facade must export gate completion ownership');
assert.equal(moduleCompletionAppender.includes('export function applyModuleCompletion('), true, 'module completion appender must own applyModuleCompletion');
assert.equal(gateAppender.includes('export function applyGateCompletion('), true, 'gate appender must own applyGateCompletion');
assert.equal(moduleCompletionAppender.includes('appendModuleLifecycleEvent(config, dir, status, lifecycleMutation)'), true, 'applyModuleCompletion must append through lifecycle spine');
assert.equal(gateAppender.includes('appendLifecycleEvent(config, {'), true, 'applyGateCompletion must append through lifecycle spine');
assert.equal(gateAppender.includes('completion: cloneSerializable(completion)'), true, 'lifecycle events must retain completion evidence');
assert.equal(projections.includes('applyGateCompletionEventToReadModels(next, event)'), true, 'lifecycle projections must delegate gate completions');
assert.equal(gateProjection.includes('function applyGateCompletionEventToReadModels('), true, 'gate projection must own gate completion read models');
assert.equal(gateProjection.includes('gate_evaluation.passed'), true, 'gate PASS completion must be a lifecycle event');
assert.equal(idempotency.includes('"gate_evaluation.passed":'), true, 'gate PASS completion must have lifecycle idempotency authority');
assert.equal(idempotency.includes('"gate_evaluation.failed":'), true, 'gate FAIL completion must have lifecycle idempotency authority');
assert.equal(idempotency.includes('"gate_evaluation.blocked":'), true, 'gate BLOCKED completion must have lifecycle idempotency authority');
