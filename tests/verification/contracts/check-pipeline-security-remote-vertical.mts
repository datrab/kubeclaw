...
try{
  ...
  const records={maximumRecords:100,maximumBytes:134217728,maximumRecordBytes:67108864};const cacheCandidates=[...(process.env.KUBECLAW_SECURITY_TEST_CACHE?[process.env.KUBECLAW_SECURITY_TEST_CACHE]:[]),path.join(os.homedir(),'.cache/trivy'),path.join(os.homedir(),'.cache/.cache/trivy'),'/tmp/.cache/trivy'];const cache=cacheCandidates.find((item)=>fs.existsSync(item));assert.ok(cache,'real Trivy database is required');
  ...
  try{const gate=createProductionNovaTestGate({stateRoot:path.join(root,'nova-state'),endpoint:`http://127.0.0.1:${address.port}`,token,sourceAuthority:'nova:production',sourceAttestationPrivateKey:privateKey,pollMilliseconds:10,maximumResponseBytes:65536,maximumResultBytes:67108864,maximumArchiveBytes:8388608,maximumArchiveStoreBytes:33554432,maximumEvidenceBytes:67108864,maximumEvidenceStoreBytes:134217728,recordLimits:records});
    const executed=await gate.execute({idempotencyKey:'security:remote:vertical',pipelineStageId:'stage:security',plan,repositoryRoot:repository,repositoryId:'repository:security',maximumConcurrency:1,grants:new Map([['dependency',['security.scan']]]),submittedAt:'2026-09-04T00:00:00.000Z',timeoutMs:360000});assert.equal(executed.remote.status.state,'completed');
    const reference=executed.remote.status.result!;const result=JSON.parse((await service.result(executed.remote.status.jobId,reference.contentDigest,reference.sizeBytes)).toString('utf8'));assert.equal(executed.remote.decision.state,'passed',JSON.stringify(result.attempts.map((attempt: { outcome: string; error?: unknown; summary?: unknown }) => ({outcome:attempt.outcome,error:attempt.error,summary:attempt.summary}))));assert.equal(result.attempts[0].providerDetails.values.scanner,'dependency-trivy');assert.equal(result.attempts[0].providerDetails.values.databaseEvidence.databases.length,2);const imports=JSON.parse(fs.readFileSync(path.join(root,'nova-state/imports/records/store.json'),'utf8')).records;assert.equal(imports.some((record:any)=>record.payload?.decision?.decisionDigest===executed.remote.decision.decisionDigest),true);
