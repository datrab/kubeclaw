...
test('original Application health Lua rejects stale sync, failed hooks, drift and unavailable Git', () => {
  ...
  const branch = object => { object.spec.source.targetRevision = 'main'; object.status.sync.comparedTo.source.targetRevision = 'main'; };
  ...
  const output = execFileSync(process.env.LUA_BIN ?? 'lua', [...JSON.parse(process.env.LUA_ARGS ?? '[]'), '-', path.resolve('charts/gitops/files/application-health.lua')], { input: script, encoding: 'utf8' });
