...
assertIncludes(deployScript, 'resolve_deploy_targets() {', 'Deploy script must support nova|buster|both target resolution');
...
assertIncludes(deployScript, 'code deploy requires $(tr', 'Deploy script must still fail closed when required code bundle inputs cannot be resolved');
...
assertIncludes(deployScript, 'restart_agent_deployment() {', 'Image deploy must define an explicit rollout restart helper');
assertIncludes(deployScript, 'kubectl rollout restart deployment/"$release" -n "$NAMESPACE"', 'Image deploy must use rollout restart instead of pod deletion to refresh mutable tags');
...
assertIncludes(deployScript, 'restart_agent_deployment "agent-${role}"', 'Image deploy must restart the selected agent deployment after Helm apply');
