...
assertIncludes(deploymentTemplate, 'export GIT_CONFIG_VALUE_0="$REPO_DIR"', 'Deployment init must scope safe-directory trust to the persisted checkout');
assertOrdered(deploymentTemplate, 'export GIT_CONFIG_VALUE_0="$REPO_DIR"', 'git diff --quiet && git diff --cached --quiet', 'Deployment init must trust the persisted checkout before inspecting it');
