...
assertIncludes(rendered, 'openclaw.json rendered into runtime config', 'Rendered init container must stage OpenClaw config into runtime storage');
...
assertIncludes(rendered, 'HOME="$DOCTOR_HOME"', 'Rendered startup doctor must stage a writable HOME before running doctor');
assertIncludes(rendered, 'doctor --fix --non-interactive', 'Rendered startup doctor must run OpenClaw doctor with fixes enabled');
assertIncludes(rendered, 'synced repaired openclaw.json back to persistent and runtime config', 'Rendered startup doctor must sync repaired config back after doctor succeeds');
...
assertIncludes(renderedBuster, 'mountPath: /home/node/.openclaw/openclaw.json', 'Buster containers must mount the runtime OpenClaw config over the retained home');
...
assertIncludes(deploymentTemplate, 'openclaw.json rendered into runtime config', 'Deployment template must render openclaw.json into the runtime config surface');
assertIncludes(deploymentTemplate, 'HOME="$DOCTOR_HOME"', 'Deployment template startup doctor must stage a writable HOME before running doctor');
assertIncludes(deploymentTemplate, 'synced repaired openclaw.json back to persistent and runtime config', 'Deployment template startup doctor must sync repaired config back after doctor succeeds');
