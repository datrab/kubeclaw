...
async function proof(pool: Pool) {
  ...
  return { models, keys, columns, indexes };
}

test('native PG18 full LiteLLM schema/auth/model backup restores on a separate server and rejects unsafe recovery', { timeout: 300000 }, async t => {
  ...
  try {
    ...
    t.diagnostic(`Real PostgreSQL18.6; ${before.columns.length} schema columns; ${before.indexes.length} restored indexes; original key/model JSON, SQL authentication and original LiteLLM legacy/AES-GCM decryption verified`);
