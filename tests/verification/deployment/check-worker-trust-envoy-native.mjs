...
function certificate(name, uri, days) {
  ...
  run('openssl', ['x509', '-req', '-in', `${name}.csr`, '-CA', 'ca.crt', '-CAkey', 'ca.key', '-CAcreateserial', '-out', `${name}.crt`, '-days', String(days), '-extfile', `${name}.ext`]);
}
...
try {
  ...
  certificate('valid', identity, 1); certificate('wrong', identity + '-foreign', 1); certificate('expired', identity, -1);
