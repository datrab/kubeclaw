...
function withBusterPayloads({ timeoutMinutes, discordWebhookUrl }, verify) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'buster-payload-'));
  try {
    const config = {
      project: 'payload-test',
      repo_root: root,
      run_id: 'run-payload-test',
      discord_webhook_url: discordWebhookUrl,
      paths: { swarm_dir: path.join(root, '.swarm'), modules_dir: path.join(root, '.swarm', 'modules') },
      buster: { runtime: { suite_timeout_ms: 120000, max_crash_retries: 1 } },
    ...
    const progress = {
      modules: { '01-nginx': { dir: '01-nginx', ...task } },
      gates: { 'final-buster': { id: 'final-buster', title: 'Final Buster', ...task } },
    };
    ...
    verify({ config, gatePayload, modulePayload });
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
}
...
test('buildBusterPayload carries pipeline Discord webhook authority for module and gate tasks', () => {
  withBusterPayloads({ timeoutMinutes: 5, discordWebhookUrl: 'https://discord.example/webhook?wait=true' }, ({ config, gatePayload, modulePayload }) => {
...
test('buildBusterPayload rounds tiny positive timeout minutes up to valid task timeout seconds', () => {
  withBusterPayloads({ timeoutMinutes: 0.001 }, ({ gatePayload, modulePayload }) => {
