...
export async function retireAttemptResult(input) {
  ...
  return withRunMutationLock(canonical,async signal=>{
    ...
    let initial;
    return attempts.retireImportedResult(scope.intent,operation=>new FileMutex(`${journal}.append-lock`,5000,'ATTEMPT_RETIREMENT_JOURNAL_LOCKED').withAsyncLock(async()=>{
      signal.throwIfAborted();initial=retainedRun(scope);return operation();
    }),async()=>{
