...
file(cilium, '19461be990b66c6484150f9f3dbd96140cd063f63b37b351d464510e98cff5e6', charts.cilium);
...
special(codexOpsValues, '$.networkPolicy.cilium', { purpose: 'Supplies the child switch for the additional Cilium kube-apiserver entity policy; the Argo bootstrap generator forwards it unchanged to the Ops chart.', acceptedValues: 'Use an unquoted YAML Boolean. With `networkPolicy.enabled=true`, `true` renders the portable NetworkPolicy and CiliumNetworkPolicy, while `false` renders only the portable policy. With the parent false, neither policy renders regardless of this value.', emptyBehavior: 'An empty scalar is not a Boolean. Removing this overlay value selects the Ops chart default `false` during Helm merge.', impact: 'Changes only the Cilium-specific policy when the parent policy remains enabled; disabling the parent removes all policy isolation.', failure: 'With both switches true on a cluster without the Cilium CRD, admission or synchronization rejects the extra object. With the parent false, this child cannot preserve API-server policy isolation.' });
...
file(registryLocalManifest, '966ab5af3851d5a6f9305d8819574301ca78d6c20f6d19074d078bf3b8e28d53');
...
file(registryMirrorManifest, '383c470fb991a43aa9dfc1634d2399c53a1e77c19f37d5bb4b8bc0d26ef207f5');
