...
const mutationKinds = [
  ...
  'plugin-registration', 'schema', 'store', 'workflow',
];
...
function fixture() {
  ...
  assert.deepEqual(value.readerTasks.map((item) => item.persona).sort(),
    ['operator', 'operator', 'pipeline-author', 'platform-developer', 'plugin-developer', 'prism-buster-specialist']);
...
function evidence(file, rows, catalog, fixtures) {
  ...
  assert(Array.isArray(value.participants) && value.participants.length >= fixtures.readerTasks.length,
  ...
  for (const participant of value.participants) {
    assert(fixtures.readerTasks.some((item) => item.persona === participant.role),
  ...
  for (const attempt of value.reader_attempts) {
    assert(fixtures.readerTasks.some((item) => item.id === attempt.fixture_id),
  ...
  const allFixtures = [...fixtures.searchQueries, ...fixtures.renderScenarios, ...fixtures.readerTasks];
  ...
  for (const result of value.fixture_results) {
    ...
    const reader = fixtures.readerTasks.find((item) => item.id === result.fixture_id);
    if (reader) {
  ...
  const readerResults = value.fixture_results.filter((result) =>
    fixtures.readerTasks.some((task) => task.id === result.fixture_id));
  unique(readerResults.map((result) => result.participant_id),
    'accepted reader-task participant identities');
  const mutationIds = fixtures.mutationTests.flatMap((item) =>
    item.operations.map((operation) => `${item.kind}:${operation}`));
  ...
  for (const mutation of value.mutations) {
    ...
    assert.equal(mutation.mutation_id, `${mutation.kind}:${mutation.operation}`);
    ...
    text(mutation.changed_surface, `${mutation.kind}.changed_surface`, 20);
...
process.stdout.write(`${JSON.stringify({
  ...
  fixtures: fixtures.searchQueries.length + fixtures.renderScenarios.length + fixtures.readerTasks.length
    + fixtures.mutationTests.reduce((sum, item) => sum + item.operations.length, 0),
