...
function unapprovedContractProposal(field) {
  ...
  if (last === 'create') return set('service-account-creation', 'Controls whether the chart creates and selects a release-specific ServiceAccount for the workload.', '`true` creates and selects the release-specific ServiceAccount. `false` creates no ServiceAccount, omits `serviceAccountName`, uses the namespace default ServiceAccount, and forces automatic token mounting off.', 'Removing the override uses the selected values file or chart default. An explicit empty or null value follows the false Helm branch.', 'Changes the workload identity from the release-specific ServiceAccount and its chart-managed RBAC to the namespace default ServiceAccount without an automatically mounted API token.', 'A non-Boolean value follows Helm receiver behavior. With `false`, an ordinary workload can start under the namespace default ServiceAccount, but the chart omits lease-client and verification RBAC. Helm stops rendering if SPIFFE worker trust or the Nova readiness client is enabled because those paths require the release-specific ServiceAccount.');
  ...
  if (last === 'enabled') return set('feature-enable-control', `Proposed control for ${parent || chartRootFor(field.sourcePath)}; replace this draft with the exact template and runtime behavior before approval.`, '`true` and `false` are the intended Boolean inputs, but the exact receiver can use a condition, a strict function, or a runtime string and must be described explicitly.', emptyByUse, `The exact impact must name the resources, rendered values, or runtime checks changed by the linked ${target} receivers.`, 'Do not approve this proposal until each receiver gate, parent condition, false branch, and operational failure is proved from source.');
