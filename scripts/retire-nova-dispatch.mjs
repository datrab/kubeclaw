...
import { coreDispatchRoots, assertCoreDispatch } from './observability-retirement/dispatch.mjs';
...
function scopeValue(input) {
  ...
  if (!shape(scope, 'action,dispatchOptions,importOptions,intent,inventoryLimits,novaStorageRoot,orchestratorIssuerId,schemaVersion,stateRoot')
...
function retainedRun(scope) {
  ...
  coreDispatchRoots(inventory, scope);
  ...
  return withRunMutationLock(canonical, async signal => {
    ...
    return imports.withRetainedImport(job, scope.intent.importPayloadDigest, authority =>
      dispatch.compactImportedArchive(scope.intent, authority, () => {
        ...
        assertCoreDispatch(current, scope, job, authority.decision);
