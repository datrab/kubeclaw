...
const slug = value => value.toLocaleLowerCase('en-US')
  .replace(/[^a-z0-9]+/gu, '-')
  .replace(/^-|-$/gu, '');
...
export function renderStatus(data) {
  ...
  const anchor = id => slug(issueById.get(id).title);
  const lines = [
    ...
    `Last verified: ${data.updated_at}; each issue states its available evidence and remaining validation`, '',
    ...
    '## Issue index', '', '| Issue | Status |', '| --- | --- |',
    ...data.issues.map(issue => `| [${cell(issue.title)}](#${anchor(issue.id)}) | ${cell(issue.status)} |`), '',
  ];
  for (const issue of data.issues) {
    ...
    lines.push(`## ${issue.title}`, '',
      ...
      '### Dependencies', '', issue.dependencies.length ? list(issue.dependencies.map(id => `[${issueById.get(id).title}](#${anchor(id)})`)) : 'No dependency on another entry in this register is established.', '',
      '### Current evidence boundary', '', issue.evidence.assessment, '',
      ...Object.entries(issue.evidence).filter(([key]) => ['prior_verification', 'implementation_commit', 'source_run_commit'].includes(key)).map(([key, value]) => `- **${key === 'prior_verification' ? 'Observed behavior' : key.replaceAll('_', ' ')}:** ${value === null ? 'Not established.' : typeof value === 'object' ? JSON.stringify(value) : value}`), '',
