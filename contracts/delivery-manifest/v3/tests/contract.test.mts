...
test('closed schema and runtime both reject non-JSON and overlong evidence media types', () => {
  ...
  for (const mutate of [
    (value: any) => { value.final.reviewArtifactEncoding = DELIVERY_MANIFEST_ARTIFACT_ENCODING; },
    (value: any) => { value.final.reviewSemanticEncoding = 'review-semantics.utf16-v1'; },
    (value: any) => { value.evidence[0].producer.attemptNumber = Number.MAX_SAFE_INTEGER + 1; },
  ]) {
    const brokenUnsigned = structuredClone(unsigned()) as any;
    mutate(brokenUnsigned);
    assert.throws(() => createDeliveryManifestV3(brokenUnsigned), /DELIVERY_MANIFEST_INVALID/);
    const brokenManifest = structuredClone(valid) as any;
    mutate(brokenManifest);
    assert.equal(validate(brokenManifest), false);
  }
});

test('creator rejects when adding the digest makes the complete artifact exceed 8 MiB', () => {
  const maximum = 8 * 1024 * 1024;
  const edge = unsigned() as any;
  const fullPath = (index: number) => `${String(index).padStart(4, '0')}/${'x'.repeat(4091)}`;
  const baselineBytes = Buffer.byteLength(portableJson(edge));
  const fullCount = Math.floor((maximum - baselineBytes - 200) / (4 * 4099)) - 1;
  const paths = Array.from({ length: fullCount }, (_, index) => fullPath(index));
  paths.push('adjust-a', 'adjust-b');
  edge.modules[0].expectedCoverage.modules[0].ownedPaths = paths;
  edge.final.expectedCoverage.modules[0].ownedPaths = paths;
  const target = maximum - 20;
  const shortBytes = Buffer.byteLength(portableJson(edge));
  let extraCharacters = Math.floor((target - shortBytes) / 4);
  for (const index of [paths.length - 2, paths.length - 1]) {
    const count = Math.min(4087, Math.max(0, extraCharacters));
    paths[index] += 'x'.repeat(count);
    extraCharacters -= count;
  }
  for (const binding of [edge.modules[0], edge.final]) {
    const coverage = binding.expectedCoverage;
    const { policyDigest: _policyDigest, ...unsignedCoverage } = coverage;
    coverage.policyDigest = gateCoverageDigest(unsignedCoverage);
    const { coverageDigest: _coverageDigest, ...unsignedResult } = binding.coverage;
    binding.coverage.coverageDigest = remotePlanDigest(unsignedResult);
  }
  const unsignedBytes = Buffer.byteLength(portableJson(edge));
  assert.ok(unsignedBytes <= maximum && unsignedBytes > maximum - 100);
  const completeBytes = Buffer.byteLength(portableJson({ ...edge, digest: hash('0') }));
  assert.ok(completeBytes > maximum);
  assert.throws(() => createDeliveryManifestV3(edge), /DELIVERY_MANIFEST_INVALID/);
});
