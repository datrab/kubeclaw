...
// ═══════════════════════════════════════════════════════════════
// PIPELINE TASK HANDLER (module_test / gate_test)
// ═══════════════════════════════════════════════════════════════
//
...
function buildPipelinePrompt(payload, memories) {
  ...
  const isGateTest   = payload.task_type === 'gate_test';
  ...
  // ── Header ──
  const header = [
    `🔬 **PIPELINE ${isModuleTest ? 'MODULE TEST' : 'GATE TEST'}**`,
    '',
  ...
  if (isModuleTest) {
    ...
    // Gate test (cross-module / final system test)
    const gateConfig = payload.gate_config || {};
    testSection = [
      '',
      '## 2. GATE TESTING INSTRUCTIONS',
      '',
      `**Gate:** ${payload.gate_id || payload.module}`,
      `**Title:** ${payload.gate_title || 'System Gate Test'}`,
      `**Time Limit:** ${gateConfig.time_limit_minutes || 30} minutes`,
      '',
      payload.instructions || '(No instructions provided — read the instructions_file from the repo)',
    ];
  }
  ...
  if (isModuleTest) {
    ...
    // Gate test — writes to output_file, signals via redis.js complete
    completionProtocol = [
      '',
      '## 3. WHEN TESTING IS COMPLETE',
      '',
      ...
      '',
      '**a)** Write results to the output file specified in the instructions above.',
      '   Include at minimum a `"status"` field (`"PASS"` or `"FAIL"`),',
      '   a `"summary"` with a brief overview, and a `"findings"` array with details.',
      '',
      '**b)** Store 1-3 key technical insights:',
      '   ```bash',
      `   node /app/skills/memory.js remember \\`,
      `     --text "Concise technical insight" \\`,
      `     --tags "buster,${payload.gate_id || payload.module},testing" \\`,
      `     --scope global \\`,
      `     --module ${payload.gate_id || payload.module}`,
      '   ```',
      '',
      '**c)** Signal completion (your LAST action):',
      '   ```bash',
      ...
      '   ```',
      '',
      '⚠️ **CRITICAL:** Do NOT skip the redis.js complete step. Without it, the pipeline cannot continue.',
    ];
  }
