...
{{/*
Resolve Mission Control auth password.
*/}}
{{- define "kubeclaw.mcAuthPass" -}}
{{- if .Values.missionControl.auth.existingSecret -}}
valueFrom:
  secretKeyRef:
    name: {{ .Values.missionControl.auth.existingSecret }}
    key: {{ .Values.missionControl.auth.existingSecretPassKey | default "mcPass" }}
{{- else -}}
value: {{ .Values.missionControl.auth.pass | quote }}
{{- end -}}
{{- end -}}

{{/*
Resolve Mission Control auth user.
*/}}
{{- define "kubeclaw.mcAuthUser" -}}
{{- if .Values.missionControl.auth.existingSecret -}}
valueFrom:
  secretKeyRef:
    name: {{ .Values.missionControl.auth.existingSecret }}
    key: {{ .Values.missionControl.auth.existingSecretUserKey | default "mcUser" }}
{{- else -}}
value: {{ .Values.missionControl.auth.user | quote }}
{{- end -}}
{{- end -}}
