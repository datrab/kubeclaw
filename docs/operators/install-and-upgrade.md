...
The answer is stored in `my-values/.workspace-namespace` for later setup/secrets runs. Use `KUBECLAW_WORKSPACE_PROMPT=false` to suppress the prompt or `KUBECLAW_WORKSPACE_PROMPT=true` to require it.
