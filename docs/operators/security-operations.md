...
| Buster rootless builder | `my-values/buster-values.yaml`; `charts/kubeclaw/templates/deployment.yaml`; `docker/Dockerfile.buster-pipeline` | Buster pod render/start | gateway and worker are non-privileged; only the worker mounts BuildKit state/results and its worker probe succeeds | BuildKit worker probe, registry push, lease, or suite startup failures |
