...
Session-backed agent observability is plugin-owned. Nova may request and target child sessions with `agent.spawn.requested` and `agent.delivery.target`, but runtime start/end/progress/transcript truth is promoted from the OpenClaw observer plugin through the agent-observability ingester. `agent.spawned` remains the canonical started event name; plugin-owned task lifecycle details use `plugin.event` with `plugin_id: "buster"`.
...
`agent.killed` is retained as a compatibility schema for older consumers, but new session termination truth comes from observer-plugin terminal events such as `agent.ended` and `agent.session.ended`. Plugin-owned task completion details use `plugin.event` with `plugin_event: "task_completed"`.
...
When legacy `agent.killed` records are encountered, they preserve the same top-level correlation fields used on `agent.spawned`, especially `session_key`.

When a spawned or terminated session belongs to gate-owned work and Nova supplied that identity to the observer path, observer-promoted lifecycle events preserve canonical `gate_type` and `dispatch_id` join keys alongside `gate_id`, `attempt`, and `session_key`.
...
Live transcript line from an ACP agent session. This event is observer-plugin-owned; Nova's HTTP/API polling path must not publish it.
...
Periodic summary of agent activity. This event is observer-plugin-owned; Nova's HTTP/API polling path must not publish it.
...
| telemetry_stream | string\|null | Canonical telemetry stream identity when the degraded surface is stream-specific |
...
| agent_type | string\|null | Agent or phase context when known |
| telemetry_stream | string\|null | Canonical telemetry stream identity when the recovered surface is stream-specific |
...
Buster is an external telemetry producer because it runs in a separate pod. It publishes plugin-owned details through `plugin.event` directly to the canonical run stream. Agent status, transcript, progress, model, tool, and LLM observability is produced by the OpenClaw observer plugin and promoted by the agent-observability ingester into canonical agent event names. Nova-owned Redis and Discord delivery for Nova events is handled by registry-owned telemetry sink plugins. Plugin events must keep the canonical envelope fields (`v`, `type`, `ts`, `project`, `run_id`, `source`, `emitter`) and include the relevant module/gate/session identity fields.
...
| Active Agent panel | observer-promoted agent.spawned, agent.session.started, agent.progress |
| Agent Stream (terminal) | observer-promoted agent.transcript |
