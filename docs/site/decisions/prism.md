...
Status: current decisions with bounded approval and proof
Audience: architecture readers, developers, and operators
Owner: Prism maintainers
Evidence: skills/prism/domain/index.ts; skills/prism/corpus/index.ts; spikes/prism/postgres-retrieval/verify.mjs
Applies to: source revision `ad67f9bb5c75cfa8cc1b926668aec1dd0168452c`
Last verified: 2026-09-17; no new live environment result is available

## Decision Group Accountability

Prism's source decisions describe connected product areas rather than a numbered
ADR for every sentence. Each row below states the shared record fields for one
group. The detailed sections retain the exact constraints and measurable limits.
...
| --- | --- | --- | --- | --- | --- | --- |
| Product and data ownership | Nova owns product architecture. Prism owns design exploration, revisions, approval evidence, and publication inside that boundary. | A second orchestrator and one undifferentiated data store are rejected because they obscure authority and recovery. | Design Document, PostgreSQL, and immutable artifacts have separate recovery and retention duties. | The Prism architecture decision marks the boundary accepted. It does not record one approver and date for every clause. | The linked domain and storage implementation establishes the current code boundary. Deployment and human product acceptance remain separate. | Later native-host and recovery decisions refine deployment and storage details without moving product authority. |
...
The Prism architecture decision marks these boundaries accepted; an individual approver and date are not recorded for every clause.
Its diagrams and provider names describe design intent, not installed services.
The three storage responsibilities are separate. The Design Document owns editable design. PostgreSQL owns durable relational state. The platform artifact store owns large immutable objects.
...
Local PGlite evidence supports development only. It does not establish native-server or physical-device acceptance.
...
Record both limits. [Prism service acceptance](../status/acceptance.md#prism-database-jobs-sessions-and-resources) must report results against both and must not call 300 ms an approved replacement without decision evidence.
...
The spike criteria
and implementation-plan criteria
...
[The no-decay preference conflict](../status/open-issues.md#accepted-no-decay-preference-contract-conflicts-with-automatic-180-day-decay) preserves that unresolved product boundary.
...
Product coverage includes state, editor, retrieval, preferences, providers, workers, integration, access, publication, recovery, observability, and acceptance.
Completion in one area does not certify every deployed path.
