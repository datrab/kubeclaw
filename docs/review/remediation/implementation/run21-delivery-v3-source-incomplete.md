...
- the generated Knip configuration is refreshed from this exact tree and its
  config check passes with 50 plugins.
...
deployment execution. A separate phase removes the import records and preserves
the exact honest `NOVA_VERIFIED_OUTPUT_IMPORT_REQUIRED` failure after manifest
integrity. The same original adapter now rejects the full bounded outer-ref
matrix (digest/tag presence and equality, size, ID, namespace, run and stage)
and stored-body matrix (inner digest, unknown key, complete-size, gate,
coverage, source, Review and competing-latest) before an import is accepted.
The archived historical CLI diagnostic also runs its native historical source
probe before its own independent legacy-import fixture writes `progress.json`;
its dedicated reproducibility regression passes without `REVIEW_SOURCE_DIRTY`.
Twelve separate-process real-disk histories now cover v2/v3 registered Summary
writes and original downstream reads at requested, accepted and completed
prefixes. Actual SIGKILL is observed in every initial process; reconstruction
retains exact prefix bytes/request, invokes requested-only exactly once, leaves
accepted-without-receipt uncertain without replacement, and replays completed
without invocation. V2 never gains the encoding and v3 never loses it.
The
...
Still open are independent confirmation of the native durability matrix,
cancellation at the original write/read boundaries, the remaining consumer
media/ambiguity/import-store negatives, and remaining
...
Knip config gate passes, while the full no-install Knip gate lacks its local
knip@6.35.1 dependency. The migration package-boundary gate also exposes the
pre-existing shared telemetry-store test import of Nova locks, outside this
Delivery delta. The
