...
Follow-up checkpoint: the three genuine historical/current SIGKILL fixture cases
pass (current cross-locale 1 POST; original-to-current 1 POST; original-to-original
cross-locale countercheck 2 POSTs). Wire JSON insertion order is asserted. Focused
lint and Nova typecheck pass. Independent negative proof is in progress separately.
Root rejected the private producer-shape validator as an integration replacement
for the canonical schema; this checkpoint remains unapproved. Required next work
is the owning shared Resource/EffectRequest/Receipt contract fix, narrowly bounded
historical key grammar and bounded versioned new key, then canonical validation
and regeneration parity. No finding status is changed by these partial passes.

Next action: obtain Root approval of that canonical contract proposal and implement;
