...
## Contract evidence for the next design decision

Freshly read remote authority includes current
`skills/nova/core/execution/CAPABILITIES.md` and
`docs/architecture/plugin-system-phase5-capabilities.json`, plus original
`a9e080ab1e1981ec5713e9b742f94280835fd347` reviews
`docs/review/components/lib.sdk.md` and
`docs/review/components/kubeclaw.runtime-dispatch.md`.

CAPABILITIES explicitly permits trusted adapter registrations to use
invokeConfidential when a granted downstream operation carries credentials;
the policy does not restrict that method to a fixed capability subset. The
vocabulary maps runtime.dispatch to its provider but contains no durable-only
mode. The original SDK review expressly describes Fence **or** confidential
without Fence. Existing executable capability-runtime contract assertions
require confidential values/resources to stay out of the journal and audit.
Consequently routing confidential dispatch into the ordinary durable journal
would change a documented confidentiality promise, and rejecting it outright
would narrow the public adapter invocation contract. Neither change follows
from the original serializer finding or session-cleanup requirement.

Original runtime review also explicitly states there is no adapter receipt or
persisted spawn/cancel ledger, remote deduplication is an external prerequisite,
and no complete crash/exactly-once guarantee is asserted. The current
AdapterInvocationOwner carries attempt, execution key, signal and phase, not a
persisted transport version. Confidential EffectRequest receives a random
ephemeral effectId. Neither it nor the mere presence of an idempotency key proves
that no historical session exists. DispatchInvocation currently drops the
confidential discriminator before dispatchOpenClaw, although the owning factory
could propagate it following a reviewed contract change.

Concrete options for root design approval, not implementation authorization:

1. Limit a new explicitly versioned transport admission to genuinely new
   durable admissions; preserve declared legacy/non-durable behavior. This is
   bounded but **partial** unless the remaining open-payload domains have their
   own accepted cutover policy. Do not present it as SDK-wide closure.
2. Define a real versioned transport ownership/admission contract covering
   confidential/direct callers too. It must preserve confidentiality, record
   the original selected transport identity before external action, and reject
   unresolved legacy ownership. Historical session absence cannot be inferred
   from a guessed locale, a new random effectId or a missing new-format record.
   This requires explicit design authority rather than inventing an ephemeral
   migration marker or a synthetic Gateway receipt.
3. Intentionally narrow confidential runtime.dispatch as an explicit reviewed
   API/security-contract change. Existing requirements do not justify doing so
   silently. This audit does not recommend removing the mode merely because
   current built-ins do not use it.

No option has been approved here. The next owner should select the smallest
policy that actually preserves the existing accepted contracts, then run
original local producer/lookup/unknown-version/confidential-redaction and
historical Core/CLI regressions. A live Gateway prerequisite is not invented
for this SDK finding; claims about accepted remote sessions would need their
own actual authority and must remain separate from the demonstrated 503 probe.
