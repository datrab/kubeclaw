...
Independent review correction: the first `expired` vector used a 59-minute
validity interval and therefore reached `DEMO_PRODUCT_TIME_INVALID`, not the
named expiry check. The strengthened exact-code assertions reproduce that
failure in `run5-native-expiry-before/native-controller.txt`. Its issued time is
now two minutes ago and expiry one minute ago: a valid one-minute interval that
must reject specifically `DEMO_PRODUCT_DECISION_EXPIRED`. All other negative
cases now assert exact rejection codes too. The native controller's outbound
Kubernetes client uses the private privileged test identity, not the chart's
controller-SA permissions; the separate original fence matrix establishes the
actual controller-SA/RBAC boundary. These proofs must not be conflated.
