...
The native multi-connection gate is invoked explicitly with npm run test:corpus-pool --prefix skills/prism. It requires PRISM_TEST_PG_URL and fails visibly when absent; it is separate from the existing PGlite/default package suite, not silently skipped.
