...
The stored v3 manifest expected ref additionally requires namespace
...
and `Buffer.byteLength(bytes) == expectedRef.sizeBytes`. The parsed bytes must
equal the value being validated, and `final.testStageId` must equal the supplied
independent `gateStageId`. Before this API runs, the evidence adapter must retain
its current response-to-expectedRef equality and integrity checks. The registered
producer/consumer chain separately proves equality among write receipt,
StageResult, lifecycle artifact, expected ref and reopened response. A latest
artifact with the same resource cannot replace a mismatching expected complete
ref.
