...
and returns a clone. Digest calculation is a private implementation detail used
only after these same no-trap, closed unsigned-shape, semantic-relation and byte-
size checks. The package exports no unchecked standalone digest oracle. There is
no permissive mode.

`assertDeliveryManifestForRead` owns the only semantic version map and requires
the independently configured `gateStageId` for the unchanged
`final.testStageId` authority check:
...
Unknown versions reject. This API validates the supplied expected complete
manifest ref and stored bytes against `owner`, including the independent
manifest and gate stage IDs; callers cannot bypass it with only a parsed value.
It does not receive an ArtifactStore response or producer StageResult and cannot
claim equality with either. The evidence adapter retains its current
`verifiedArtifactJsonText(response, expectedRef)`, complete
`response.artifact == expectedRef`, digest and size checks before calling this
API. Equality between write receipt, StageResult, lifecycle artifact and reopen
response remains a registered-chain invariant outside this pure value/ref API.
The project-summary producer uses `createDeliveryManifestV3`; the evidence
adapter uses `assertDeliveryManifestForRead`. The producer's v2 branch stays
byte-for-byte source-equivalent to the existing builder. A parity test pins the
legacy v2 branch of the shared reader API to every currently accepted/rejected
reader fixture—including `final.testStageId == owner.gateStageId`—before
replacing the private `assertManifest` call.

## Exact closed v3 value contract

All manifest-owned objects listed here use `additionalProperties: false`.
Nested `GateCoverageV1` and `GateCoverageResultV1` retain their existing public
schemas and semantic validators exactly; in particular valid requirement
statements remain valid JSON strings under those owners and are not given a new
blanket control-character restriction. Other nested contract-owned values keep
their existing field-specific domains. `Digest` means exact
...
namespace/stage/artifact-prefix relationships specified below at both producer
and reader boundaries. Their total source bytes remain <=8 MiB as today.

The reader validates evidence independently; producer validation alone is not
authority. For `n = modules.length`, cardinality is exactly `3*n+4` without a
final Review and `3*n+6` with one. Positions and owners are fixed:

```text
for each module i, in manifest module order:
  3*i+0  implementation ref: namespace kubeclaw.implementation-agent,
         producer.stageId == modules[i].sourceStageId,
         artifactId starts "implementation:"
  3*i+1  decision ref: namespace kubeclaw.buster-quality-gate,
         producer.stageId == modules[i].testStageId,
         artifactId contains ":decision:"
  3*i+2  quality ref: namespace kubeclaw.buster-quality-gate,
         producer.stageId == modules[i].testStageId,
         artifactId does not contain ":decision:"
then:
  3*n+0  final implementation ref, same rules using final.sourceStageId
  3*n+1  final decision ref, same rules using final.testStageId
  3*n+2  final quality ref, same rules using final.testStageId
  3*n+3  lint ref: namespace kubeclaw.lint,
         producer.stageId == final.lintStageId
if final Review exists:
  3*n+4  report ref: namespace kubeclaw.review,
         producer.stageId == final.reviewStageId,
         artifactId starts "review-report:"
  3*n+5  bundle ref: namespace kubeclaw.review,
         producer.stageId == final.reviewStageId,
         artifactId starts "review-bundle:"
```

Decision and quality refs in each pair have the same complete producer attempt
identity. Review report and bundle likewise have the same complete producer
attempt identity. Every ref producer runId equals top-level runId. No additional,
missing or reordered ref is admitted. No two positions may contain the same
complete ref except the final implementation position: it must equal the
implementation ref of the exactly one module whose sourceStageId equals
`final.sourceStageId`. That is the sole permitted duplicate. These rules pin the
current selectors at the consumer and do not rely on the producer having run.

The stored v3 manifest expected ref additionally requires
`artifactId == "project-summary:" + manifest.runId`, namespace
...
configured Summary stage ID, size 1..8 MiB, `sha256(bytes) == expectedRef.digest`
and `Buffer.byteLength(bytes) == expectedRef.sizeBytes`. The evidence adapter's
existing request boundary also requires
`request.resource.canonicalId == expectedRef.artifactId`; a latest artifact with
the same resource cannot replace a mismatching expected complete ref. The parsed
bytes must equal the value being validated, and `final.testStageId` must equal
the supplied independent `gateStageId`. Before this API runs, the evidence
adapter retains its current response-to-expectedRef equality and integrity
checks. The registered producer/consumer chain separately proves equality among
write receipt, StageResult, lifecycle artifact, expected ref and reopened
response. The exact artifactId formula is new v3 strictness only: the legacy v2
reader keeps its current canonicalId-to-supplied-ref check and predicates and is
not retroactively narrowed to this formula.
...
1. **Contract and no-trap.** Exhaust every required/optional field, bounds,
   conditional Review cross-product, 1/128/129 modules, 7/9 and 388/390 evidence
