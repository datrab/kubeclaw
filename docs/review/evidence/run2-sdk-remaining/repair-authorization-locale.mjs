// Current repair-identity regression, not complete pipeline/native acceptance.
// The original failing producer is retained with hashes in the immutable
// tests/fixtures/repair-budget/legacy-core.json archive.
...
if (mode === '--produce') {
  ...
  try {
    ...
    assert.equal(swedish.originalDigest, swedish.recoveredDigest);
    assert.equal(swedish.authorized, true);
    assert.equal(swedish.error, undefined);
    assert.deepEqual(fs.readFileSync(stored.journal), originalJournal);
    process.stdout.write(`${JSON.stringify({ repairIdentityRegressionPassed: true, producer, english, swedish,
