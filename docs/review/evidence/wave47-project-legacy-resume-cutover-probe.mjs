...
const oldCore=path.resolve(process.argv[3]);
...
const hook=String.raw`
  ...
  fs.writeFileSync(path.join(legacyProjectRoot,'source.ts'),execFileSync('git',['show','b6b2b1b:skills/nova/project/source.ts'],{cwd:root}));
  fs.writeFileSync(path.join(legacyProjectRoot,'compiler.ts'),execFileSync('git',['show','b6b2b1b:skills/nova/project/compiler.ts'],{cwd:root}));
  ...
  try{
    fs.writeFileSync(unfixedCLI,execFileSync('git',['show','b6b2b1b:skills/nova/project/cli.ts'],{cwd:root}),{flag:'wx'});
