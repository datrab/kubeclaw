...
async function scenario(mixed) {
  ...
  const source={identityEncoding:PORTABLE_JSON_ENCODING,projectId:'project',repositoryRoot:repository,architectureRef:'main',paths:['architecture.md','modules/api/FORGE.md']};
  const contract={projectId:'project',baseRevision:revision,requiredFiles:['architecture.md'],modules:[{moduleId:'api',modulePath:'modules/api',ownedPaths:['api.mjs'],serveDockerfile:'api.mjs',apiSpecFile:null}],policy:mixed?{I:1,i:2}:{}};
  ...
  const bytes=portableJson(expected),digest=sha256Text(bytes);
  ...
  try {
    ...
    const ref=sourceResult.artifacts[0];assert.equal(ref.encoding,PORTABLE_JSON_ENCODING);assert.equal(ref.digest,digest);
...
if(process.argv[2]==='child')await scenario(process.argv[3]==='mixed');
else for(const kind of ['fixed','mixed']) {
  const r=spawnSync(process.execPath,[self,'child',kind],{cwd:checkout,env:{...process.env,LANG:'en_US.UTF-8',LC_ALL:'en_US.UTF-8'},encoding:'utf8',maxBuffer:4*1024*1024});
