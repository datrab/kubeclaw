...
| `prism-product-lint.txt` | focused Product-authority ESLint command listed in the implementation report | 0; empty successful output |
| `prism-lint.txt` | same focused command plus the existing `skills/prism/server/control-server.ts` monolith | 1; 25 pre-existing whole-file complexity/length errors retained, not used as a passing gate |
