...
When resuming documentation work, first read the relevant topic-map row or audit artifact, then inspect the named source files before editing. If the prompt conflicts with the topic map or changelog, prefer the newer artifact and record the divergence in the changelog.
...
- A topic-map row is marked `rich` while its pages lack commands, artifacts/state, failure signals, or verification.
