...
Status: placeholder for next full review
...
This file tracks findings from the next documentation-driven implementation review.
...
| B00a-ISSUE-001 | Buster conventions still instruct subagents to update `status.json` directly | Resolved | Medium | Small | `check-buster-pipeline-slice-surface.mjs` conventions drift assertions passed | `skills/buster/CONVENTIONS.md`, `tests/verification/contracts/check-buster-pipeline-slice-surface.mjs`, implementation maps |
...
| OI-38 | Nova compatibility shim files duplicate canonical common pipeline package surfaces | Later | Low | Small | Keep runtime collision/shim coverage proving common overwrites shims | `skills/nova/pipeline/*` common compatibility shims, `skills/common/pipeline/` |
...
- `kubeclaw-main/tests/verification/behavior/areas/foundations.mjs` — added coverage proving `.swarm/modules/<id>/status.json`, `.swarm/logs/...`, gate status, summary, and project-summary paths are treated as runtime-state paths with relative, slash-prefixed, and repo-prefixed forms where applicable.
...
Git `--name-only` paths are repo-relative, such as `.swarm/modules/01/status.json`. Current `isRuntimeStatePath()` requires patterns containing `/.swarm/...`, so it returns `false` for relative runtime-state paths and `true` only for slash-prefixed paths.

Live reproduction during P03:

```text
.swarm/modules/01/status.json => false
/.swarm/modules/01/status.json => true
...
- `kubeclaw-main/skills/nova/pipeline/services/telemetry/payload-schema.ts` — compatibility shim that re-exports the common schema owner.
...
- `kubeclaw-main/skills/nova/pipeline/services/redis-message-contract.ts` — compatibility shim to the common Redis message contract.
...
- `kubeclaw-main/skills/nova/pipeline/services/polling.ts` — added artifact-only Forge completion polling; `status.json` is no longer a Forge completion signal.
...
Resolved behavior: Forge module agents write the typed `forge-completion.json` artifact with `status: READY_FOR_TESTING` or `status: BLOCKED`, then stop. Gate/review fix agents stop after making local changes. Nova pipeline owns lifecycle transitions and git synchronization. Forge completion polling reads the completion artifact only; `status.json` is not treated as a Forge completion signal.
...
## B00a-ISSUE-001 — Buster conventions still instruct subagents to update `status.json` directly
...
- `kubeclaw-main/skills/buster/CONVENTIONS.md` — replaced stale direct `status.json` workflow guidance with the artifact-only module/gate completion contract.
...
- `kubeclaw-main/tests/verification/contracts/check-buster-pipeline-slice-surface.mjs` — added docs drift assertions preventing direct `status.json` update instructions from returning to Buster conventions.
...
- `kubeclaw-main/docs/pipeline/implementation-map/data-schemas.md` — clarified child-agent completion schema excludes direct `status_json_path` / `status.json` edits.
- `kubeclaw-main/docs/pipeline/implementation-map/path-construction.md` — aligned legacy `status_json_path` notes with README and CONVENTIONS.
...
The Buster runtime and README have moved to artifact-based completion authority. At the time of B00a, module-test subagents wrote a prompt-provided result artifact and gate-test subagents wrote `output_file`; B02a later normalized both task types to required `output_file`. In all cases, subagents must not edit `status_json_path`; Buster Pipeline reads the artifact, performs verify/push and cleanup, and emits canonical completion. However, `CONVENTIONS.md` still contained stale workflow text telling the subagent to update `status.json` using a prompt-provided lifecycle update command.

This created a documentation conflict in the Buster subagent operating contract. Agents or operators following `CONVENTIONS.md` could treat `status.json` as a direct child-agent completion target even though current authority belongs to result artifacts and Buster Pipeline completion.
...
Resolved by updating `CONVENTIONS.md` workflow step 5 to require only the prompt-provided artifact, to forbid `status_json_path` / `status.json` edits, and to stop after artifact/result-file write. B02a later normalized the artifact field to `output_file` for both module and gate tasks. Focused verification added conventions drift assertions to `check-buster-pipeline-slice-surface.mjs` and passed.
...
- `kubeclaw-main/skills/buster/pipeline/services/pipeline-helpers.ts` — removed completion fallback to `result_artifact_path`, `result_file`, or `status_json_path`; added required `output_file` read/write helpers.
...
During decision review this was expanded into the current Buster completion contract problem: Buster emitted Redis completion after reading local artifacts, but before any Buster-owned scoped push. That allowed Nova to consume completion before the Buster artifact was guaranteed to be on Git. Buster also had too many artifact names/fallbacks (`result_artifact_path`, `result_file`, `output_file`, and `status_json_path` fallback), which weakened the intended separation where status JSON is only an operator/projection artifact.
...
- `kubeclaw-main/skills/nova/pipeline/services/telemetry/payload-schema.ts` — changed to a compatibility shim that re-exports the common schema owner.
- `kubeclaw-main/skills/buster/pipeline/services/telemetry/payload-schema.ts` — added the Buster-side compatibility shim for the common schema owner.
...
- `kubeclaw-main/skills/nova/pipeline/services/acp-gateway-contract.ts` — added Nova compatibility shim for the common contract owner.
- `kubeclaw-main/skills/buster/pipeline/services/acp-gateway-contract.ts` — added Buster compatibility shim for the common contract owner.
...
- `kubeclaw-main/skills/nova/pipeline/services/status-store.ts` — `loadStatus()` now projects lifecycle read models only; `status.json` remains write-only diagnostic output for the pipeline.
- `kubeclaw-main/skills/nova/pipeline/services/status-store-compat.ts` and `status-store-compat/module-projection.js` — verified compatibility helpers no longer implicitly read legacy module status unless explicit diagnostic evidence is supplied.
...
- `kubeclaw-main/tests/verification/behavior/areas/restart-recovery.mjs` and `polling.mjs` — seeded lifecycle authority explicitly and covered recovery/polling without `status.json` reads.

### Problem / in-depth issue description

Imported from archived tracker OI-34. Legacy `status.json` and `active-session.json` fallback logic needs an explicit milestone-based cutoff. The intended model is that lifecycle/read-model state owns module truth and active dispatch/session lease owns live session truth, while legacy files are diagnostic/operator evidence only.
...
Resolved by cutting off pipeline reads of module `status.json`: `loadStatus()` now returns lifecycle read-model projection only, scheduler projections do not implicitly read legacy status evidence, and gate file-only active-session evidence no longer becomes recovery identity. Focused verification passed: status-store contract, gate active-session contract, restart-recovery, polling, foundations, and lifecycle-state-surface.
...
- `kubeclaw-main/skills/nova/pipeline/services/task-transport-contract.ts` — added Nova compatibility shim to the canonical common transport contract.
- `kubeclaw-main/skills/buster/pipeline/services/task-transport-contract.ts` — added Buster compatibility shim to the canonical common transport contract.
- `kubeclaw-main/skills/nova/pipeline/tools/redis.ts` — migrated task publishing to `publishTask(...)` through the TaskQueue adapter while keeping `sendTask(...)` as compatibility alias.
...
- `kubeclaw-main/skills/buster/pipeline/tools/redis.ts` — migrated legacy task send path to the TaskQueue adapter while keeping `sendTask(...)` as compatibility alias.
...
- `kubeclaw-main/docs/pipeline/implementation-map/authority-map.md` — updated living authority rows for TaskQueue/EventBus ownership and compatibility aliases.
...
## OI-38 — Nova compatibility shim files duplicate canonical common pipeline package surfaces
...
- `kubeclaw-main/skills/nova/pipeline/agents/acp-monitor.ts` — Nova-local compatibility shim for common implementation.
- `kubeclaw-main/skills/nova/pipeline/agents/lifecycle.ts` — Nova-local compatibility shim for common implementation.
- `kubeclaw-main/skills/nova/pipeline/agents/runtime.ts` — Nova-local compatibility shim for common implementation.
- `kubeclaw-main/skills/nova/pipeline/integrations/discord-webhook.ts` — Nova-local compatibility shim for common implementation.
- `kubeclaw-main/skills/nova/pipeline/lifecycle-state.ts` — Nova-local compatibility shim for common implementation.
- `kubeclaw-main/skills/nova/pipeline/security.ts` — Nova-local compatibility shim for common implementation.
- `kubeclaw-main/skills/nova/pipeline/telemetry.ts` — Nova-local compatibility shim for common implementation.
- `kubeclaw-main/skills/common/pipeline/` — canonical shared helper owner.

### Problem / in-depth issue description

Imported from archived tracker OI-38. Nova-local compatibility shims duplicate package surfaces for canonical `skills/common/pipeline/*` helpers.
...
- `kubeclaw-main/skills/nova/pipeline/services/status-store-compat/gate-projection.ts` — delegates gate output read-model path projection to `gateOutputPath()`.
...
- `kubeclaw-main/skills/nova/pipeline/agents/tracked-agents.ts` — Nova compatibility shim for the new common owner.
- `kubeclaw-main/skills/buster/pipeline/agents/tracked-agents.ts` — Buster compatibility shim for the new common owner.
...
- `kubeclaw-main/skills/nova/pipeline/services/pipeline-event-contract.ts` — Nova compatibility shim for the common event contract.
- `kubeclaw-main/skills/buster/pipeline/services/pipeline-event-contract.ts` — Buster compatibility shim for the common event contract.
