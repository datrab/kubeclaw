...
Deployment values still use moving KubeClaw runtime image tags. The OpenClaw build base and tool dependencies are pinned, and CI publishes immutable SHA tags, but a redeploy can still resolve a different KubeClaw image without a values change.
