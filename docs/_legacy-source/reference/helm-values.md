...
Generated from: `charts/kubeclaw/values.yaml`, `my-values/nova-values.yaml`, `my-values/buster-values.yaml`, `my-values/infra/redis-values.yaml`, `my-values/infra/postgresql-values.yaml`, `my-values/infra/litellm-values.yaml`, `my-values/infra/litellm-config.yaml`, `my-values/infra/litellm-deployment.yaml`, `my-values/infra/tailscale-operator-values.yaml`, `my-values/infra/registry-mirror.yaml`, `my-values/infra/registry-local.yaml`, `my-values/infra/network-policies.yaml`, `my-values/infra/buster-namespace-fence.yaml`

## Values And Manifest Files

| File | Top-level keys | Secret references |
| --- | --- | --- |
| `charts/kubeclaw/values.yaml` | `agentRole`, `image`, `imagePullSecrets`, `codeBundle`, `runtimeInfrastructure`, `gateway`, `initSetup`, `pluginSeed`, `capabilityProviders`, `replicaCount`, `workerTrust`, `auth`, `litellm`, `stitch`, `discord`, `discordWebhook`, `agent`, `redis`, `workspace`, `service`, `persistence`, `resources`, `runAsRoot`, `probes`, `shutdown`, `serviceAccount`, `busterNamespaceBroker`, `commands`, `swarmConfig`, `swarmConfigJson`, `semgrepConfigYaml`, `eslintConfigMjs`, `customSkills`, `nodeSelector`, `tolerations`, `podAnnotations`, `extraEnv`, `extraContainers`, `archviewer`, `extraVolumes`, `extraVolumeMounts`, `busterRuntimePersistence` | `secretName: redis-secrets` at line 231 |
| `my-values/nova-values.yaml` | `agentRole`, `gateway`, `initSetup`, `image`, `imagePullSecrets`, `codeBundle`, `auth`, `stitch`, `litellm`, `discord`, `discordWebhook`, `commands`, `agent`, `serviceAccount`, `busterNamespaceBroker`, `archviewer`, `capabilityProviders`, `workerTrust`, `extraEnv`, `resources`, `probes`, `extraContainers`, `extraVolumes`, `extraVolumeMounts`, `workspace` | `existingSecret: github-bundle-reader` at line 37<br>`existingSecret: openclaw-shared-secrets` at line 41<br>`existingSecret: openclaw-shared-secrets` at line 45<br>`existingSecret: openclaw-shared-secrets` at line 49<br>`existingSecret: openclaw-shared-secrets` at line 54<br>`secretName: openclaw-shared-secrets` at line 59<br>`secretName: git-deploy-key-nova` at line 73<br>`existingSecret: nova-archviewer-auth` at line 89 |
| `my-values/buster-values.yaml` | `agentRole`, `gateway`, `initSetup`, `image`, `imagePullSecrets`, `codeBundle`, `auth`, `litellm`, `discord`, `commands`, `discordWebhook`, `agent`, `serviceAccount`, `workerTrust`, `service`, `extraContainers`, `extraVolumes`, `busterNamespaceBroker`, `resources`, `workspace`, `busterRuntimePersistence` | `existingSecret: github-bundle-reader` at line 34<br>`existingSecret: openclaw-shared-secrets` at line 38<br>`existingSecret: openclaw-shared-secrets` at line 42<br>`existingSecret: openclaw-shared-secrets` at line 47<br>`secretName: openclaw-shared-secrets` at line 61<br>`secretName: git-deploy-key-buster` at line 68 |
| `my-values/infra/redis-values.yaml` | `image`, `architecture`, `commonConfiguration`, `auth`, `master` | `existingSecret: redis-secrets` at line 22 |
| `my-values/infra/postgresql-values.yaml` | `image`, `architecture`, `auth`, `primary` | `existingSecret: postgresql-secrets` at line 10 |
...
| `my-values/infra/litellm-deployment.yaml` | `apiVersion`, `kind`, `metadata`, `spec` | `secretName: google-sa-key` at line 81 |
...
| `my-values/infra/registry-local.yaml` | `apiVersion`, `kind`, `metadata`, `spec`, `data` |  |
