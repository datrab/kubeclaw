...
## Candidate lint and observer recovery

Compiled lint stages now resolve the implementation artifact's exact commit and execute against an isolated Git checkout. The stored report includes that revision. Real Git/shell checks prove selection remains correct with newer HEAD and that temporary checkouts are removed after errors; CI's real shellcheck/shfmt tests also compare a committed defect and its repair against contradictory working-directory contents. This does not provision project dependencies or establish complete multi-module live acceptance.

Observer retry budgets now count durable attempts across repeated drains and restarts. Exhaustion retains evidence and reports an explicit failure without silently retrying or advancing that run's checkpoint. Completed delivery records can rebuild a missing checkpoint without executing the observer again. The regression uses an installed filesystem-writing observer, real OS write failure, durable journals and reconstructed runtimes; it fails against the prior implementation. Uncertain delivery outcomes, explicit operator replay of exhausted deliveries, projection retention, and removal of required observer writes from execution authority remain separate open work.

The E2E evidence reader now uses the production durable record and blob stores instead of the removed JSONL catalog and `.json` blob layout. It validates blob integrity and byte counts, filters by run identity, and selects the latest attempt per stage so repaired runs are not judged against superseded failures. A mandatory CI regression writes through the real artifact adapter, reads those bytes through acceptance code, and rejects corruption.

## Result capacity admission

Buster reserves up to `maximumResultBytes` for each new job before accepting it. A cross-process admission lock prevents competing submissions from oversubscribing `maximumResultStoreBytes`. Completed jobs count their actual result bytes; unfinished or failed jobs conservatively keep their reservation, including across reconstruction. Existing idempotent submissions remain readable at capacity; new admissions receive HTTP 429 with `BUSTER_REMOTE_RESULT_CAPACITY_EXCEEDED`. This reserves the configured result-byte budget, not physical filesystem space, extracted workspaces or future metadata growth. Retention/import acknowledgement and reclamation remain open; operators must not delete recovery evidence just to admit work.

Completion now validates job, plan, plan digest and run identity before storing result bytes, and rejects state/digest conflicts before writing. Competing completion writes are serialized. A real Git archive and signed source snapshot feed the reservation regression; competing child processes cannot accept both jobs into one remaining slot. The same test fails against the previous implementation. The real isolated-provider CI check also rejects attaching one completed job's actual result to another job.
