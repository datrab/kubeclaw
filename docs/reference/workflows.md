...
| --- | --- | --- | --- | --- | --- |
| `.github/workflows/build-images.yaml` (Build Runtime Images And Skill Bundles) | `push`, `schedule`, `workflow_dispatch` |  | `0 3 * * *` | `detect-build-inputs`, `check-base-image`, `build`, `bundle-skills`, `prism-live-acceptance` | `bash scripts/deploy.sh prism-e2e`<br>`npm ci --ignore-scripts`<br>`npm run plugin-system:sandbox:build`<br>`uses: actions/attest-build-provenance@v4`<br>`uses: actions/checkout@v6`<br>`uses: actions/setup-node@v5`<br>`uses: aquasecurity/trivy-action@0.33.1`<br>`uses: azure/setup-helm@v4`<br>`uses: azure/setup-kubectl@v4`<br>`uses: docker/build-push-action@v7`<br>`uses: docker/login-action@v4`<br>`uses: docker/metadata-action@v6`<br>`uses: docker/setup-buildx-action@v4`<br>`uses: dorny/paths-filter@v3` |
