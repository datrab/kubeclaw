...
| `.github/workflows/build-ops-mcp.yaml` (Build Ops MCP Image) | `pull_request`, `push`, `workflow_dispatch` | `tools/ops-mcp/**`<br>`.github/workflows/build-ops-mcp.yaml` |  | `build` | `uses: actions/checkout@v6`<br>`uses: docker/build-push-action@v7`<br>`uses: docker/login-action@v4`<br>`uses: docker/metadata-action@v6`<br>`uses: docker/setup-buildx-action@v4` |
