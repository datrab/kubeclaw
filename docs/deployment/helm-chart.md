...
- `deployment.yaml` owns init sync, OpenClaw config migration, skills merge, main container env, probes, the dedicated rootless BuildKit Buster sidecar, and extra sidecars.
...
The rendered services include gateway and bridge ports for each agent. Nova adds `prism-preview` as an extra service port from production values. Buster adds the dedicated pipeline sidecar, BuildKit state, result staging, lease-client RBAC, and namespace controller.
...
| Storage | `persistence.*`, `configPersistence.*`, `busterPipeline.storageSize`, `busterPipeline.resultsSize` | PVCs, workspace/config mounts, Buster transient BuildKit/result storage |
