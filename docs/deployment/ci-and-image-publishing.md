...
| Code bundle selection | `scripts/deploy.sh`; `charts/kubeclaw/templates/deployment.yaml` | `code` deploy derives `https://github.com/<repo>/releases/download/agent-code-bundles/<role>-<sha>.tgz` by default when only `*_CODE_BUNDLE_EXPECTED_COMMIT` is set; explicit archive URLs still override that path |
