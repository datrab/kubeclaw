...
The shared runtime is where cross-role contracts should live. Examples include telemetry payload schema, Redis message contracts, Discord field contracts, lifecycle-state helpers, agent semantic-artifact publication, and security utilities. Keep role-specific policy in Nova or Buster, and keep shared message and artifact validation in common code so current and future roles fail consistently.
...
| Agent semantic artifact publication | `skills/common/pipeline/agent-artifact.ts`; role-local facades and writer tools | combines a pipeline-created immutable identity context with agent semantic results | run/module/gate/attempt/dispatch/schema/path context plus semantic payload | validated atomic watched artifact; immutable fields cannot be agent-overridden | `tests/skills/common/pipeline/agent-artifact.test.mjs`; role prompt/writer tests |
...
| OpenClaw observer plugin | `skills/common/plugins/openclaw-agent-observer/src/index.ts`; `hook-normalizers.ts`; `redis-writer.ts`; `redis-transport.ts` | gateway-owned capture and delivery of OpenClaw native-subagent and ACP hook/model/runtime events through the neutral `contracts/agent-observability/v1` boundary | gateway plugin entry, `agent_observability.ingester.*`, Redis env | `openclaw.agent.*`, `openclaw.llm.*`, `openclaw.tool.*`, dead-letter stream | observer plugin and bridge-contract conformance tests; deployment truth verifies the self-contained contract materialization and gateway enablement |
| Helm deployment surface | `charts/kubeclaw/templates/deployment.yaml`; `pvc.yaml`; `configmap-swarm-config.yaml`; production values | renders agent pods, retained config/workspace PVCs, runtime config overlays, and the dedicated Buster pipeline worker | `persistence.*`, `busterPipeline.*`, `gateway.url`, `service.*`, Secret refs | Kubernetes Deployments, PVCs, ConfigMaps, Services, NetworkPolicies | `tests/verification/deployment/check-deployment-truth.mjs` |
