...
| OpenClaw observer plugin | `plugins/openclaw-agent-observer/src/index.ts`; `hook-normalizers.ts`; `redis-writer.ts`; `redis-transport.ts` | gateway-owned normalization of OpenClaw native-subagent and ACP hook/model/runtime events; Buster enables it permanently in gateway config | gateway plugin entry, `agent_observability.ingester.*`, Redis env | `openclaw.agent.*`, `openclaw.llm.*`, `openclaw.tool.*`, dead-letter stream | observer plugin tests; deployment truth verifies plugin source is packaged and Buster enables it |
