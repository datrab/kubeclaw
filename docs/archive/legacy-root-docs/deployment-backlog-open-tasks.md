...
- Agent Helm deployment template already injects `REDIS_PASSWORD` from `.Values.redis.secretName` / `.Values.redis.secretKey` into the main `kubeclaw` container.
...
- [x] Helm env wiring: chart template injects `REDIS_PASSWORD` via `secretKeyRef` for the main agent container.
...
- [x] Processor cleanup: removed the legacy stream-processor sidecar, processor ConfigMap template, and processor values because `buster-pipeline.ts` owns Redis task consumption.
