...
For automatic runtime deployment after a source PR merges, use the
[agent bundle and image deployment contract](automatic-runtime-deployments.md).
It selects code bundles independently of compatible environment images and
publishes Argo manifests without another release-selection PR. The manual image
promotion path below remains available for initial/legacy selection and Ops.
For a runtime selection with `runtime-code.json`, chart/value matching uses the
code source commit; image provenance remains bound to the original image build.
