...
| --- | --- | --- | --- | --- |
| New CLI flag or operator command | `skills/nova/pipeline/cli.ts`; `skills/nova/pipeline/cli-args.ts` | normalized flags must stay JSON-safe and bounded by `sanitizeJsonEgress` | CLI JSON output or process exit code only at boundary | `node --test tests/skills/nova/pipeline/cli.test.mjs tests/skills/common/pipeline/cli-args.test.mjs`; strict CLI contract check |
